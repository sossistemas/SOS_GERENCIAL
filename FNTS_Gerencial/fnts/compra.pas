...
uses
  ...
  AdvOfficeStatusBar, wwdblook, RzEdit, RzDBEdit, RzDBBnEd,
  AdvGlowButton, AdvToolBar, AdvToolBarStylers, AdvPreviewMenu, Grids, Wwdbigrd,
  Wwdbgrid, ImgList, RzCmboBx, DB, ZAbstractRODataset, ZAbstractDataset,
...
{$ENDIF}, ZDataset, AdvPreviewMenuStylers, RzDBCmbo, AdvMenus, AdvMenuStylers,
    AdvGrid, DBAdvGrid, Menus, PageView, AdvShapeButton,
...
type
  Tfrmcompra = class(TForm)
    ...
    qrnota_itemFATOR: TFloatField;
    qrnota_itemTIPOFATOR: TWideStringField;
    qrnota_itemCSOSN: TWideStringField;
    qrnota_itemCFOP_CAD: TWideStringField;
    qrnota_itemVBCSTRET: TFloatField;
    qrnota_itemPST: TFloatField;
    qrnota_itemVICMSSTRET: TFloatField;
    qrnota_itemPREDBCEFET: TFloatField;
    qrnota_itemVBCEFET: TFloatField;
    qrnota_itemPICMSEFET: TFloatField;
    qrnota_itemVICMSEFET: TFloatField;
    qrnota_itemQTDVENDIDO: TFloatField;
    qrnota_itemID_CONVERSOR_UNIDADE: TIntegerField;
    qrnota_itemDESCRICAO_FATOR: TWideStringField;
    qrnota_itemID_CONVERSAO_UNIDADE: TIntegerField;
    qrnota_itemATB: TWideStringField;
...
uses
  modulo, principal, xloc_fornecedor, xloc_cfop, compra_item,
  compra_menu, xloc_modelo, loc_conta;
...
begin
  ...
  begin
    query.close;
    query.sql.clear;
    query.sql.add('delete from c000088 where atb like :atb and notafiscal = ''' + enumero.Text + '''');
    query.sql.add('and codfornecedor = ''' + ECODFORNECEDOR.Text + '''');
    query.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
    query.ExecSQL;

    qrpagar.close;
    qrpagar.sql.clear;
    qrpagar.sql.add('DELETE FROM C000046 WHERE atb like :atb and CODNOTA = ''' + frmcompra_menu.QRNOTA.FIELDBYNAME('CODIGO').ASSTRING + '''');
...
begin
  ...
  frmcompra_menu.qrempresa.sql.add('select * from c000004 WHERE atb like :atb');
  frmcompra_menu.qrempresa.ParamByName('atb').Value := ME_FiltraATB('TB_EMPRESA');
  ...
  frmcompra_menu.qrfornecedor.sql.add('select * from c000009 where atb like :atb and codigo = ''' + frmcompra_menu.qrnotaCODFORNECEDOR.Value + '''');
  frmcompra_menu.qrfornecedor.ParamByName('atb').Value := ME_FiltraATB('TB_FORNECEDOR');
  ...
  qrnota_item_lista.sql.add('where item.atb like :atbitem and produto.atb like :atbproduto and item.codproduto = produto.codigo');
  qrnota_item_lista.sql.add('and codnota = ''' + frmcompra_menu.QRNOTA.FIELDBYNAME('codigo').ASSTRING + ''' order by item');
  qrnota_item_lista.ParamByName('atbitem').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
  qrnota_item_lista.ParamByName('atbproduto').Value := ME_FiltraATB('TB_PRODUTO');
  ...
  begin
    ...
    if (ecodmodelo.Text = '07') or (ecodmodelo.Text = '08') or (ecodmodelo.Text = '09') or (ecodmodelo.Text = '10') or (ecodmodelo.Text = '11') then
    begin
  ...
  qrpagar.close;
  qrpagar.sql.clear;
  qrpagar.sql.add('SELECT * FROM C000046 WHERE atb like :atb and CODNOTA = ''' + frmcompra_menu.QRNOTA.FIELDBYNAME('NUMERO').ASSTRING + ''' ORDER BY DATA_VENCIMENTO');
  qrpagar.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
  qrpagar.open;
  ...
  begin
    ...
    query.close;
    query.sql.clear;
    query.sql.add('select * from c000032 where atb like :atb and numeronota = ''' + frmcompra_menu.QRNOTA.FIELDBYNAME('numero').ASSTRING + '''');
    ...
    begin
      ...
      begin
        ...
        if (query.FIELDBYNAME('cst').Value = '') or (query.FIELDBYNAME('cst').Value = null) then
        ...
        frmcompra.qrnota_itemIPI_TIPO.ASSTRING := '0.Não Tem IPI';
        frmcompra.qrnota_itemATB.ASSTRING := ME_GravaATB('TB_NOTA_ENTRADA_ITENS');
...
begin
  ...
  begin
    qrnf.close;
    qrnf.SQL.clear;
    qrnf.SQL.Add('select * from c000087 where atb like :atb and numero = ''' + frmcompra_menu.qrnota.fieldbyname('numero').asstring + '''');
    ...
    qrnf.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA');
    qrnf.open;
  ...
  begin
    ...
    if ((frmcompra_menu.qrnota.FieldByName('TOTAL_produtos').asfloat - frmcompra_menu.qrnota.FieldByName('conf_TOTALprodutos').asfloat) > 0.10) or ((frmcompra_menu.qrnota.FieldByName('TOTAL_produtos').asfloat - frmcompra_menu.qrnota.FieldByName('CONF_TOTALprodutos').asfloat) < (-0.10)) then
    begin
      texto := PWideChar('Foi alterado alguma Informação na Nota Fiscal de Entrada' + #13 + 'ou há Inconsistência no valor total dos Produtos.' + #13 + 'Por isso, esta nota precisa ser gravada para sua finalização!' + #13 + 'Deseja prosseguir com a finalização?' + #13 + 'Diferença: R$' + formatfloat('###,###,##0.00', (frmcompra_menu.qrnota.FieldByName('TOTAL_NOTA').asfloat - frmcompra_menu.qrnota.FieldByName('conf_TOTALNOTA').asfloat)));
   ...
   (********************** verificar total do ipi ***********************)
    ...
    if ((frmcompra_menu.qrnota.FieldByName('ipi').asfloat - frmcompra_menu.qrnota.FieldByName('conf_ipi').asfloat) > 0.10) or ((frmcompra_menu.qrnota.FieldByName('ipi').asfloat - frmcompra_menu.qrnota.FieldByName('CONF_ipi').asfloat) < (-0.10)) then
    begin
      texto := PWideChar('Houve inconsistência no valor total do IPI!' + #13 + 'Diferença: R$' + formatfloat('###,###,##0.00', (frmcompra_menu.qrnota.FieldByName('TOTAL_NOTA').asfloat - frmcompra_menu.qrnota.FieldByName('conf_TOTALNOTA').asfloat)) + #13 + 'Deseja prosseguir com a finalização?');
  ...
  qrnota_item.sql.add('select * from c000088 where atb like :atb and codproduto is not null and codnota = ''' + frmcompra_menu.qrnota.fieldbyname('codigo').asstring + ''' order by item');
  ...
  begin
    ...
    FRMMODULO.QRPRODUTO_MOV.SQL.ADD('SELECT * FROM C000032 WHERE atb like :atb and CODNOTA = ''XXXXXX''');
    FRMMODULO.QRPRODUTO_MOV.OPEN;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FRMMODULO.QRPRODUTO_MOV.CLOSE;
        FRMMODULO.QRPRODUTO_MOV.SQL.CLEAR;
        FRMMODULO.QRPRODUTO_MOV.SQL.ADD('SELECT * FROM C000032 WHERE atb like :atb and CODNOTA = ' + QuotedStr(qrnota_itemCODNOTA.AsString + 'NE'));
        ...
        begin
          ...
          FRMMODULO.QRPRODUTO_MOV.CLOSE;
          FRMMODULO.QRPRODUTO_MOV.SQL.CLEAR;
          FRMMODULO.QRPRODUTO_MOV.SQL.ADD('SELECT * FROM C000032 WHERE atb like :atb and CODNOTA = ''' + qrnota_item.FIELDBYNAME('CODNOTA').ASSTRING + 'NE' + '''');
        ...
        begin
          ...
          query.Close;
          query.sql.clear;
          query.sql.add('select codigo, produto from c000025 where atb like :atb and codigo = ''' + qrnota_item.fieldbyname('codproduto').asstring + '''');
...
begin
  if (trim(eserie.Text) = '0') or (trim(eserie.Text) = '00') or (trim(eserie.Text) = '000') then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      frmcompra_menu.qrfornecedor.close;
      frmcompra_menu.qrfornecedor.sql.clear;
      frmcompra_menu.qrfornecedor.sql.add('select * from c000009 where atb like :atb and codigo = ''' + ECODFORNECEDOR.Text + '''');
      frmcompra_menu.qrfornecedor.ParamByName('atb').Value := ME_FiltraATB('TB_FORNECEDOR');
      frmcompra_menu.qrfornecedor.open;
      ...
      else if (frmcompra_menu.qrfornecedorCNPJ.Value = '') or (frmcompra_menu.qrfornecedorUF.Value = '') or (frmcompra_menu.qrfornecedorIE.Value = '') then
      begin
...
begin
  ...
  begin
    frmcompra_menu.qrfornecedor.close;
    frmcompra_menu.qrfornecedor.sql.clear;
    frmcompra_menu.qrfornecedor.sql.add('select * from c000009 where atb like :atb and codigo = ''' + resultado_pesquisa1 + '''');
    frmcompra_menu.qrfornecedor.ParamByName('atb').Value := ME_FiltraATB('TB_FORNECEDOR');
    frmcompra_menu.qrfornecedor.open;
    frmcompra_menu.QRNOTA.FIELDBYNAME('codfornecedor').ASSTRING := resultado_pesquisa1;

    if (frmcompra_menu.qrfornecedorCNPJ.Value = '') or (frmcompra_menu.qrfornecedorUF.Value = '') or (frmcompra_menu.qrfornecedorIE.Value = '') then
    begin
      showmessage('Atenção! Dados de Fornecedor Inconsistente...');
      exit;
    end;
...
begin
  ...
  begin
    if XDESCONTO.checked then
      frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_TOTALNOTA').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_ICMSRETIDO').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_TOTALPRODUTOS').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_FRETE').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_SEGURO').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_OUTRAS').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_ITEM_ESPECIAL').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_IPI').asfloat - frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_DESCONTO').asfloat
    else
      frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_TOTALNOTA').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_ICMSRETIDO').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_TOTALPRODUTOS').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_FRETE').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_SEGURO').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_OUTRAS').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_ITEM_ESPECIAL').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_IPI').asfloat;
  end
  else
  begin
    if XDESCONTO.checked then
      frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_TOTALNOTA').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_VALORSUB').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_TOTALPRODUTOS').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_FRETE').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_SEGURO').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_OUTRAS').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_ITEM_ESPECIAL').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_IPI').asfloat - frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_DESCONTO').asfloat
    else
      frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_TOTALNOTA').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_VALORSUB').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_TOTALPRODUTOS').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_FRETE').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_SEGURO').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_OUTRAS').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('conf_ITEM_ESPECIAL').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_IPI').asfloat;
  end;
...
begin
  ...
  begin
    ...
    if frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_VALORICMS').asfloat > 0 then
      frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_ICMSREDITO_PERC').asfloat := (frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_ICMSRETIDO').asfloat * 100) / frmcompra_menu.QRNOTA.FIELDBYNAME('CONF_VALORICMS').asfloat;
...
begin
  ...
  vprodutos := 0;
  vtotal := 0;
  codg := frmprincipal.codifica('000088');
  qrnota_item.close;
  qrnota_item.sql.clear;
  qrnota_item.sql.add('select * from c000088 where atb like :atb and codigo = ''' + codg + ''' and codnota = ''' + qrnota_item_lista.FIELDBYNAME('codnota').ASSTRING + '''');
  qrnota_item.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
  qrnota_item.open;
  ...
  qrnota_itemATB.ASSTRING := ME_GravaATB('TB_NOTA_ENTRADA_ITENS');
...
begin
  ...
  begin
    ...
    frmcompra_menu.QRNOTA.FIELDBYNAME('ICMS_ISENTO').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('ICMS_ISENTO').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('ICMS_ISENTO').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('ICMS_NAOTRIBUTADO').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('ICMS_OUTRAS').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('ICMS_OUTRAS').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('ICMS_OUTRAS').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('BASE_ICMS').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('BASE_ICMS').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('icms_base').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('valor_ICMS').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('valor_ICMS').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('icms_valor').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('BASE_sub').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('Base_sub').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('sub_base').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('valor_sub').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('valor_sub').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('sub_valor').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('frete').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('frete').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('frete').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('seguro').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('seguro').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('seguro').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('outras').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('outras').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('outras').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('ipi').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('ipi').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('ipi_valor').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('credito_icms').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('credito_icms').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('credito_icms').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('pis').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('pis').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('pis').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('cofins').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('cofins').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('cofins').asfloat;

    frmcompra_menu.QRNOTA.FIELDBYNAME('total_produtos').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('total_produtos').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('subtotal').asfloat;
    ...
    begin
      ...
      begin
        ...
        frmcompra_menu.QRNOTA.FIELDBYNAME('desconto').asfloat := frmcompra_menu.QRNOTA.FIELDBYNAME('desconto').asfloat + frmcompra.qrnota_item_lista.FIELDBYNAME('desconto').asfloat;

        frmcompra_menu.QRNOTA.FIELDBYNAME('total_nota').asfloat := (frmcompra_menu.QRNOTA.FIELDBYNAME('total_produtos').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('frete').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('seguro').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('outras').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('ipi').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('item_especial_total').asfloat + frmcompra_menu.QRNOTA.FIELDBYNAME('valor_sub').asfloat) - frmcompra_menu.QRNOTA.FIELDBYNAME('desconto').asfloat;
...
begin
  ...
  begin
    ...
    begin
      frmmodulo.QRPRODUTO_MOV.close;
      frmmodulo.QRPRODUTO_MOV.sql.clear;
      frmmodulo.QRPRODUTO_MOV.sql.add('DELETE FROM C000032 WHERE atb like :atb and CODIGO = ''' + qrnota_item_lista.FIELDBYNAME('CODLANCAMENTO').ASSTRING + '''');
      ...
      qrgrade_entrada.sql.add('select * from c000033 where atb like :atb and coditem = ''' + qrnota_item_lista.FIELDBYNAME('CODLANCAMENTO').ASSTRING + '''');
      ...
      frmmodulo.qrgrade_produto.sql.add('select * from c000021 where atb like :atb and codproduto = ''' + qrnota_item_lista.FIELDBYNAME('CODPRODUTO').ASSTRING + '''');
    ...
    qrnota_item.close;
    qrnota_item.sql.clear;
    qrnota_item.sql.add('delete from c000088 where atb like :atb and codigo = ''' + qrnota_item_lista.FIELDBYNAME('codigo').ASSTRING + ''' and codnota = ''' + qrnota_item_lista.FIELDBYNAME('codnota').ASSTRING + '''');
    qrnota_item.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
    qrnota_item.ExecSQL;
    frmmodulo.Conexao.Commit;
    Application.ProcessMessages;
    ...
    qrnota_item.close;
    qrnota_item.sql.clear;
    qrnota_item.sql.add('select * from c000088 where atb like :atb and codnota = ''' + qrnota_item_lista.FIELDBYNAME('codnota').ASSTRING + '''');
    qrnota_item.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
    qrnota_item.open;
...
begin
  ...
  begin
    ...
    begin
      frmcompra_menu.qrfornecedor.close;
      frmcompra_menu.qrfornecedor.sql.clear;
      frmcompra_menu.qrfornecedor.sql.add('select * from c000009 where atb like :atb and codigo = ''' + ecodremetente.Text + '''');
      frmcompra_menu.qrfornecedor.ParamByName('atb').Value := ME_FiltraATB('TB_FORNECEDOR');
      frmcompra_menu.qrfornecedor.open;
      if frmcompra_menu.qrfornecedor.RecordCount = 0 then
      begin
...
begin
  ...
  frmcompra_menu.QRNOTA.FIELDBYNAME('nota_fiscal').ASSTRING := frmprincipal.zerarcodigo(frmcompra_menu.QRNOTA.FIELDBYNAME('nota_fiscal').ASSTRING, 6);

  qrnf.close;
  qrnf.sql.clear;
  qrnf.sql.add('select * from c000087 where atb like :atb and numero = ''' + frmcompra_menu.QRNOTA.FIELDBYNAME('nota_fiscal').ASSTRING + '''');
  qrnf.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA');
  qrnf.open;
  ...
  begin
    ...
    begin
      if FIELDBYNAME('ITEM_FRETE_BASE').asfloat = 0 then
        FIELDBYNAME('ITEM_FRETE_BASE').asfloat := FIELDBYNAME('ITEM_FRETE_VALOR').asfloat;
      if FIELDBYNAME('ITEM_FRETE_BASE').asfloat > 0 then
      begin
    ...
    begin
      if FIELDBYNAME('ITEM_SEGURO_BASE').asfloat = 0 then
        FIELDBYNAME('ITEM_SEGURO_BASE').asfloat := FIELDBYNAME('ITEM_SEGURO_VALOR').asfloat;
      if FIELDBYNAME('ITEM_SEGURO_BASE').asfloat > 0 then
      begin
    ...
    begin
      if FIELDBYNAME('ITEM_PIS_BASE').asfloat = 0 then
        FIELDBYNAME('ITEM_PIS_BASE').asfloat := FIELDBYNAME('ITEM_PIS_VALOR').asfloat;
      if FIELDBYNAME('ITEM_PIS_BASE').asfloat > 0 then
      begin
    ...
    begin
      if FIELDBYNAME('ITEM_COMPLEMENTO_BASE').asfloat = 0 then
        FIELDBYNAME('ITEM_COMPLEMENTO_BASE').asfloat := FIELDBYNAME('ITEM_COMPLEMENTO_VALOR').asfloat;
      if FIELDBYNAME('ITEM_COMPLEMENTO_BASE').asfloat > 0 then
      begin
    ...
    begin
      if FIELDBYNAME('ITEM_SERVICO_BASE').asfloat = 0 then
        FIELDBYNAME('ITEM_SERVICO_BASE').asfloat := FIELDBYNAME('ITEM_SERVICO_VALOR').asfloat;
      if FIELDBYNAME('ITEM_SERVICO_BASE').asfloat > 0 then
      begin
    ...
    begin
      if FIELDBYNAME('ITEM_OUTRAS_BASE').asfloat = 0 then
        FIELDBYNAME('ITEM_OUTRAS_BASE').asfloat := FIELDBYNAME('ITEM_OUTRAS_VALOR').asfloat;
      if FIELDBYNAME('ITEM_OUTRAS_BASE').asfloat > 0 then
      begin
    ...
    FIELDBYNAME('ITEM_ESPECIAL_VALOR').asfloat := FIELDBYNAME('ITEM_FRETE_VALOR').asfloat + FIELDBYNAME('ITEM_SEGURO_VALOR').asfloat + FIELDBYNAME('ITEM_PIS_VALOR').asfloat + FIELDBYNAME('ITEM_COMPLEMENTO_VALOR').asfloat + FIELDBYNAME('ITEM_SERVICO_VALOR').asfloat + FIELDBYNAME('ITEM_OUTRAS_VALOR').asfloat;

    FIELDBYNAME('ITEM_ESPECIAL_TOTAL').asfloat := FIELDBYNAME('ITEM_FRETE_ICMS').asfloat + FIELDBYNAME('ITEM_SEGURO_ICMS').asfloat + FIELDBYNAME('ITEM_PIS_ICMS').asfloat + FIELDBYNAME('ITEM_COMPLEMENTO_ICMS').asfloat + FIELDBYNAME('ITEM_SERVICO_ICMS').asfloat + FIELDBYNAME('ITEM_OUTRAS_ICMS').asfloat;
...
begin
  edtotal.Value := (edprodutos.Value + edvst.Value + edfrete.Value + edseguro.Value + edoutras.Value + edipi.Value + edespecial.Value) - rdesconto.Value;
end;
...
begin
  ...
  qrnota_item.close;
  qrnota_item.sql.clear;
  qrnota_item.sql.add('select * from c000088 where atb like :atb and codigo = ''' + qrnota_item_lista.FIELDBYNAME('codigo').ASSTRING + ''' and codnota = ''' + qrnota_item_lista.FIELDBYNAME('codnota').ASSTRING + '''');
  qrnota_item.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
  qrnota_item.open;
...
begin
  if (UpperCase(Trim(frmcompra.Edit1.Text)) = UpperCase(Trim(frmmodulo.qrempresaUF.AsString))) and (copy(ecfop.Text, 1, 1) = '2') then
  begin
    showmessage('CFOP Invalido para essa Operação com Empresa de Dentro do Estado!');
    ecfop.setfocus;
    exit;
  end
  else if (UpperCase(Trim(frmcompra.Edit1.Text)) <> UpperCase(Trim(frmmodulo.qrempresaUF.AsString))) and (copy(ecfop.Text, 1, 1) = '1') then
  begin
    showmessage('CFOP Invalido para essa Operação com Empresa de Dentro do Estado!');
    ecfop.setfocus;
    exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Application.messagebox('Modelo não cadastrado!', 'Atenção', mb_ok + MB_ICONWARNING);
...
begin
  ...
  begin
    qrnf.close;
    qrnf.sql.clear;
    qrnf.sql.add('select * from c000087 where atb like :atb and numero = ''' + enumero.Text + '''');
...
begin
  ...
  begin
    qrpagar.close;
    qrpagar.sql.clear;
    qrpagar.sql.add('DELETE FROM C000046 WHERE atb like :atb and CODNOTA = ''' + frmcompra_menu.QRNOTA.FIELDBYNAME('NUMERO').ASSTRING + '''');
    qrpagar.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
    qrpagar.ExecSQL;
  ...
  begin
    ...
    qrpagar.FIELDBYNAME('codnota').ASSTRING := frmcompra_menu.QRNOTA.FIELDBYNAME('NUMERO').ASSTRING;
    ...
    qrpagar.FIELDBYNAME('ATB').ASSTRING := ME_GravaATB('TB_CONTAS_PAGAR');
    qrpagar.Post;
  end;
  frmmodulo.Conexao.Commit;
  Application.ProcessMessages;
...
begin
  if dsnota.DataSet.State in [dsinsert, dsedit] then
    dsnota.DataSet.FIELDBYNAME('obs7').ASSTRING := frmprincipal.zerarcodigo(dsnota.DataSet.FIELDBYNAME('obs7').ASSTRING, 6);
end;
