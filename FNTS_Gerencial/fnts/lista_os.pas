...
uses
  ...
  Menus, AdvGlowButton, AdvMetroButton,
  AdvSmoothExpanderPanel, JvExMask, JvToolEdit, ZAbstractDataset, AdvSmoothPanel;
...
uses
  modulo, loc_setor, xloc_cliente, xloc_funcionario,
  principal, xloc_cliente_veiculo;
...
procedure Tfrmlista_os.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  begin
    ...
    if resultado_pesquisa1 <> '' then
    begin
      combo_VEICULO.Items.Add(resultado_pesquisa3);
...
var
  cliente, tecnico, veiculo, tipo, atendimento, situacao, setor, periodo, ordem: string;
begin
  ...
  if combo_tecnico.Text = 'TODOS' then
    tecnico := ''
  else
  begin
    tecnico := ' and DETECTADO_CODTECNICO = ''' + copy(combo_tecnico.text, 1, 6) + '''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA4').asstring := 'TÉCNICO: ' + COMBO_TECNICO.TEXT;
  end;
  if combo_veiculo.Text = 'TODOS' then
    veiculo := ''
  else
  begin
    veiculo := ' and os.serial = ''' + combo_veiculo.text + '''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring := 'VEÍCULO ' + COMBO_VEICULO.TEXT + '   ';
  end;
  if combo_tipo.Text = 'TODAS' then
    tipo := ''
  else
  begin
    tipo := 'and os.tipo like ''%' + combo_tipo.text + '%''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring := frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring + 'TIPO: ' + COMBO_TIPO.TEXT;
  end;
  if combo_atendimento.Text = 'TODOS' then
    atendimento := ''
  else
  begin
    atendimento := ' and atendimento like ''%' + combo_atendimento.text + '%''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring := frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring + 'Atendimento: ' + COMBO_atendimento.TEXT;
  end;
  ...
  begin
    ...
    begin
      if COMBO_SITUACAO.ITEMINDEX = 5 then
        situacao := ' and os.situacao = ''FECHADA'' AND TOTAL = 0';
  ...
  if combo_setor.Text = 'TODOS' then
    setor := ''
  else
  begin
    setor := ' and codsetor = ''' + copy(combo_setor.text, 1, 6) + '''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA7').asstring := 'SETOR: ' + COMBO_SETOR.TEXT;
  end;
  ...
  begin
    ...
    qros.sql.add('where os.atb like :atbos and cli.atb like :atbcli and fun.atb like :atbfun and ' + periodo + cliente + tecnico + veiculo + tipo + situacao + setor + atendimento + ' order by ' + ordem);
    QRos.Params.ParamByName('datai').asdatetime := dateedit1.date;
    QRos.Params.ParamByName('dataf').asdatetime := dateedit2.date;
    qros.ParamByName('atbos').Value := ME_FiltraATB('TB_ORDEM_SERVICO');
    qros.ParamByName('atbcli').Value := ME_FiltraATB('TB_CLIENTE');
    qros.ParamByName('atbfun').Value := ME_FiltraATB('TB_FUNCIONARIO');
    qros.open;
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELATÓRIO DE ORDENS DE SERVIÇO - SINTÉTICO';
    fXOS.LoadFromFile('\SOS\server\rel\f000051.fr3');
    fXOS.ShowReport;
  end;
  ...
  begin
    ...
    qros.sql.add('left join c000007 cli on cli.codigo = os.codcliente');
    qros.sql.add('left join c000008 fun on fun.codigo = os.detectado_codtecnico');
    qros.sql.add('where os.atb like :atbos and cli.atb like :atbcli and fun.atb like :atbfun and' + periodo + cliente + tecnico + veiculo + tipo + situacao + setor + atendimento + ' order by DETECTADO_CODTECNICO, ' + ordem);
    QRos.Params.ParamByName('datai').asdatetime := dateedit1.date;
    QRos.Params.ParamByName('dataf').asdatetime := dateedit2.date;
    ...
    fXOS.LoadFromFile('\SOS\server\rel\f000052.fr3');
    fXOS.ShowReport;
  end;
  ...
  begin
    ...
    begin
      tecnico := ' and CODTECNICO = ''' + copy(combo_tecnico.text, 1, 6) + '''';
      frmmodulo.qrrelatorio.fieldbyname('LINHA4').asstring := 'TÉCNICO: ' + COMBO_TECNICO.TEXT;
    end;
    frmmodulo.qrservicos_periodicos.close;
    frmmodulo.qrservicos_periodicos.sql.clear;
    frmmodulo.qrservicos_periodicos.sql.add('select * from servicos_periodicos where atb like :atb and STATUS IS NOT NULL ' + cliente + tecnico + ' order by codcliente');
  ...
  begin
    ...
    begin
      cliente := ' and os.codcliente = ''' + copy(combo_cliente.text, 1, 6) + '''';
      frmmodulo.qrrelatorio.fieldbyname('LINHA3').asstring := 'CLIENTE: ' + COMBO_CLIENTE.TEXT;
    end;
    if combo_tecnico.Text = 'TODOS' then
      tecnico := ''
    ...
    begin
      tecnico := ' and tec.codigo = ''' + copy(combo_tecnico.text, 1, 6) + '''';
      frmmodulo.qrrelatorio.fieldbyname('LINHA4').asstring := 'TÉCNICO: ' + COMBO_TECNICO.TEXT;
    end;
    if combo_veiculo.Text = 'TODOS' then
      veiculo := ''
    ...
    begin
      veiculo := ' and os.codveiculo = ''' + copy(combo_veiculo.text, 1, 6) + '''';
      frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring := 'VEÍCULO ' + COMBO_VEICULO.TEXT + '   ';
    end;
    if combo_tipo.Text = 'TODAS' then
      tipo := ''
    ...
    begin
      atendimento := ' and os.atendimento like ''%' + combo_atendimento.text + '%''';
      frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring := frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring + 'Atendimento: ' + COMBO_atendimento.TEXT;
    end;
    if combo_SITUACAO.Text = 'TODAS' then
      situacao := ''
    ...
    qros_servico.sql.add('and serv.atb like :atbserv and os.atb like :atbos');
    qros_servico.sql.add('and nserv.atb like :atbnserv and cli.atb like :atbcli and TEC.atb like :atbTEC');
    ...
    qros_servico.ParamByName('atbserv').Value := ME_FiltraATB('TB_OS_SERVICOS');
    qros_servico.ParamByName('atbos').Value := ME_FiltraATB('TB_ORDEM_SERVICO');
    qros_servico.ParamByName('atbnserv').Value := ME_FiltraATB('TB_SERVICOS');
    qros_servico.ParamByName('atbcli').Value := ME_FiltraATB('TB_CLIENTE');
    qros_servico.ParamByName('atbTEC').Value := ME_FiltraATB('TB_FUNCIONARIO');

    qros_servico.open;

    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELATÓRIO DE SERVIÇOS - AGRUPADA POR TÉCNICO';
    fXOS.LoadFromFile('\SOS\server\rel\f000051_2.fr3');
    fXOS.ShowReport;
