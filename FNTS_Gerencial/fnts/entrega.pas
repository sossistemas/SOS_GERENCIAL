...
uses
  ...
  Dialogs, ExtCtrls, Grids, Wwdbigrd, Wwdbgrid, StdCtrls, Mask, RzEdit, RzBtnEdt,
  AdvGlowButton, DB, ZAbstractRODataset, ZAbstractDataset, ZDataset, Menus,
  frxClass, frxDBSet, frxDesgn, UCBase, AdvMetroButton, AdvSmoothPanel,
...
uses
  modulo, xloc_cliente, principal, entrega_ficha;
...
begin
  ...
  begin
    ...
    qrcliente2.sql.add('select * from c000007 where atb like :atb and codigo = ''' + resultado_pesquisa1 + '''');
  ...
  begin
    ...
    begin
      ...
      begin
        qrcliente2.close;
        qrcliente2.sql.Clear;
        qrcliente2.sql.add('select * from c000007 where atb like :atb and upper(nome) like ''' + ansiuppercase(ecliente.text) + '%''');
        qrcliente2.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
        qrcliente2.Open;
        if qrcliente2.RecordCount > 0 then
        begin
...
begin
  ...
  qrprodutos.sql.add('pro.atb like :atbpro and ');
  qrprodutos.sql.add('ent.atb like :atbent and ');
  ...
  qrprodutos.ParamByName('atbpro').Value := ME_FiltraATB('TB_PRODUTO');
  qrprodutos.ParamByName('atbent').Value := ME_FiltraATB('TB_ENTREGA');
...
begin
  ...
  qrentrega.close;
  qrentrega.sql.clear;
  qrentrega.sql.add('select * from c000115 where atb like :atb and codproduto  = ''' + qrprodutos.fieldbyname('codproduto').asstring + '''');
  ...
  qrentrega.ParamByName('atb').Value := ME_FiltraATB('TB_ENTREGA');
  qrentrega.open;
...
begin
  ...
  while frmentrega_ficha.qrentrega.RecordCount > 0 do
    frmentrega_ficha.qrEntrega.delete;
  ...
  begin
    ...
    begin
      ...
      frmentrega_ficha.qrEntrega.FieldByName('ATB').asstring := ME_GRAVAATB('TB_ENTREGA');
...
begin
  ...
  rel_entrega.Report.LoadFromFile('\SOS\server\rel\F000169.fr3');
...
begin
  ...
  query.sql.add('where');
  query.sql.add('pro.atb like :atbpro and ');
  query.sql.add('ent.atb like :atbent and ');
  ...
  query.ParamByName('atbpro').Value := ME_FiltraATB('TB_PRODUTO');
  query.ParamByName('atbent').Value := ME_FiltraATB('TB_ENTREGA');
  query.ParamByName('atbcli').Value := ME_FiltraATB('TB_CLIENTE');
  query.open;

  rel_entrega.Report.LoadFromFile('\SOS\server\rel\F000171.fr3');
  rel_entrega.ShowReport;
