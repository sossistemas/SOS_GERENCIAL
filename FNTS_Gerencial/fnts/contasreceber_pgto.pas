...
uses
  ...
  Dialogs, Menus, StdCtrls, Buttons, ExtCtrls, Mask, TFlatPanelUnit,
  AdvGlowButton, AdvShapeButton, ComCtrls, wwdbdatetimepicker, AdvMetroButton,
...
uses
  contasreceber, modulo, principal, troco, contasreceber_banco, Math,
  contasreceber_recibo;
...
begin
  try
    try
      ...
      FRMMODULO.QRCAIXA_MOV.SQL.ADD('select * from c000044 where atb like :atb and codigo = ''999999''');
      ...
      qrBaixa.CLOSE;
      qrBaixa.SQL.CLEAR;
      qrBaixa.SQL.ADD('select 0 restante,  cta.* from c000049 cta where cta.atb like :atb and cta.codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + ''' and cta.situacao = 1 and cta.codigo in (' + numero_contas + ') order by cta.data_vencimento, cta.codigo');
      qrBaixa.ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_RECEBER');
      qrBaixa.open;
      qrBaixa.First;
      while not qrBaixa.Eof do
      begin
        if frmcontasreceber.qrcr.Locate('codigo', qrBaixaCODIGO.AsString, []) then
        begin
          qrBaixa.Edit;
          qrBaixaRESTANTE.AsFloat := frmcontasreceber.QRCrRESTANTE.AsFloat;
          qrBaixaVALOR_JUROS.AsFloat := frmcontasreceber.QRCrJUROS.AsFloat;
          qrBaixaVALOR_DESCONTO.AsFloat := frmcontasreceber.QRCrVALOR_DESCONTO.AsFloat;
          qrBaixaVALOR_ATUAL.AsFloat := frmcontasreceber.QRCrRESTANTE.AsFloat;
          qrBaixa.Post;
        end;
        qrBaixa.Next;
      end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            qrBaixa.FieldByName('valor_atual').ASfloat := qrBaixa.FieldByName('valor_atual').ASfloat - rdesconto2.VALUE - rsoma.VALUE;
          ...
          begin
            ...
            qrBaixa.FieldByName('valor_atual').ASfloat := (qrBaixa.FieldByName('valor_atual').ASfloat) - (rdesconto2.VALUE + rsoma.VALUE + rdiferenca.VALUE);
        ...
        begin
          qrBaixa.FieldByName('VALOR_PAGO').ASfloat := qrBaixa.FieldByName('VALOR_PAGO').ASfloat + rsoma.VALUE;
          qrBaixa.FieldByName('valor_atual').ASfloat := STRTOFLOAT(formatfloat('########0.00', (qrBaixa.FieldByName('valor_atual').ASfloat - rdesconto2.VALUE - rsoma.VALUE)));
        end;
        if (qrBaixa.FieldByName('valor_atual').ASfloat < 0.02) and (qrBaixa.FieldByName('valor_atual').ASfloat > (-0.02)) then
        begin
        ...
        begin
          ...
          SQL.ADD('(codconta,data,valor_parcela,valor_juros,valor_desconto,valor_pago,dinheiro,chequeav,chequeap,cartao,boleto,troco,credito,codcliente,caixa_banco, atb)');
          SQL.ADD('values');
          SQL.ADD('(:codconta,:data,:valor_parcela,:valor_juros,:valor_desconto,:valor_pago,:dinheiro,:chequeav,:chequeap,:cartao,:boleto,:troco,:credito,:codcliente,:caixa_banco, :atb)');
          ...
          ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_RECEBER_PGTO');
          execsql;
        end;
        if rcaixa.Checked then
        begin
      ...
      begin
        ...
        frmcontasreceber.qrcontasreceber_pgto.SQL.ADD('select * from c000050 where atb like :atb and 1=2');
        frmcontasreceber.qrcontasreceber_pgto.ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_RECEBER_PGTO');
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if vvalor_pago1 > 0 then
              begin
                if (vvalor_pago1 >= qrBaixa.FieldByName('valor_atual').ASfloat) then
                begin
              ...
              qrcontasreceber_pgto.FieldByName('atb').AsString := ME_GravaATB('TB_CONTA_RECEBER_PGTO');
            ...
            begin
              if (vvalor_pago >= qrBaixa.FieldByName('valor_atual').ASfloat) then
              begin
              ...
              begin
                ...
                vvalor_pago := 0;
                if qrBaixa.FieldByName('valor_pago').ASfloat < 0 then
                begin
                  qrBaixa.FieldByName('valor_atual').ASfloat := 0;
                  qrBaixa.FieldByName('situacao').ASINTEGER := 2;
            ...
            if qrBaixa.FieldByName('valor_atual').ASfloat < 0 then
            begin
              qrBaixa.FieldByName('valor_atual').ASfloat := 0;
              qrBaixa.FieldByName('situacao').ASINTEGER := 2;
            end;
...
procedure Tfrmcontasreceber_pgto.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
procedure Tfrmcontasreceber_pgto.rdesconto1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
...
begin
  ...
  rsoma.VALUE := rdinheiro.VALUE + rchequeav.VALUE + rchequeap.VALUE + rcartao.VALUE + rboleto.VALUE;
...
procedure Tfrmcontasreceber_pgto.rboletoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bgravar.setfocus;
end;
