...
uses
  ...
  ZDataset, DBCtrls, Buttons, ImgList, AdvToolBar,

  AdvGlowButton, JvExMask, JvToolEdit, ZAbstractDataset;
...
procedure Tfrmxloc_produto_entrada.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure Tfrmxloc_produto_entrada.wwDBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tfrmxloc_produto_entrada.edata_inicioKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
...
begin
  ...
  query.close;
  query.sql.clear;
  query.SQL.add('select * from c000087 where atb like :atb and data_lancamento BETWEEN :datai and :dataf ' + nota + ' order by data_lancamento');
