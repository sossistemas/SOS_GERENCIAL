...
uses
  ...
  frxDBSet, frxDCtrl, ImgList, ZAbstractRODataset,
  ZAbstractDataset, ZDataset, UCBase, AdvMetroButton, AdvSmoothPanel,
  AdvSmoothExpanderPanel, System.ImageList, JvExMask, JvToolEdit, cxGraphics,
  cxControls, cxLookAndFeels, cxLookAndFeelPainters, cxStyles, cxCustomData,
  cxFilter, cxData, cxDataStorage, cxEdit, cxNavigator, cxDBData, cxContainer,
  cxDBLabel, cxLabel, cxGridLevel, cxGridCustomTableView, cxGridTableView,
...
type
  TfrmCartao = class(TForm)
    ...
    qrcartaoCODIGO_AUTORIZACAO: TWideStringField;
    qrcartaoCNPJ: TWideStringField;
    cxGrid1: TcxGrid;
    Grid: TcxGridDBBandedTableView;
    GridCOD_VENDA: TcxGridDBBandedColumn;
    GridDATA_VENDA: TcxGridDBBandedColumn;
    GridVALOR: TcxGridDBBandedColumn;
    GridCOMISSAO: TcxGridDBBandedColumn;
    GridLIQUIDO: TcxGridDBBandedColumn;
    GridDATA_RECEBIMENTO: TcxGridDBBandedColumn;
    GridDATA_BAIXA: TcxGridDBBandedColumn;
    GridSITUACAO: TcxGridDBBandedColumn;
    GridTIPO: TcxGridDBBandedColumn;
    GridPARCELA: TcxGridDBBandedColumn;
    GridVALOR_TOTAL: TcxGridDBBandedColumn;
    GridBANDEIRA: TcxGridDBBandedColumn;
...
begin
  ...
  if fi_codigo <> '' then begin
    ...
    query.sql.add('liquido, data_recebimento, situacao, tipo, parcela, valor_total, ATB)');
    ...
    query.ParamByName('atb').Value := ME_GravaATB('TB_MOVIMENTO_CARTAO');
    query.ExecSQL;
...
begin
  ...
  if qrcartao.fieldbyname('situacao').ASSTRING = 'ABERTO' then begin
    ...
    if fi_codigo <> '' then begin
      ...
      query.sql.add('where atb like :atb and codigo = ''' + qrcartao.fieldbyname('codigo').ASSTRING + '''');
...
begin
  if Key = #13 then begin
    if ed_cod_cartao.Text <> '' then begin
      query.close;
      query.sql.clear;
      query.sql.add('select * from c000013 where atb like :atb and CARTAO_CREDITO = 1 and');
...
begin
  ...
  qrcartaoCOMISSAO.DisplayFormat := mascara_valor;
  Grid.DataController.Summary.FooterSummaryItems[1].Format := mascara_valor;
  Grid.DataController.Summary.FooterSummaryItems[2].Format := mascara_valor;
  Grid.DataController.Summary.FooterSummaryItems[3].Format := mascara_valor;
  Grid.DataController.Summary.FooterSummaryItems[4].Format := mascara_valor;
end;
...
begin
  ...
  qrcartao.sql.add('and c000124.ATB LIKE ' + QuotedStr(ME_FiltraATB('TB_MOVIMENTO_CARTAO')));
  qrcartao.sql.add('and c000007.ATB LIKE ' + QuotedStr(ME_FiltraATB('TB_CLIENTE')));
  qrcartao.sql.add('and c000013.ATB LIKE ' + QuotedStr(ME_FiltraATB('TB_BANCO')));
