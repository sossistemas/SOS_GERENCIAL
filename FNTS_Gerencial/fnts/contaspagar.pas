...
uses
  ...
  Wwdbgrid, ExtCtrls, StdCtrls, TFlatPanelUnit, Mask, Buttons,
  wwdblook, wwDialog, wwidlg, Menus, DBCtrls, ImgList, Wwkeycb, AdvGlowButton,
  UCBase, AdvMetroButton, AdvSmoothPanel, AdvSmoothExpanderPanel, System.ImageList,
...
type
  Tfrmcontaspagar = class(TForm)
    ...
    qrcontaspagarIDONLINE: TIntegerField;
    qrcontaspagarATB: TWideStringField;
    ...
    procedure qrcontaspagarNewRecord(DataSet: TDataSet);
  private
...
uses
  modulo, contaspagar_ficha, contaspagar_fichaalteracao, contaspagar_pgto,
  principal, lista_pagar2, contaspagar_data, contaspagar_nf;
...
begin
  ...
  qrcontaspagar.sql.Add('SELECT * FROM C000046 WHERE atb like :atb and SITUACAO = 1 ORDER BY DATA_VENCIMENTO');
...
procedure Tfrmcontaspagar.qrcontaspagarNewRecord(DataSet: TDataSet);
begin
  qrcontaspagarATB.AsString := ME_GravaATB('TB_CONTAS_PAGAR');
end;

procedure Tfrmcontaspagar.wwDBGrid1TitleButtonClick(Sender: TObject; AFieldName: string);
var
  situacao, fornecedor, periodo: string;
begin
  if (ansiuppercase(AFieldName) = ansiuppercase('data_vencimento')) or (ansiuppercase(AFieldName) = ansiuppercase('historico')) or (ansiuppercase(AFieldName) = ansiuppercase('notafiscal')) or (ansiuppercase(AFieldName) = ansiuppercase('documento')) or (ansiuppercase(AFieldName) = ansiuppercase('especie')) or (ansiuppercase(AFieldName) = ansiuppercase('valor')) then
  begin
    ...
    if COMBOFORNECEDOR.TEXT = '' then
      ...
      fornecedor := ' and codfornecedor = ''' + frmmodulo.qrfornecedor.FieldByName('codigo').asstring + '''';
    ...
    qrcontaspagar.close;
    qrcontaspagar.sql.clear;
    qrcontaspagar.sql.Add('select * from c000046 where atb like :atb and codigo is not null' + situacao + fornecedor + periodo + ' order by ' + AFieldName);
    if combo_situacao.itemindex = 1 then
  ...
  begin
    ...
    qrcontaspagar.close;
    qrcontaspagar.sql.clear;
    qrcontaspagar.sql.Add('select * from c000046 where atb like :atb and codigo is not null' + situacao + fornecedor + periodo + ' order by codfornecedor');
    if combo_situacao.itemindex = 1 then
      qrcontaspagar.Params.ParamByName('datax').asdatetime := DATE;
    qrcontaspagar.Params.ParamByName('datai').asdatetime := DateEdit1.DATE;
    qrcontaspagar.Params.ParamByName('dataf').asdatetime := DateEdit2.DATE;
...
begin
  ...
  qrcontaspagar.close;
  qrcontaspagar.sql.clear;
  qrcontaspagar.sql.Add('select * from c000046 where atb like :atb and codigo is not null' + situacao + fornecedor + periodo + ' order by data_vencimento');
  qrcontaspagar.Params.ParamByName('datai').asdatetime := DateEdit1.DATE;
  qrcontaspagar.Params.ParamByName('dataf').asdatetime := DateEdit2.DATE;
  qrcontaspagar.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
  qrcontaspagar.open;
...
begin
  ...
  begin
    qrnota.close;
    qrnota.sql.clear;
    qrnota.sql.Add('select * from c000087 where atb like :atb and codigo = ''' + qrcontaspagar.FieldByName('codnota').asstring + '''');
    ...
    begin
      qritem.close;
      qritem.sql.clear;
      qritem.sql.Add('select * from c000088 where atb like :atb and codnota = ''' + qrnota.FieldByName('codigo').asstring + '''');
      qritem.sql.Add('ordeR by item');
      qritem.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
      qritem.open;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              texto := PWideChar('Esta conta foi baixada pelo caixa ' + qrcontaspagar.FieldByName('codcaixa').asstring + '. Apenas ele tem autorização para extorná-la!');
      ...
      begin
        texto := PWideChar('O pagamento desta conta foi lançado no caixa do dia: ' + qrcontaspagar.FieldByName('data_pagamento').asstring + '. Não será possível fazer um lançamento de estorno no caixa atual! Deseja prosseguir com o estorno?');
      ...
      frmprincipal.logUC('Estornou Conta a Pagar Conta Nr: ' + qrcontaspagar.fieldbyname('documento').asstring + ' Fornecedor: ' + qrcontaspagar.fieldbyname('fornecedor').asstring, 3);
...
begin
  ...
  begin
    ...
    begin
      ...
      frmcontaspagar_pgto.qrcontaspagar.sql.Add('select * from c000046 where atb like :atb and situacao = 1 and filtro = 1 order by data_vencimento');
      frmcontaspagar_pgto.qrcontaspagar.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
...
begin
  ...
  begin
    if qrcontaspagar.FieldByName('situacao').ASINTEGER = 1 then
    begin
      texto := PWideChar('Confirma a Exclusão da Conta: ' + #13 + 'Fornecedor: ' + qrcontaspagar.FieldByName('fornecedor').asstring + '           ' + #13 + 'Vencimento: ' + qrcontaspagar.FieldByName('data_vencimento').asstring + #13 + 'Documento: ' + qrcontaspagar.FieldByName('documento').asstring + #13 + 'Valor - R$: ' + formatfloat('###,###,##0.00', qrcontaspagar.FieldByName('valor').asfloat));
      if application.messagebox(texto, 'Atenção', mb_yesno + MB_ICONWARNING) = idyes then
      begin
        frmprincipal.logUC('Excluiu Conta a Pagar Conta Nr: ' + frmcontaspagar.qrcontaspagar.FieldByName('documento').asstring + ' Fornecedor: ' + frmcontaspagar.qrcontaspagar.FieldByName('fornecedor').asstring, 3);
...
procedure Tfrmcontaspagar.COMBOFORNECEDORKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    combo_periodo.SETFOCUS;
end;

procedure Tfrmcontaspagar.combo_situacaoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    COMBOFORNECEDOR.SETFOCUS;
end;
...
begin
  ...
  qrcontaspagar.close;
  qrcontaspagar.sql.clear;
  qrcontaspagar.sql.Add('select * from c000046 where atb like :atb and filtro = ''1''');
  qrcontaspagar.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
  qrcontaspagar.open;
...
begin
  ...
  Qrsoma.sql.Add('select sum(VALOR) apagar, ' + // sum_0 A VENCER
    ...
    'from c000046 where atb like :atb and situacao = 1 ' + periodo + fornecedor + situacao);
  Qrsoma.Params.ParamByName('datai').asdatetime := DateEdit1.DATE;
  Qrsoma.Params.ParamByName('dataf').asdatetime := DateEdit2.DATE;
  Qrsoma.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
  Qrsoma.open;
