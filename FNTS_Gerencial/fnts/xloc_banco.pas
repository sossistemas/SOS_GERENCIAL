...
procedure Tfrmxloc_banco.Localizarpor1Click(Sender: TObject);
var
  X: INTEGER;
begin
...
procedure Tfrmxloc_banco.Referncia1Click(Sender: TObject);
var
  X: INTEGER;
begin
...
procedure Tfrmxloc_banco.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;

procedure Tfrmxloc_banco.AdvMetroButton1Click(Sender: TObject);
begin
  close;
end;

procedure Tfrmxloc_banco.Fechar1Click(Sender: TObject);
begin
  if voltar then
    loc.setfocus
  else
...
procedure Tfrmxloc_banco.locKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = vk_down then
  begin
    grid.setfocus;
...
procedure Tfrmxloc_banco.GRIDKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure Tfrmxloc_banco.locKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    if query.RecNo > 0 then
...
procedure Tfrmxloc_banco.locEnter(Sender: TObject);
begin
  tedit(Sender).color := $00A0FAF8;
  voltar := false;
end;

procedure Tfrmxloc_banco.locExit(Sender: TObject);
begin
  tedit(Sender).color := clwindow;
end;

procedure Tfrmxloc_banco.FormShow(Sender: TObject);
begin
  voltar := false;
  query.Close;
  query.SQL.clear;
...
procedure Tfrmxloc_banco.GRIDEnter(Sender: TObject);
begin
  voltar := true;
  lvoltar.Caption := 'VOLTAR';
end;

procedure Tfrmxloc_banco.GRIDExit(Sender: TObject);
begin
  lvoltar.Caption := 'FECHAR';
  VOLTAR := FALSE;
end;

procedure Tfrmxloc_banco.locChange(Sender: TObject);
var
  ref, tipo: string;
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000013 where atb like :atb and upper(' + tipo + ') like ''' + ref + loc.Text + '%'' order by ' + tipo);
    query.ParamByName('atb').Value := ME_FiltraATB('TB_BANCO');
    query.open;
  except
  end;
...
procedure Tfrmxloc_banco.GRIDDblClick(Sender: TObject);
begin
  resultado_pesquisa1 := query.fieldbyname('NUMERO').asstring;
  resultado_pesquisa2 := query.fieldbyname('BANCO').asstring;
  close;
...
procedure Tfrmxloc_banco.ParmetrosdaPesquisa1Click(Sender: TObject);
begin
  LOC.SETFOCUS;
end;
