...
uses
  ...
  Mask, maskutils, AdvGlowButton, wwdbedit, Wwdotdot, Wwdbcomb, UCBase,
...
uses
  modulo, principal, xloc_convenio;
...
begin
  ...
  begin
    ...
    qrloc.close;
    qrloc.sql.clear;
    qrloc.sql.add('select * from c000007 where atb like :atb and codconvenio = ''' + DBEdit1.Text + '''');
    qrloc.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
    qrloc.open;
...
begin
  ...
  frmmodulo.qrconvenio.sql.add('select * from c000071 where atb like :atb order by codigo');
