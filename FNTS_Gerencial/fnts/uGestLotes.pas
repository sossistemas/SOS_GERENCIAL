...
begin
  ...
  begin
    Application.MessageBox('Nenhum registro selecionado.','Atenção',MB_ICONERROR);
    Exit;
  end;
...
begin
  if (not (qryLote.Active) or not (qryLote.RecordCount > 0)) then
  begin
    Application.MessageBox('Nenhum registro para excluir.','Atenção',MB_ICONERROR);
    Exit;
  end;

  if Application.MessageBox('Deseja excluir o Lote selecionado?','Atenção', 36)=6 then
  begin
    if ExcluirLote(qryLote.FieldByName('ID').AsInteger) then
    begin
      Application.MessageBox('Lote Excluído com sucesso.','OK',MB_ICONINFORMATION);
      bfiltrar.Click;
    end;
  end;
