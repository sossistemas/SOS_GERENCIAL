...
uses
  ...
  Mask, maskutils, PageView, Wwdbigrd, Wwdbgrid, wwdbedit, wwdblook,
  AdvGlowButton, frxClass,
  frxDBSet, UCBase, frxExportPDF, ACBrUtil, ComCtrls,
  FileCtrl, inifiles, IdMessage, IdBaseComponent, IdTCPConnection,
  IdTCPClient, IdSMTP, IdAuthentication,
  IdAttachmentFile, AdvMetroButton, AdvSmoothPanel, AdvSmoothExpanderPanel,
  JvBaseEdits, JvExMask, JvToolEdit, JvDBControls;
...
type
  Tfrmproduto_pedido = class(TForm)
    ...
    DBMemo1: TDBMemo;
    qrpedido_itemATB: TWideStringField;
    qrpedido_produtoOBSERVACAO: TBlobField;
    qrpedido_produtoATB: TWideStringField;
    qrnfOBSERVACAO: TBlobField;
    qrnfATB: TWideStringField;
    ...
    procedure qrpedido_itemNewRecord(DataSet: TDataSet);
...
uses
  modulo, principal, loc_fornecedor, produto_pedido_item, Math, email;
...
begin
  ...
  frmmodulo.qrpedido_produto.sql.add('select * from c000037 where atb like :atb order by codigo');
...
begin
  ...
  begin
    ...
    qrgrade_pedido.sql.add('delete from c000039 where atb like :atb and codnota = ''' + frmmodulo.qrpedido_produto.fieldbyname('codigo').asstring + '''');
...
begin
  ...
  begin
    qrpedido_produto.sql.add('select * from c000037 where atb like :atb and upper(numero) = ''' + ansiuppercase(frmprincipal.zerarcodigo(LOC.Text, 6)) + ''' order by numero');
  end;
  if rcodigo.Checked then
  begin
    qrpedido_produto.sql.add('select * from c000037 where atb like :atb and data = :datai order by data');
    qrpedido_produto.Params.ParamByName('datai').value := strtodate(LOC.Text);
  end;
  if rapelido.Checked then
  begin
    qrpedido_produto.sql.add('select * from c000037 where atb like :atb and codfornecedor = ''' + copy(LOC.Text, 1, 6) + ''' order by numero');
  end;
...
var
  ...
  email: TEmail;
  sXML, edtSmtpHost, edtSmtpPort, edtSmtpUser, edtSmtpPass, edtEmailAssunto, edtcompras, caminho, mmEmailMsg: string;
...
begin
  ...
  qrapoio2.sql.add('where atb like :atb and CODFORNECEDOR = ''' + efornecedor.Text + '''');
  qrapoio2.ParamByName('atb').Value := ME_FiltraATB('TB_FORNECEDOR_CODIGO_PRODUTO');
  qrapoio2.open;
  ...
  PDF.DefaultPath := 'C:\SOS\Server\PDF\';
  PDF.ShowDialog := false;
  PDF.ShowProgress := false;
  PDF.OverwritePrompt := false;
  fxpedido.LoadFromFile('\SOS\server\rel\f000132_2.fr3');
  ...
  edtfinanceiro := frmprincipal.LeInistr(caminho, 'Email', 'PadraoFinanceiro', '');
  ...
  qrapoio.sql.add('where atb like :atb and codigo = ''' + efornecedor.Text + '''');
  qrapoio.ParamByName('atb').Value := ME_FiltraATB('TB_FORNECEDOR');
  qrapoio.open;
  ...
  TIdAttachmentFile.create(EmailCorpo.MessageParts, Tfilename('C:\SOS\Server\PDF\' + DBEdit2.Text + '-' + DBEdit23.Text + '.pdf'));
...
procedure Tfrmproduto_pedido.RxDBCalcEdit2KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bgravar.setfocus;
end;
...
begin
  ...
  begin
    qrgrade_pedido.close;
    qrgrade_pedido.sql.clear;
    qrgrade_pedido.sql.add('delete from c000039 where atb like :atb and coditem = ''' + dspedido_item.DataSet.fieldbyname('codigo').asstring + '''');
    qrgrade_pedido.ParamByName('atb').Value := ME_FiltraATB('TB_PEDIDO_COMPRA_ITENS_GRADE');
    qrgrade_pedido.ExecSQL;
    frmmodulo.qrpedido_produto.fieldbyname('total_nota').asfloat := frmmodulo.qrpedido_produto.fieldbyname('total_nota').asfloat - dspedido_item.DataSet.fieldbyname('TOTAL').asfloat;
...
begin
  ...
  begin
    qrnf.close;
    qrnf.sql.clear;
    qrnf.sql.add('select * from c000037 where atb like :atb and codfornecedor = ''' + efornecedor.Text + ''' and numero = ''' + DBEdit2.Text + '''');
    qrnf.ParamByName('atb').Value := ME_FiltraATB('TB_PEDIDO_COMPRA');
    qrnf.open;
...
begin
  try
    ...
    qrpedido_item.sql.add('select * from c000038 where atb like :atb and codnota = ''' + frmmodulo.qrpedido_produto.fieldbyname('codigo').asstring + ''' order by codigo');
    qrpedido_item.ParamByName('atb').Value := ME_FiltraATB('TB_PEDIDO_COMPRA_ITENS');
    qrpedido_item.open;
...
begin
  try
    qrpedido_item.fieldbyname('FALTA').asfloat := qrpedido_item.fieldbyname('QTDE').asfloat - qrpedido_item.fieldbyname('RECEBIDO').asfloat;
  except
  end;
end;

procedure Tfrmproduto_pedido.qrpedido_itemNewRecord(DataSet: TDataSet);
begin
  qrpedido_itemATB.AsString := ME_GravaATB('TB_PEDIDO_COMPRA_ITENS');
end;

procedure Tfrmproduto_pedido.qrpedido_produtoNewRecord(DataSet: TDataSet);
begin
...
begin
  ...
  qrpedido_fornecedor.sql.add('select * from c000009 where atb like :atb and codigo = ''' + frmmodulo.qrpedido_produto.fieldbyname('codfornecedor').asstring + '''');
  qrpedido_fornecedor.ParamByName('atb').Value := ME_FiltraATB('TB_FORNECEDOR');
  qrpedido_fornecedor.open;
  fxpedido.LoadFromFile('\SOS\server\rel\f000132.fr3');
  fxpedido.ShowReport;
end;
