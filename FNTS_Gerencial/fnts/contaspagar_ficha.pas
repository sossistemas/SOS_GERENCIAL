...
uses
  ...
  Dialogs, StdCtrls, Mask, Buttons, TFlatPanelUnit, Grids, Wwdbigrd, Wwdbgrid,
  ExtCtrls, DB, ZAbstractRODataset, ZDataset,
  Menus, wwdbedit, Wwdotdot, Wwdbcomb, AdvGlowButton, AdvMetroButton,
  AdvSmoothExpanderPanel, JvBaseEdits, JvExMask, JvToolEdit, ZAbstractDataset,
  AdvSmoothPanel;
...
uses
  modulo, loc_fornecedor, loc_conta, principal, contaspagar;
...
procedure Tfrmcontaspagar_ficha.DateEdit1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
...
begin
  ...
  try
    ...
    begin
      ...
      qrgrade.fieldbyname('valor').asfloat := frmprincipal.Arredondar(qrgrade.fieldbyname('valor').asfloat, 2);
    ...
    begin
      qrgrade.Last;
      qrgrade.Edit;
      qrgrade.fieldbyname('valor').asfloat := qrgrade.fieldbyname('valor').asfloat + diferenca;
      qrgrade.post;
    end;
    qrgrade.Refresh;
    RTOTAL.value := RVALOR.value;
...
procedure Tfrmcontaspagar_ficha.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;
...
begin
  ...
  query.close;
  query.sql.clear;
  query.sql.add('select * from c000046 where atb like :atb and notafiscal = ''' + Edit5.Text + ''' and codfornecedor = ''' + efornecedor.Text + ''' and data_vencimento = :vdat');
  query.Params.ParamByName('vdat').asdatetime := qrgradeVENCIMENTO.asdatetime;
  query.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
  query.open;
  if query.recordcount > 0 then
  begin
    if application.messagebox(pchar('Já existe uma NOTA FISCAL Cadastrada com este Número' + #13 + 'para este FORNECEDOR e com esse VENCIMENTO!' + #13 + 'Deseja Continuar Com a Operação?'), 'Aviso', mb_yesno + MB_ICONWARNING) = idno then
      exit;
  end;
  ...
  begin
    ...
    begin
      ...
      //
      FRMCONTASPAGAR.qrcontaspagar.fieldbyname('ATB').asstring := ME_GravaATB('TB_CONTAS_PAGAR');
      FRMCONTASPAGAR.qrcontaspagar.post;
      qrgrade.Next;
    end;
    frmmodulo.Conexao.Commit;
    frmprincipal.logUC('Inclui Conta a Pagar Conta Nr: ' + FRMCONTASPAGAR.qrcontaspagar.fieldbyname('documento').asstring + ' Fornecedor: ' + efornecedor.Text, 3);
