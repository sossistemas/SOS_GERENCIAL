    ...
    if DBCheckBox67.Checked then begin
      ...
      while not qrcaixa_operador.Eof do begin
        if qrcaixa_operadorFECHAMENTO_CEGO.AsString = 'S' then begin
          Application.MessageBox('Inpossivel ativar "Fechamento e Abertura Automática dos Caixas do Gerencial" sendo que esteja Ativo Fechamento Cego no PDV no cadastro de Operadores de Caixas!','Atenção!',MB_ICONINFORMATION);
          Exit;
        end;
