...
begin
  ...
  if COMBO_ORDEM.Text = 'CÓDIGO DE BARRAS' then
	...
	Else
	if combo_relatorio.Text = 'INVENTÁRIO DE ESTOQUE ATUAL' then    //qiqi
	begin
		qrproduto.CLOSE;
		qrproduto.SQL.CLEAR;
		qrproduto.SQL.ADD('select prod.*, estq.*'+sLineBreak+
...
begin
  ...
  begin
	...
	if (combo_relatorio.Text = 'POSIÇÃO DE ESTOQUE') or (combo_relatorio.Text = 'RELAÇÃO COM PREÇOS (VENDA/SUGERIDO/LUCRO 0)') or (combo_relatorio.Text = 'SUGESTÃO DE COMPRAS') then
  begin
    grem.Visible := TRUE;
    grem.Enabled := TRUE;
  end;
