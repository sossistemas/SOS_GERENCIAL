...
uses
  ...
  Dialogs, ExtCtrls, TFlatPanelUnit, StdCtrls,
  Grids, Wwdbigrd, Wwdbgrid, DB,
  ...
  JvExMask, JvToolEdit, JvBaseEdits, obsVenda;
...
type
  Tfrmvenda = class(TForm)
    ...
    O2: TMenuItem;
    qrvenda_produtoCODLANCAMENTO: TWideStringField;
...
uses
  modulo, venda_fechamento, venda_troca,
  venda_fechamento_especial, venda_item_2, principal, venda_devolucao,
  orcamento_localizar, ItensOrcamento,
  xloc_cliente;
...
begin
  ...
  begin
    ...
    qrcontasreceber.sql.add('select codcliente,situacao,data_vencimento,SUM(VALOR_ATUAL) TOTAL_CLIENTE from c000049 where atb like :atb and codcliente = ''' + resultado_pesquisa1 + ''' and situacao = 1');
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            qrcliente.close;
            qrcliente.sql.clear;
            qrcliente.sql.add('select * from c000007 where atb like :atb and codigo = ''' + cliAntigo + '''');
            qrcliente.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
            qrcliente.open;
            exit;
          end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        frmmodulo.qrserial_produto.sql.Add('update c000022 set situacao = 1, cliente = null, datavenda = null, precovenda = 0 where atb like :atb and codproduto = ''' + qrvenda_produto.FieldByName('codproduto').asstring + ''' and serial = ''' + qrvenda_produto.FieldByName('serial').asstring + '''');
      ...
      begin
        ...
        frmvenda.qrproduto_grade.sql.Add('select * from c000021 where atb like :atb and codproduto = :codproduto and codigo = :codigo');
...
begin
  ObservacoesVenda.Obs1 := '';
  ObservacoesVenda.Obs2 := '';
  ObservacoesVenda.Obs3 := '';
  ObservacoesVenda.Obs4 := '';
  frmmodulo.qrconfig.Open;
  ...
  qrcliente.close;
  qrcliente.sql.Clear;
  qrcliente.sql.Add('select * from c000007 where atb like :atb and codigo = ''' + lcliente_codigo.Caption + '''');
  qrcliente.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
  qrcliente.Open;
  ...
  begin
    ...
    begin
      qrapoio2.close;
      qrapoio2.sql.Clear;
      qrapoio2.sql.Add('SELECT CODPRODUTO, SUM(QTDE) as quantidade FROM C000057 WHERE atb like :atb and CODORCAMENTO = :CODORCAMENTO group by CODPRODUTO');
      qrapoio2.ParamByName('atb').Value := ME_FiltraATB('TB_ORCAMENTO_ITENS');
    ...
    begin
      qrapoio.close;
      qrapoio.sql.Clear;
      qrapoio.sql.Add('select estoque_atual,codproduto from c000100 where atb like :atb order by codproduto');
      qrapoio.ParamByName('atb').Value := ME_FiltraATB('TB_ESTOQUE');
      qrapoio.Open;
      ...
      begin
        ...
        begin
          if qrapoio.Locate('codproduto', qrapoio2.FieldByName('CODPRODUTO').asstring, [loCaseInsensitive]) then
          begin
    ...
    begin
      ...
      begin
        qrapoio2.close;
        qrapoio2.sql.Clear;
        qrapoio2.sql.Add('SELECT * FROM C000057 WHERE atb like :atb and CODORCAMENTO = :CODORCAMENTO');
        qrapoio2.Params.ParamByName('CODORCAMENTO').asstring := '999999';
        qrapoio2.ParamByName('atb').Value := ME_FiltraATB('TB_ORCAMENTO_ITENS');
        qrapoio2.Open;
      end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if frmmodulo.qrproduto.Locate('CODIGO', qrapoio2.FieldByName('CODPRODUTO').asstring, [loCaseInsensitive]) then
            begin
            ...
            begin
              ...
              frmvenda.qrproduto_serial.sql.Add('select * from c000022 where atb like :atb and codproduto = :codproduto and codigo = :codigo');
              ...
              frmvenda.qrproduto_serial.ParamByName('atb').Value := ME_FiltraATB('TB_SERIAL_PRODUTO');
            ...
            begin
              ...
              frmvenda.qrproduto_grade.ParamByName('codigo').asstring := qrapoio2.FieldByName('codgrade').asstring;
              frmvenda.qrproduto_grade.ParamByName('codproduto').asstring := qrapoio2.FieldByName('CODPRODUTO').asstring;
              frmvenda.qrproduto_grade.ParamByName('atb').Value := ME_FiltraATB('TB_GRADE_PRODUTO');
              frmvenda.qrproduto_grade.Open;
...
begin
  ...
  frmmodulo.qrorcamento.sql.Add('select * from c000056 where atb like :atb and codcliente = :codcliente and GERA_NF = :GERA_NF order by codigo');
  ...
  frmmodulo.qrorcamento.close;
  frmmodulo.qrorcamento.sql.Clear;
  frmmodulo.qrorcamento.sql.Add('select * from c000056 where atb like :atb and codigo = :codigo');
  frmmodulo.qrorcamento.Params.ParamByName('codigo').asstring := frmmodulo.cod_orcamento;
  frmmodulo.qrorcamento.Open;

  ObservacoesVenda.Obs1 := frmmodulo.qrorcamentoOBS1.AsString;
  ObservacoesVenda.Obs2 := frmmodulo.qrorcamentoOBS2.AsString;
  ObservacoesVenda.Obs3 := frmmodulo.qrorcamentoOBS3.AsString;
  ObservacoesVenda.Obs4 := frmmodulo.qrorcamentoOBS4.AsString;
  ...
  begin
    ...
    with frmItensOrcamento do
    begin
      qrItens.close;
      qrItens.sql.Clear;
      qrItens.sql.Add('SELECT ' + QuotedStr('S') + ' AS SEL, OC.* FROM C000057 OC WHERE OC.atb like :atb and OC.CODORCAMENTO = :CODORCAMENTO');
      qrItens.Params.ParamByName('CODORCAMENTO').asstring := frmmodulo.cod_orcamento;
      qrItens.ParamByName('atb').Value := ME_FiltraATB('TB_ORCAMENTO_ITENS');
      qrItens.Open;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            frmvenda.qrvenda_produto.FieldByName('item').asstring := FRMPRINCIPAL.zerarcodigo(INTTOSTR(item), 3);
            serial_grade := 0;
            if frmmodulo.qrproduto.Locate('CODIGO', qrItens.FieldByName('CODPRODUTO').asstring, [loCaseInsensitive]) then
            begin
              if frmmodulo.qrproduto.FieldByName('piscofins').asstring = 'S' then
                frmvenda.qrvenda_produto.FieldByName('piscofins').asstring := 'S'
              else
...
procedure Tfrmvenda.O2Click(Sender: TObject);
begin
  ObservacoesVenda := RetOBSVenda(ObservacoesVenda);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      frmmodulo.qrgrade_produto.sql.Add('update c000021 set estoque = estoque+' + qrvenda_produto.FieldByName('qtde').asstring + ' where atb like :atb and codigo = ''' + qrvenda_produto.FieldByName('codgrade').asstring + '''');
