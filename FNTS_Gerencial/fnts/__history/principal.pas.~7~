...
begin
  ...
  begin
    ...
    if qrconfig.IsEmpty then begin
      Application.MessageBox('¡rea de Trabalho sem configuraÁıes!','AtenÁ„o!',MB_ICONINFORMATION);
...
begin
  ...
  begin
    if (RetRegistro.Situacao = tsBloqueado) then
      Application.MessageBox(PWideChar(RetRegistro.MensagemBloqueio), 'AtenÁ„o!', MB_ICONWARNING);
    Close;
  ...
  begin
    ...
    if not Empresa then
    begin
      Application.MessageBox('ImpossÌvel usar o sistema sem uma empresa cadastrada!', 'AtenÁ„o!', MB_ICONINFORMATION);
      goto Inicio;
    end;
    Application.CreateForm(TfrmSelecionaEmpresa, frmSelecionaEmpresa);
    frmSelecionaEmpresa.ShowModal;
    ...
    if ATB_ATIVA.ATB = '' then
    begin
      Application.MessageBox('ImpossÌvel continuar usando o sistema sem uma ·rea de trabalho ativa!', 'AtenÁ„o!', MB_ICONINFORMATION);
      goto Inicio;
    end;
  end;
  FIM:
  ...
  begin
    ...
    begin
      if application.messagebox(Pchar('A Data do Caixa "GERAL" est· diferente da atual!' + #13 + 'Deseja fechar o caixa do dia ' + qrcx.FIELDBYNAME('data').asstring + '?'), 'AtenÁ„o', MB_IconInformation + MB_YESNO) = idYes then
      begin
        Action5.Execute
      end;
    end;
...
begin
  if (MultiLoja) and not(PermiteCadastroProduto) then begin
    Application.MessageBox('O acesso a est· tela esta desativado devido a configuraÁ„o do sistema n„o permitir cadastro de produtos e dependentes nesta m·quina!','AtenÁ„o!',MB_ICONINFORMATION);
    Exit;
  end;
  barra_estoque.Visible := False;
