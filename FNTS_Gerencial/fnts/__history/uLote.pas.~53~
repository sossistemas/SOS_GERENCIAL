...
uses ZDataset, modulo, System.SysUtils, Data.SqlExpr, ZAbstractDataset;
...
type
  ...
  public
    ...
    function GravarLote: Boolean;
    function VerificarIDLote(vID:integer):Boolean;
    function GetLote(vID:integer):TZQuery;
...
function TLote.GetLote(vID: integer): TZQuery;
begin
  try
    FQryAUX.Close;
    FQryAUX.SQL.Clear;
    FQryAUX.SQL.Add('select * from lote L');
    FQryAUX.SQL.Add('where L.id =:id');
    FQryAUX.Params.ParamByName('id').AsInteger := vID;
    FQryAUX.Open;
...
function TLote.GravarLote: Boolean;
begin
  TRY
    with FSPLote_IU do
    begin
...
function TLote.VerificarIDLote(vID: integer): Boolean;
begin
  FQryAUX.Close;
  FQryAUX.SQL.Clear;
  FQryAUX.SQL.Add('select * from lote L');
  ...
  Result := FQryAUX.FieldByName('ID').AsInteger > 0;
end;
