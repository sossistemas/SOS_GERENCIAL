...
uses
  ...
  Dialogs, ExtCtrls, TFlatPanelUnit, StdCtrls, wwdblook, Mask, Grids,
  Wwdbigrd, Wwdbgrid, DB, Buttons, RzSpnEdt, AdvGlowButton, Vcl.Clipbrd,
  System.Math, Vcl.Menus, AdvMetroButton, AdvSmoothExpanderPanel,
  JvExMask, JvToolEdit, JvBaseEdits, AdvSmoothPanel, RzEdit;
...
type
  Tfrmvenda_contasreceber = class(TForm)
    ...
    procedure combocondpgtoChange(Sender: TObject);
  private
...
uses
  modulo, venda, venda_fechamento, formas_pgto;

{$R *.dfm}

procedure Tfrmvenda_contasreceber.combocondpgtoChange(Sender: TObject);
var
  prestacao: Integer;
  valor: real;
  StrX: string;
      ...
      begin
        ...
        begin
          ...
          if rsoma.Value > 0 then
            valor := rsoma.Value * (frmmodulo.qrcondpgto_parcela.fieldbyname('percentual').asfloat / 100);
          ...
          begin
            ...
            if tipox = '2' then
            begin
              frmvenda.qrvenda_contasreceber.fieldbyname('TIPO').asstring := 'CHEQUE A VISTA';
            end;
            if tipox = '3' then
            begin
              frmvenda.qrvenda_contasreceber.fieldbyname('TIPO').asstring := 'CARTAO DE DEBITO';
            end;

          end
          else

            frmvenda.qrvenda_contasreceber.fieldbyname('TIPO').asstring := frmmodulo.qrcondpgto_parcela.fieldbyname('TIPO').asstring;
...
procedure Tfrmvenda_contasreceber.combocondpgtoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
...
procedure Tfrmvenda_contasreceber.wwDBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure Tfrmvenda_contasreceber.wwDBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  ...
  begin
    ...
    xparcelaorig := RoundTo(frmvenda.qrvenda_contasreceber.fieldbyname('valor').asfloat, arredonda_valor);

    // gravo o valor da nova parcela
    valornovo := RoundTo(frmvenda.qrvenda_contasreceber.fieldbyname('valor').asfloat, arredonda_valor);
    ...
    begin
      ...
      begin
        ...
        xvalor2 := RoundTo(xvalor2 + frmvenda.qrvenda_contasreceber.fieldbyname('valor').asfloat, arredonda_valor);
      end;
      ...
      begin
        ...
        frmvenda.qrvenda_contasreceber.fieldbyname('valor').asfloat := xparcelaorig;
        frmvenda.qrvenda_contasreceber.post;
        exit;
      end;
      xvalor := RoundTo(xvalor + frmvenda.qrvenda_contasreceber.fieldbyname('valor').asfloat, arredonda_valor);
    ...
    begin
      ...
      begin
        frmvenda.qrvenda_contasreceber.edit;
        frmvenda.qrvenda_contasreceber.fieldbyname('valor').asfloat := RoundTo(xparcelaorig, arredonda_valor);
        frmvenda.qrvenda_contasreceber.post
      end;
...
procedure Tfrmvenda_contasreceber.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
