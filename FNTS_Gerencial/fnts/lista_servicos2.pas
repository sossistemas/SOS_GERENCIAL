...
uses
  modulo, principal, xloc_gruposervico;

{$R *.dfm}

procedure Tfrmlista_servicos2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  begin
    ...
    qrSERVICO.close;
    qrSERVICO.sql.clear;
    qrSERVICO.sql.add('select * from c000011 where atb like :atb and CODIGO IS NOT NULL ' + GRUPO + ' order by ' + COMBO_ORDEM.TEXT);
    ...
    FXSERVICO.LoadFromFile('\SOS\server\rel\f000013.fr3');
  ...
  begin
    ...
    qrgrupo.CLOSE;
    qrgrupo.SQL.Clear;
    qrgrupo.SQL.ADD('SELECT CODgrupo, COUNT(CODIGO) qtde FROM C000011 WHERE atb like :atb and codigo is not null ' + GRUPO + ' GROUP BY CODgrupo');
    qrgrupo.ParamByName('atb').Value := ME_FiltraATB('TB_SERVICOS');
    qrgrupo.OPEN;
    ...
    FXSERVICO.LoadFromFile('\SOS\server\rel\f000014.fr3');
    FXSERVICO.ShowReport;
    qrservico.MasterSource := nil;
    qrservico.MasterFields := '';
    qrservico.IndexFieldNames := '';
