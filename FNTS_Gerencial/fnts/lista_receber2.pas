...
uses
  ...
  Menus, Grids, DBGrids, wwdblook, AdvGlowButton,
...
uses
  modulo, loc_caixa, xloc_cliente, xloc_funcionario, principal;
...
procedure Tfrmlista_receber2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
var
  cliente, vendedor, cedente, caixa, tipo, forma, situacao, periodo, ordem, conta: string;
begin
  ...
  begin
    vendedor := ' and CR.CODvendedor = ''' + copy(combo_vendedor.text, 1, 6) + '''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA4').asstring := 'VENEDEDOR: ' + COMBO_vendedor.TEXT;
  end;
  ...
  begin
    forma := ' and CR.TIPO like ''%' + combo_FORMA.text + '%''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring := frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring + 'FORMA DE RECEBIMENTO: ' + COMBO_FORMA.TEXT;
  end;
  ...
  begin
    if COMBO_SITUACAO.ITEMINDEX = 0 then
      situacao := ' and CR.situacao = 1 ';
    if COMBO_SITUACAO.ITEMINDEX = 1 then
      situacao := ' and CR.situacao = 2 ';
  ...
  begin
    ...
    QRCONTASRECEBER.sql.add('select  CR.*, CLIENTE.NOME CLIENTE, cliente.cpf, cliente.endereco, cliente.bairro, cliente.cidade, cliente.uf, cliente.cep, cliente.telefone1, cliente.complemento  ');
    QRCONTASRECEBER.sql.add('from c000049 CR, C000007 CLIENTE where CR.atb like :atbCR and CLIENTE.atb like :atbCLIENTE and CR.CODCLIENTE = CLIENTE.CODIGO AND ' + periodo + cliente + vendedor + caixa + situacao + cedente + forma + ' order by ' + ordem);
    ...
    fxcontas_receber.LoadFromFile('\SOS\server\rel\f000058.fr3');
  ...
  begin
    ...
    QRCONTASRECEBER1.sql.add('select CR.*, CLIENTE.NOME CLIENTE from c000050 CR, C000007 CLIENTE where CR.atb like :atbCR and CLIENTE.atb like :atbCLIENTE and CR.CODCLIENTE = CLIENTE.CODIGO AND ' + periodo + cliente + ' order by ' + ordem);
    ...
    QRCONTASRECEBER1.ParamByName('atbCR').Value := ME_FiltraATB('TB_CONTA_RECEBER');
    QRCONTASRECEBER1.ParamByName('atbCLIENTE').Value := ME_FiltraATB('TB_CLIENTE');
    QRCONTASRECEBER1.open;
    QRCONTASRECEBER1.REFRESH;

    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELATÓRIO DE RECEBIMENTOS';
    fxcontas_receber.LoadFromFile('\SOS\server\rel\f000058_1.fr3');
    fxcontas_receber.ShowReport;
  end;
  ...
  begin
    ...
    QRCONTASRECEBER.sql.clear;
    QRCONTASRECEBER.sql.add('select CR.*, CLIENTE.NOME CLIENTE, cliente.cpf, cliente.endereco, cliente.bairro, cliente.cidade, cliente.uf, cliente.cep, cliente.telefone1, cliente.complemento ');
    QRCONTASRECEBER.sql.add('from c000049 CR, C000007 CLIENTE where CR.atb like :atbCR and CLIENTE.atb like :atbCLIENTE and CR.CODCLIENTE = CLIENTE.CODIGO AND ' + periodo + cliente + vendedor + caixa + situacao + cedente + forma + ' order by cliente');
    QRCONTASRECEBER.Params.ParamByName('datai').asdatetime := dateedit1.date;
    QRCONTASRECEBER.Params.ParamByName('dataf').asdatetime := dateedit2.date;
    QRCONTASRECEBER.ParamByName('atbCR').Value := ME_FiltraATB('TB_CONTA_RECEBER');
    QRCONTASRECEBER.ParamByName('atbCLIENTE').Value := ME_FiltraATB('TB_CLIENTE');
    ...
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELATÓRIO DE CONTAS A RECEBER';
    fxcontas_receber.LoadFromFile('\SOS\server\rel\f000059.fr3');
    fxcontas_receber.ShowReport;
  end;
  ...
  begin
    ...
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELATÓRIO DE CONTAS A RECEBER';
    fxcontas_receber.LoadFromFile('\SOS\server\rel\f000059_2.fr3');
    fxcontas_receber.ShowReport;
  end;
  ...
  begin
    ...
    QRRESUMO.SQL.ADD('jOIN c000049 cr on cliente.codigo = cr.codcliente  and cr.valor_atual <> 0  and ' + periodo + cliente + vendedor + cedente + caixa + situacao + forma + ' where CR.atb like :atbCR and CLIENTE.atb like :atbCLIENTE group by cliente.codigo, cliente.nome ');
    ...
    fxcontas_receber.LoadFromFile('\SOS\server\rel\f000060.fr3');
    fxcontas_receber.ShowReport;
  end;
  ...
  begin
    QRCONTASRECEBER.close;
    QRCONTASRECEBER.sql.clear;
    QRCONTASRECEBER.sql.add('select CR.*, CLIENTE.NOME CLIENTE, cliente.cpf, cliente.endereco, cliente.bairro, cliente.cidade, cliente.uf, cliente.cep, cliente.telefone1, cliente.complemento  from c000049 CR, C000007 CLIENTE ');
    QRCONTASRECEBER.sql.add('where CR.atb like :atbCR and CLIENTE.atb like :atbCLIENTE and CR.CODCLIENTE = CLIENTE.CODIGO AND ' + periodo + cliente + vendedor + caixa + cedente + situacao + forma + ' order by cliente');
    QRCONTASRECEBER.Params.ParamByName('datai').asdatetime := dateedit1.date;
    QRCONTASRECEBER.Params.ParamByName('dataf').asdatetime := dateedit2.date;
    QRCONTASRECEBER.ParamByName('atbCR').Value := ME_FiltraATB('TB_CONTA_RECEBER');
    QRCONTASRECEBER.ParamByName('atbCLIENTE').Value := ME_FiltraATB('TB_CLIENTE');
    ...
    fxcontas_receber.LoadFromFile('\SOS\server\rel\f000062.fr3');
    fxcontas_receber.ShowReport;
  end;
  ...
  begin
    ...
    QRCONTASRECEBER.sql.clear;
    QRCONTASRECEBER.sql.add('select CR.*, CLIENTE.NOME CLIENTE, cliente.cpf, cliente.endereco, cliente.bairro, cliente.cidade, cliente.uf, cliente.cep, cliente.telefone1, cliente.complemento  ');
    QRCONTASRECEBER.sql.add('from c000049 CR, C000007 CLIENTE where CR.atb like :atbCR and CLIENTE.atb like :atbCLIENTE and CR.CODCLIENTE = CLIENTE.CODIGO AND ' + periodo + cliente + vendedor + caixa + cedente + situacao + forma + ' order by cliente');
    QRCONTASRECEBER.Params.ParamByName('datai').asdatetime := dateedit1.date;
    QRCONTASRECEBER.Params.ParamByName('dataf').asdatetime := dateedit2.date;
    QRCONTASRECEBER.ParamByName('atbCR').Value := ME_FiltraATB('TB_CONTA_RECEBER');
    QRCONTASRECEBER.ParamByName('atbCLIENTE').Value := ME_FiltraATB('TB_CLIENTE');
    ...
    fxcontas_receber.LoadFromFile('\SOS\server\rel\f000070.fr3');
    fxcontas_receber.ShowReport;
  end;
...
begin
  ...
  begin
    forma := ' and TIPO like ''%' + combo_FORMA.text + '%''';
  end;
  if combo_SITUACAO.Text = 'TODAS' then
    situacao := ''
  else
  ...
  query.close;
  query.sql.clear;
  query.sql.add('select * from c000049 where atb like :atb and codcliente = ''' + qrresumoCODIGO.value + ''' AND ' + periodo + vendedor + caixa + situacao + forma);
