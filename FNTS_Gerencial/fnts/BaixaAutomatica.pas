    ...
    try
      ...
      while not cdsBaixa.Eof do begin
        if (cdsBaixaSTATUS.AsString = 'BAIXAR') then begin
          ...
          SqlAux.ParamByName('atb').AsString := ME_GravaATB('TB_CONTA_RECEBER');


          //Movimento da conta bancarias
          SqlAux.ExecSQL;
          SqlAux.SQL.Clear;
          SqlAux.SQL.Add('insert into C000042 (CODIGO, CODCONTACORRENTE, DATA, HISTORICO, CODCONTA, VALOR, TIPO, DOCUMENTO, ATB)');
          SqlAux.SQL.Add('values (:CODIGO, :CODCONTACORRENTE, :DATA, :HISTORICO, :CODCONTA, :VALOR, :TIPO, :DOCUMENTO, :ATB)');
          SqlAux.ParamByName('CODIGO').AsString := frmBoleto.codifica('000042');
          ...
          frmBoleto.qrconfig.Open;
          SqlAux.ParamByName('CODCONTA').AsString := frmBoleto.qrconfig.FieldByName('PLANO_RECEBTO_CREDIARIO').asstring;
          ...
          SqlAux.ParamByName('ATB').AsString := ME_GravaATB('TB_MOV_CONTA_CORRENTE');
          SqlAux.ExecSQL;

          SqlAux.SQL.Clear;
          SqlAux.SQL.Add('insert into c000050');
          SqlAux.SQL.Add('(codconta,data,valor_parcela,valor_juros,valor_desconto,valor_pago,boleto,codcliente,caixa_banco, ATB)');
          ...
          SqlAux.params.parambyname('ATB').asstring := ME_GravaATB('TB_CONTA_RECEBER_PGTO');
          SqlAux.execsql;
        end;
        cdsBaixa.Next;
      end;
