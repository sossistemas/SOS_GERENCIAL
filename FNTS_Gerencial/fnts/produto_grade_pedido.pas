...
uses
  ...
  Dialogs, DB, ZAbstractRODataset, ZDataset, StdCtrls, Buttons,
  ExtCtrls, TFlatPanelUnit, Collection, Grids, Wwdbigrd, Wwdbgrid, Menus, Mask,
  JvExMask, JvToolEdit, JvBaseEdits, ZAbstractDataset;
...
type
  Tfrmproduto_grade_pedido = class(TForm)
    ...
    Gravar1: TMenuItem;
    Fechar1: TMenuItem;
    qrgrade_pedidoCODIGO: TWideStringField;
    qrgrade_pedidoCODPRODUTO: TWideStringField;
    qrgrade_pedidoCODNOTA: TWideStringField;
    qrgrade_pedidoQTDE: TFloatField;
    qrgrade_pedidoQTDE_REC: TFloatField;
    qrgrade_pedidoCODGRADE: TWideStringField;
    qrgrade_pedidoCODITEM: TWideStringField;
...
uses
  modulo, produto_pedido, principal;
...
begin
  ...
  qrgrade_pedido.SQL.add('select * from c000039 where atb like :atb and codITEM = ''' + frmPRODUTO_pedido.qrpedido_ITEM.fieldbyname('codigo').asstring + ''' order by codigo');
  ...
  begin
    ...
    frmmodulo.qrgrade_produto.sql.add('select * from c000021 where atb like :atb and codproduto = ''' + frmmodulo.qrproduto.fieldbyname('codigo').asstring + ''' order by codigo');
    ...
    begin
      ...
      qrgrade_pedido.fieldbyname('atb').asstring := ME_GravaATB('TB_PEDIDO_COMPRA_ITENS_GRADE');
      qrgrade_pedido.post;
...
begin
  try
    if rdiferenca.Value = 0 then
      rdiferenca.Font.Color := clblue
    else
      rdiferenca.Font.Color := clred;
  except
  end;
end;
...
procedure Tfrmproduto_grade_pedido.qrgrade_pedidoCalcFields(DataSet: TDataSet);
begin
  qrgrade_pedido.FieldByName('FALTA').ASFLOAT := qrgrade_pedido.FieldByName('QTDE').ASFLOAT - qrgrade_pedido.FieldByName('QTDE_REC').ASFLOAT;
