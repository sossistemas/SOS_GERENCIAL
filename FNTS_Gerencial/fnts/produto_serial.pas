...
uses
  ...
  ZDataset, StdCtrls, Buttons, ExtCtrls, TFlatPanelUnit, Collection, Mask, Menus,
...
uses
  modulo, principal, compra, compra_menu;
...
procedure Tfrmproduto_serial.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  qrserial_produto.sql.add('select * from c000022 where atb like :atb and coditem = ''' + frmcompra.qrnota_item.fieldbyname('codigo').asstring + ''' order by codigo');
...
begin
  ...
  if qrserial_produto.fieldbyname('estoque').asinteger = 0 then
...
procedure Tfrmproduto_serial.wwDBGrid2DrawDataCell(Sender: TObject; const Rect: TRect; Field: TField; State: TGridDrawState);
begin
