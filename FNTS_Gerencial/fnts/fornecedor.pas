...
uses
  ...
  Mask, maskutils, PageView, Wwdbigrd, SHELLAPI, Wwdbgrid, ImgList,
  AdvGlowButton, UCBase, dxGDIPlusClasses, Vcl.Imaging.jpeg,
  pngimage, AdvMetroButton,
  AdvSmoothExpanderPanel,
  JvToolEdit, JvBaseEdits, JvExMask, JvDBControls,
  AdvSmoothButton, System.ImageList, ZAbstractDataset, AdvSmoothPanel;
...
type
  Tfrmfornecedor = class(TForm)
    ...
    procedure GRID5DrawDataCell(Sender: TObject; const Rect: TRect; Field: TField; State: TGridDrawState);
...
begin
  ...
  frmmodulo.qrFORNECEDOR.SQL.add('select * from c000009 where atb like :atb order by codigo');
...
begin
  ...
  begin
    ...
    qrduplo.close;
    qrduplo.SQL.clear;
    qrduplo.SQL.add('select * from c000046 where atb like :atb and codfornecedor = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''');
    qrduplo.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
    qrduplo.open;
    ...
    begin
      ...
      qrduplo.close;
      qrduplo.SQL.clear;
      qrduplo.SQL.add('select * from c000087 where atb like :atb and codfornecedor = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''');
      qrduplo.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA');
      qrduplo.open;
      if qrduplo.recordcount > 0 then
      begin
    ...
    begin
      ...
      begin
        ...
        begin
          QUERY1.close;
          QUERY1.SQL.clear;
          QUERY1.SQL.add('select * from c000009 where atb like :atb and CNPJ = ''' + DBEdit15.Text + '''');
          QUERY1.ParamByName('atb').Value := ME_FiltraATB('TB_FORNECEDOR');
          QUERY1.open;
...
begin
  ...
  qrsugestao.close;
  PageView1Change(frmfornecedor);
  if (frmmodulo.qrFORNECEDOR.State <> dsinsert) and (frmmodulo.qrFORNECEDOR.State <> dsedit) then
  begin
    try
...
begin
  ...
  begin
    ...
    QUERY.close;
    QUERY.SQL.clear;
    QUERY.SQL.add('select SUM(TOTAL_NOTA) TOTAL_GERAL from c000087 where atb like :atb and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''');
    ...
    qrnf.close;
    qrnf.SQL.clear;
    qrnf.SQL.add('select * from c000087 where atb like :atb and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + ''' order by data_LANCAMENTO, NUMERO');
    ...
    QRCOMPRA_MES.SQL.add('select sum(TOTAL_NOTA) sum_0, ' +
      '(select sum(TOTAL_NOTA) from c000087 where atb like :atb and data_lancamento >= :data11 and data_lancamento < :data10 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''), ' +
      '(select sum(TOTAL_NOTA) from c000087 where atb like :atb and data_lancamento >= :data10 and data_lancamento < :data09 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''), ' +
      '(select sum(TOTAL_NOTA) from c000087 where atb like :atb and data_lancamento >= :data09 and data_lancamento < :data08 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''), ' +
      '(select sum(TOTAL_NOTA) from c000087 where atb like :atb and data_lancamento >= :data08 and data_lancamento < :data07 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''), ' +
      '(select sum(TOTAL_NOTA) from c000087 where atb like :atb and data_lancamento >= :data07 and data_lancamento < :data06 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''), ' +
      '(select sum(TOTAL_NOTA) from c000087 where atb like :atb and data_lancamento >= :data06 and data_lancamento < :data05 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''), ' +
      '(select sum(TOTAL_NOTA) from c000087 where atb like :atb and data_lancamento >= :data05 and data_lancamento < :data04 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''), ' +
      '(select sum(TOTAL_NOTA) from c000087 where atb like :atb and data_lancamento >= :data04 and data_lancamento < :data03 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''), ' +
      '(select sum(TOTAL_NOTA) from c000087 where atb like :atb and data_lancamento >= :data03 and data_lancamento < :data02 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''), ' +
      '(select sum(TOTAL_NOTA) from c000087 where atb like :atb and data_lancamento >= :data02 and data_lancamento < :data01 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''), ' + '(select sum(TOTAL_NOTA) from c000087 where data_lancamento >= :data01 and data_lancamento < :data00 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + ''') ' + 'from c000087 where data_lancamento >= :data12 and data_lancamento < :data11 and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''');
    ...
    QRCOMPRA_MES.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA');
    QRCOMPRA_MES.open;
    ...
    QRNF_PRODUTO.SQL.add('select * from c000088 where atb like :atb and codFORNECEDOR = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''');
    QRNF_PRODUTO.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
    QRNF_PRODUTO.open;

    qrcontasPAGAR.close;
    qrcontasPAGAR.SQL.clear;
    qrcontasPAGAR.SQL.add('select * from c000046 where atb like :atb and codFORNECEDOR =''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + ''' and situacao = 1 order by data_vencimento, codigo');
    qrcontasPAGAR.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
    qrcontasPAGAR.open;

    qrcontasPAGARpg.close;
    qrcontasPAGARpg.SQL.clear;
    qrcontasPAGARpg.SQL.add('select * from c000046 where atb like :atb and codFORNECEDOR =''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + ''' and situacao = 2 order by data_PAGAMENTO, codigo');
    qrcontasPAGARpg.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
  ...
  begin
    ...
    frmmodulo.qrfornecedor_codigo.SQL.add('where atb like :atb and codfornecedor = :vfor');
...
begin
  QRNF_PRODUTO.close;
  QRNF_PRODUTO.SQL.clear;
  QRNF_PRODUTO.SQL.add('select * from c000088 where atb like :atb and codnota = ''' + qrnf.fieldbyname('codigo').asstring + '''');
  QRNF_PRODUTO.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
  QRNF_PRODUTO.open;
  ...
  qrcontasPAGAR.close;
  qrcontasPAGAR.SQL.clear;
  qrcontasPAGAR.SQL.add('select * from c000046 where atb like :atb and codNOTA = ''' + qrnf.fieldbyname('codigo').asstring + ''' and codFORNECEDOR =''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + ''' and situacao = 1 order by data_vencimento, codigo');
  qrcontasPAGAR.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
  qrcontasPAGAR.open;

  qrcontasPAGARpg.close;
  qrcontasPAGARpg.SQL.clear;
  qrcontasPAGARpg.SQL.add('select * from c000046 where atb like :atb and codNOTA = ''' + qrnf.fieldbyname('codigo').asstring + ''' and codFORNECEDOR =''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + ''' and situacao = 2 order by data_PAGAMENTO, codigo');
  qrcontasPAGARpg.ParamByName('atb').Value := ME_FiltraATB('TB_CONTAS_PAGAR');
  qrcontasPAGARpg.open;
  rtotal_pago.value := 0;
  rtotal.value := 0;
...
begin
  ...
  begin
    tipo := 'CNPJ';
    campo := PWideChar(frmprincipal.somenteNumero(frmmodulo.qrFORNECEDOR.fieldbyname('CNPJ').asstring));
  end
  else
  begin
    ...
    begin
      tipo := 'IE';
      campo := PWideChar(frmprincipal.somenteNumero(frmmodulo.qrFORNECEDOR.fieldbyname('IE').asstring));
    end
    else
    begin
...
begin
  ...
  begin
    ...
    qrduplo.ParamByName('atb').Value := ME_FiltraATB('TB_FORNECEDOR');
    qrduplo.open;
    if qrduplo.recordcount > 0 then
    begin
      texto := PWideChar('Existe um fornecedor cadastrado com este nome!' + #13 + qrduplo.fieldbyname('codigo').asstring + ' - ' + qrduplo.fieldbyname('nome').asstring + #13 + 'Endereço: ' + qrduplo.fieldbyname('ENDERECO').asstring + '!' + #13 + 'Deseja prosseguir?');
...
begin
  ...
  else if combo_pessoa.ItemIndex = 2 then
  begin
  ...
  else if combo_pessoa.ItemIndex = 3 then
  begin
    LAPELIDO.Caption := 'Nome:';
    LRG.Caption := 'IE/RG:';
...
begin
  if Trim(combo_pessoa.Text) = '' then
  begin
...
begin
  ...
  qrsugestao.SQL.add('where pro.atb like :atbpro and est.atb like :atbest and pro.situacao = 0 and pro.codigo = est.codproduto and codfornecedor = ''' + frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring + '''');
...
procedure Tfrmfornecedor.ldataini_remarcadoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    PERFORM(WM_NEXTDLGCTL, 0, 0);
end;

procedure Tfrmfornecedor.ldatafim_remarcadoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bfiltrar.SetFocus;
end;
...
procedure Tfrmfornecedor.GRID5DrawDataCell(Sender: TObject; const Rect: TRect; Field: TField; State: TGridDrawState);
begin
...
procedure Tfrmfornecedor.dsfornecedor2DataChange(Sender: TObject; Field: TField);
begin
  if (frmmodulo.qrFORNECEDOR.State <> dsinsert) and (frmmodulo.qrFORNECEDOR.State <> dsedit) then
  begin
    try
