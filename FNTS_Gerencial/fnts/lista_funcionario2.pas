...
procedure Tfrmlista_funcionario2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  if combo_FUNCIONARIO.Text = 'TODOS' then
    funcionario := ''
  else
  begin
    funcionario := ' and codIGO = ''' + copy(combo_FUNCIONARIO.text, 1, 6) + '''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA3').asstring := 'FUNCIONÁRIO: ' + COMBO_FUNCIONARIO.TEXT;
  end;
  ...
  begin
    ...
    qrFUNCIONARIO.sql.add('select * from c000008 where atb like :atb and NOME IS NOT NULL ' + funcionario + ' order by ' + COMBO_ORDEM.TEXT);
    qrFUNCIONARIO.ParamByName('atb').Value := ME_FiltraATB('TB_FUNCIONARIO');
    qrFUNCIONARIO.open;
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELAÇÃO DE FUNCIONÁRIOS';
    FXFUNCIONARIO.LoadFromFile('\SOS\server\rel\f000007.fr3');
  ...
  begin
    ...
    FXFUNCIONARIO.LoadFromFile('\SOS\server\rel\f000008.fr3');
    FXFUNCIONARIO.ShowReport;
  end;
