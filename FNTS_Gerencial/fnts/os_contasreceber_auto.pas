...
uses
  ...
  Wwdbigrd, Wwdbgrid, DB, Buttons, AdvGlowButton, RzSpnEdt, JvExMask,
  JvToolEdit, JvBaseEdits, RzEdit;
...
uses
  modulo, os_auto, os_fechamento_auto;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          valor := frmOS_fechamento_auto.rCREDIARIO.Value * (frmmodulo.qrcondpgto_parcela.fieldbyname('percentual').asfloat / 100);
...
procedure Tfrmos_contasreceber_auto.combocondpgtoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
...
procedure Tfrmos_contasreceber_auto.wwDBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    BGRAVAR.SETFOCUS;
end;
...
begin
  ...
  begin
    application.MessageBox('Os valores informados nas parcelas não confere com o valor total desta O.S.! Favor verificar!', 'Atenção', mb_ok + MB_ICONWARNING);
    wwdbgrid1.setfocus;
    exit;
  end;
...
procedure Tfrmos_contasreceber_auto.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;
...
procedure Tfrmos_contasreceber_auto.rqtdeKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    perform(wm_nextdlgctl, 0, 0);
end;
