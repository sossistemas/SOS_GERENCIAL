...
procedure Tfrmxloc_convenio.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  PARAMETRO_PESQUISA := '';
  Action := CAFREE;
end;
...
procedure Tfrmxloc_convenio.locKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = vk_down then
  begin
    GRID.setfocus;
...
begin
  ...
  begin
    query.SQL.Add('select * from c000071 where atb like :atb and codigo Is NOT NULL ORDER BY NOME');
    query.ParamByName('atb').Value := ME_FiltraATB('TB_CONVENIO');
    query.open;
    loc.setfocus;
  end
  else
  begin
    query.SQL.Add('select * from c000071 where atb like :atb and codigo Is NOT NULL AND UPPER(NOME) LIKE ''' + ANSIUPPERCASE(PARAMETRO_PESQUISA) + '%'' ORDER BY NOME');
    query.ParamByName('atb').Value := ME_FiltraATB('TB_CONVENIO');
    query.open;
    GRID.setfocus;
  end;
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.SQL.Add('select * from c000071 where atb like :atb and upper(' + tipo + ') like ''' + ref + loc.text + '%'' order by ' + tipo);
    query.ParamByName('atb').Value := ME_FiltraATB('TB_CONVENIO');
    query.open;
  except
  end;
