...
uses
  ...
  Wwdbigrd, Wwdbgrid, DB, DBCtrls, Menus, frxDesgn, frxClass,
  frxDBSet, frxDCtrl, ZAbstractRODataset, ZDataset, UCBase,
...
uses
  xloc_financeira, modulo, principal, financeira_pgto, financeira_ficha,
...
begin
  ...
  begin
    ...
    query.sql.add('(CODIGO, CODBANCO, DATA, CODCLIENTE, VALOR, SITUACAO, DATA_PGTO, VALOR_PAGO, BORDERO, PROPOSTA, OBS, CODVENDA, ATB)');
    ...
    query.ParamByName('ATB').ASSTRING := ME_GravaATB('TB_FINANCEIRA_LANCAMENTO');
    query.ExecSQL;
...
begin
  ...
  begin
    ...
    begin
      ...
      query.sql.add('where atb like :atb and codigo = ''' + qrfinanceira.fieldbyname('codigo').ASSTRING + '''');
      ...
      query.ParamByName('OBS').ASSTRING := FI_OBS;
      query.ParamByName('VALOR').ASFLOAT := STRTOFLOAT(FI_VALOR);
      query.ParamByName('ATB').ASSTRING := ME_FiltraATB('TB_FINANCEIRA_LANCAMENTO');
      query.ExecSQL;
...
begin
  ...
  begin
    frmprincipal.logUC('Excluiu Mov.Financeira - CodVenda: ' + qrfinanceira.fieldbyname('CODVENDA').ASSTRING, 3);
...
procedure TfrmFinanceira.ed_cod_financeiraKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
begin
  ...
  qrfinanceira.sql.add('c000123.atb like :atbc000123 and ');
  qrfinanceira.sql.add('c000007.atb like :atbc000007 and ');
  qrfinanceira.sql.add('c000013.atb like :atbc000013 and ');
  ...
  qrfinanceira.ParamByName('atbc000123').Value := ME_FiltraATB('TB_FINANCEIRA_LANCAMENTO');
  qrfinanceira.ParamByName('atbc000007').Value := ME_FiltraATB('TB_CLIENTE');
  qrfinanceira.ParamByName('atbc000013').Value := ME_FiltraATB('TB_BANCO');
  qrfinanceira.open;
...
begin
  ...
  begin
    ...
    begin
      ...
      frmprincipal.logUC('Estornou Baixa Mov.Financeira - CodVenda: ' + qrfinanceira.fieldbyname('CODVENDA').ASSTRING, 3);
