...
uses
  ...
  DBAccess, Mask, RzEdit,
  RzSpnEdt, AdvReflectionImage;

type
  Tfrmvenda_impressaoCP = class(TForm)
...
var
  frmvenda_impressaoCP: Tfrmvenda_impressaoCP;
...
uses modulo, principal, venda, venda_fechamento, ecf,
  extenso1, venda_contasreceber,
  senha, cliente, fornecedor, transportador, funcionario, servico, banco, formapgto,
  ...
  senha_caixa, contaspagar, venda_inicio,
  ...
  sintegra_menu,
  ECF_REDUCAO, prevenda, compra, compra_menu, ecf_reducao_menu, notafiscal_menu, progresso, usuario, cheque_menu,
  ...
  xloc_csosn, importa_nfe;

{$R *.dfm}

procedure Tfrmvenda_impressaoCP.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action := cafree;
end;

procedure Tfrmvenda_impressaoCP.bcancelarClick(Sender: TObject);
begin
  close;
end;

procedure Tfrmvenda_impressaoCP.bgravarClick(Sender: TObject);
begin
{
01 - Cupom Fiscal
02 - Nota Fiscal
...
10 - Papel Razão - Laser / Tinta
  ...
  case combo_modelo.ItemIndex of
    0: TPCupom_FiscalClick(frmvenda_impressaoCP);
    1: TPNota_FiscalClick(frmvenda_impressaoCP);
    2: TPBobinaClick(frmvenda_impressaoCP);
    3: TPRazaoClick(frmvenda_impressaoCP);
    4: TPCartaClick(frmvenda_impressaoCP);

    6: TPContratoClick(frmvenda_impressaoCP);
    7: TPPersonalizadoClick(frmvenda_impressaoCP);

    9: TPrazaotintaclick(frmvenda_impressaoCP);
    10: FormulrioContnuoRazo21click(frmvenda_impressaoCP);
    11: ContratoCapa1Click(frmvenda_impressaoCP);
    12: Bobinasemformatao1click(frmvenda_impressaoCP);
    13: PedidoCarne1Click(frmvenda_impressaoCP);
    14: TPContrato1Click(frmvenda_impressaoCP);
    15: PedidoCarncomcdigodebarras1Click(frmvenda_impressaocp);
    16: RazoLaser21Click(frmvenda_impressaocp);
  end;
...
procedure Tfrmvenda_impressaoCP.FormShow(Sender: TObject);
begin
...
procedure Tfrmvenda_impressaoCP.combo_modeloKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then bgravar.SetFocus;
end;

procedure Tfrmvenda_impressaoCP.eviasKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then bgravar.setfocus;
end;

procedure Tfrmvenda_impressaoCP.TPNota_FiscalClick(Sender: TObject);
begin
//verificar
end;

procedure Tfrmvenda_impressaoCP.TPBobinaClick(Sender: TObject);
...
procedure Tfrmvenda_impressaoCP.TPRazaoClick(Sender: TObject);
begin
...
procedure Tfrmvenda_impressaoCP.TPCartaClick(Sender: TObject);
var arquivo: textfile;
  texto1, texto2, texto3, texto4, texto5, texto6: string;
...
procedure Tfrmvenda_impressaoCP.TPContratoClick(Sender: TObject);
begin
  qrcontasreceber.close;
  qrcontasreceber.sql.Clear;
  qrcontasreceber.sql.add('select * from c000049 where codvenda = ''' + qrvenda.fieldbyname('codigo').asstring + ''' order by data_vencimento');
  ...
  begin
    ...
    frmmodulo.qrrelatorio.fieldbyname('linha6').asstring := IntToStr(frmvenda.qrvenda_produto.RecordCount);
    frmmodulo.qrrelatorio.post;
    FXvenda.LoadFromFile('C:\SOS\server\rel\f000048.fr3');
    FXvenda.ShowReport;
...
procedure Tfrmvenda_impressaoCP.TPPersonalizadoClick(Sender: TObject);
...
procedure Tfrmvenda_impressaoCP.tprazaotintaClick(Sender: TObject);
var formapgto, v1, v2: string;
  y: integer;
begin
...
procedure Tfrmvenda_impressaoCP.TPCupom_FiscalClick(Sender: TObject);
...
procedure Tfrmvenda_impressaoCP.FormulrioContnuoRazo21Click(Sender: TObject);
var arquivo: textfile;
  texto1, texto2, texto3, texto4, texto5, texto6: string;
  conta_formas, I, VIAS: integer;
  dinheiro, chequeav, chequeap, cartaocred, cartaodeb, crediario: real;
...
procedure Tfrmvenda_impressaoCP.ContratoCapa1Click(Sender: TObject);
begin
  qrcontasreceber.close;
  qrcontasreceber.sql.Clear;
  qrcontasreceber.sql.add('select * from c000049 where codvenda = ''' + qrvenda.fieldbyname('codigo').asstring + ''' order by data_vencimento');
  ...
  begin
    ...
    FXvenda.LoadFromFile('C:\SOS\server\rel\f000049.fr3');
    FXvenda.ShowReport;
    FXvenda.LoadFromFile('C:\SOS\server\rel\f000081.fr3');
    FXvenda.ShowReport;
...
procedure Tfrmvenda_impressaoCP.Bobinasemformatao1Click(Sender: TObject);
var arquivo: textfile;
  texto1, texto2, texto3, texto4, texto5, texto6: string;
...
procedure Tfrmvenda_impressaoCP.PedidoCarne1Click(Sender: TObject);
begin
  qrcontasreceber.close;
  qrcontasreceber.sql.Clear;
...
procedure Tfrmvenda_impressaoCP.TPContrato1Click(Sender: TObject);
begin
  qrcontasreceber.close;
  qrcontasreceber.sql.Clear;
  qrcontasreceber.sql.add('select * from c000049 where codvenda = ''' + qrvenda.fieldbyname('codigo').asstring + ''' order by data_vencimento');
  ...
  begin
    ...
    frmmodulo.qrrelatorio.fieldbyname('linha6').asstring := IntToStr(frmvenda.qrvenda_produto.RecordCount);
    frmmodulo.qrrelatorio.post;
    FXvenda.LoadFromFile('C:\SOS\server\rel\f000048_1.fr3');
    FXvenda.ShowReport;
    FXvenda.LoadFromFile('C:\SOS\server\rel\f000049.fr3');
    FXvenda.ShowReport;
...
procedure Tfrmvenda_impressaoCP.PedidoCarncomcdigodebarras1Click(
  Sender: TObject);
begin
  qrcontasreceber.close;
  qrcontasreceber.sql.Clear;
...
procedure Tfrmvenda_impressaoCP.RazoLaser21Click(Sender: TObject);
begin
