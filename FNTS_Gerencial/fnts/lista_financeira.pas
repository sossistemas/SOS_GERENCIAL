...
procedure Tfrmlista_financeira.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  begin
    cliente := ' and c000123.codcliente = ''' + copy(combo_cliente.text, 1, 6) + '''';
  ...
  begin
    banco := ' and c000123.CODbanco = ''' + copy(combo_banco.text, 1, 3) + '''';
  ...
  begin
    if COMBO_SITUACAO.ITEMINDEX = 1 then
      situacao := ' and C000123.situacao = 1 ';
    if COMBO_SITUACAO.ITEMINDEX = 2 then
      situacao := ' and C000123.situacao = 2 ';
  ...
  begin
    ...
    fxFinanceira.LoadFromFile('\SOS\server\rel\f000182.fr3');
...
begin
  case qrFinanceira.fieldbyname('SITUACAO').asinteger of
    1:
      qrFinanceira.fieldbyname('SIT').Value := 'ABERTO';
    2:
      qrFinanceira.fieldbyname('SIT').Value := 'BAIXADO';
  end;
end;
