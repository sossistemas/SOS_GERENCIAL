...
type
  TfrmLancLotes = class(TForm)
    ...
    procedure jvDataFabricaProdExit(Sender: TObject);
    procedure jvDataFabricaProdKeyPress(Sender: TObject; var Key: Char);
  private
    FIncluir: Boolean;
    FLote : TLote;
    ...
    procedure CarregarItensLoteII(vIDLote:integer);
...
uses modulo, xloc_fornecedor, xloc_produto, uGestLotes;

procedure TfrmLancLotes.btSairClick(Sender: TObject);
begin
  frmLancLotes.Close;

  if frmLotes.Showing then
    frmLotes.bfiltrarClick(Sender);
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        frmLancLotes.jvDataFabricaProd.SetFocus;
      end;
...
begin
  ...
  begin
    ...
    jvDataFabricaProd.Text   := DateToStr(cdsTempItensLoteFABRICACAO.Value);
    ...
    cxQtdProd.Text           := FormatFloat('0.000', cdsTempItensLoteQTD.AsFloat);
    cxValCusto.Text          := FormatCurr('0.00', cdsTempItensLotePRECUSTO.AsCurrency);
    cxValTotal.Text          := FormatCurr('0.00', cdsTempItensLoteTOTALCUSTO.AsCurrency);
...
begin
  ...
  begin
    ...
    Application.MessageBox('Registro excluído com sucesso.',Pchar(UpperCase(Application.ExeName)),MB_ICONINFORMATION);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        self.CarregaCamposLote(StrToInt(lblNroLote.Caption));
        self.CarregarItensLote(StrToInt(lblNroLote.Caption));
    ...
    Application.MessageBox(Pchar('Lote não encontrado.' + #13 + 'Verificar inconsistência na Base de Dados.'),'Erro',MB_ICONERROR);
...
procedure TfrmLancLotes.CarregarItensLoteII(vIDLote: integer);
var
  vQuery : TZQuery;
begin
   vQuery := TZQuery.Create(nil);
...
begin
  if cdsTempItensLote.Active then
  begin
    if cdsTempItensLote.RecordCount > 0 then
      cdsTempItensLote.EmptyDataSet;
...
begin
  ...
  begin
    if not (FIDItem > 0) then
    begin
      if FIrec > 0 then
      begin
        cdsTempItensLote.RecNo := FIrec;
        cdsTempItensLote.Delete;
      end;

      cdsTempItensLote.Append;
      cdsTempItensLoteFABRICACAO.Value  := StrToDate(jvDataFabricaProd.Text);
      cdsTempItensLoteIDLOTE.Clear;
    end
    else
    begin
      cdsTempItensLote.edit;
      cdsTempItensLoteFABRICACAO.Value := FFabricaProd;
      cdsTempItensLoteIDLOTE.AsInteger := StrToInt(lblNroLote.Caption);
    end;

    cdsTempItensLoteID.AsInteger          := FIDItem;
...
procedure TfrmLancLotes.jvDataFabricaProdExit(Sender: TObject);
begin
  try
    StrToDate(jvDataFabricaProd.Text);
  except
    on EConvertError do
    begin
...
procedure TfrmLancLotes.jvDataFabricaProdKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
  begin
...
begin
  ...
  if ((tjDTValidade.Text = '  /  /    ') or (JvDTValidadeProduto.Text = '  /  /    ') or (jvDataFabricaProd.Text = '  /  /    ')) then
  begin
