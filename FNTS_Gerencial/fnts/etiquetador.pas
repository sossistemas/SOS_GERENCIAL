...
uses
  ...
  ExtCtrls, Mask, DBCtrls, wwdblook, StdCtrls, Wwdbdlg, dxCore,
  Grids, DBGrids, Db, Wwdatsrc, Wwdbigrd, Wwdbgrid, Buttons, ZAbstractRODataset,
  ZAbstractDataset, ZDataset, Collection, Menus, frxClass, frxDBSet, frxDesgn,
  frxBarcode, ComCtrls, wwdbedit, AdvGlowButton, RzEdit,
...
uses
  Principal, Modulo, xloc_produto_entrada, Math, etiqueta_grade, UFuncoes;
...
begin
  ...
  begin
    ...
    if ComboBox1.ItemIndex = 2 then
    begin
      fxproduto.LoadFromFile(ExtractFilePath(application.ExeName) + '\rel\f000077.fr3');
  ...
  begin
    ...
    begin
      ...
      DAT1 := copy(DateToStr(QRPRODUTO.FIELDBYNAME('ULTIMA_COMPRA').AsDateTime), 7, 4) + copy(DateToStr(QRPRODUTO.FIELDBYNAME('ULTIMA_COMPRA').AsDateTime), 4, 2) + copy(DateToStr(QRPRODUTO.FIELDBYNAME('ULTIMA_COMPRA').AsDateTime), 1, 2);
      QRPRODUTO.NEXT;
      qa := qa + 1;
      ...
      begin
        ...
        DAT2 := copy(DateToStr(QRPRODUTO.FIELDBYNAME('ULTIMA_COMPRA').AsDateTime), 7, 4) + copy(DateToStr(QRPRODUTO.FIELDBYNAME('ULTIMA_COMPRA').AsDateTime), 4, 2) + copy(DateToStr(QRPRODUTO.FIELDBYNAME('ULTIMA_COMPRA').AsDateTime), 1, 2);
        ...
        begin
          ...
          DAT3 := copy(DateToStr(QRPRODUTO.FIELDBYNAME('ULTIMA_COMPRA').AsDateTime), 7, 4) + copy(DateToStr(QRPRODUTO.FIELDBYNAME('ULTIMA_COMPRA').AsDateTime), 4, 2) + copy(DateToStr(QRPRODUTO.FIELDBYNAME('ULTIMA_COMPRA').AsDateTime), 1, 2);
...
begin
  ...
  begin
    ...
    begin
      qrgrade.close;
      qrgrade.SQL.clear;
      qrgrade.SQL.add('select * from c000021 where atb like :atb and codproduto = ''' + frmmodulo.QRPRODUTO.FIELDBYNAME('codigo').ASSTRING + ''' order by codigo');
...
begin
  ...
  begin
    ...
    query.SQL.add('c000088 e, c000025 p where e.atb like :atbe and p.atb like :atbp and e.notafiscal = ''' + resultado_pesquisa1 + ''' and');
    query.SQL.add('e.codproduto = p.codigo order by e.notafiscal');
    query.ParamByName('eatb').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
    query.ParamByName('patb').Value := ME_FiltraATB('TB_PRODUTO');
    query.open;
...
begin
  ...
  qrremarcados.SQL.add('where atb like :atb and precovenda_novo > ''0'' and');
  ...
  qrremarcados.ParamByName('atb').Value := ME_FiltraATB('TB_PRODUTO');
  qrremarcados.open;
