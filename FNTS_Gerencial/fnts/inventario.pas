...
uses
  ...
  Dialogs, Grids, Wwdbigrd, Wwdbgrid, AdvGlowButton, StdCtrls, Mask, RzEdit,
  RzSpnEdt, ExtCtrls, DB, ZAbstractRODataset, ZDataset,
  RzCmboBx, RzPrgres, wwdbedit, Wwkeycb, Menus, frxDesgn, frxClass, frxDBSet,
...
type
  TfrmInventario = class(TForm)
    ...
    qrgrupo_tipoTIPO: TWideStringField;
    ...
    dtfldDATA_POSTERIOR: TDateField;
...
uses
  modulo, principal, dados_inventario, inventario_ficha;
...
begin
  ...
  qrinventario.sql.add('where atb like :atb and ano = 99999');
...
begin
  ...
  qrinventario.sql.clear;
  qrinventario.sql.add('select * from inventario');
  qrinventario.sql.add('where atb like :atb and ano = ' + rano.text + ' and mes = ' + rmes.text);
...
procedure TfrmInventario.gridTitleButtonClick(Sender: TObject; AFieldName: string);
begin
...
procedure TfrmInventario.combo_estoqueKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    dateedit1.setfocus;
end;
...
begin
  ...
  begin
    qrinventario.fieldbyname('total').asfloat := qrinventario.fieldbyname('custo').asfloat * qrinventario.fieldbyname('estoque').asfloat;
  end;
...
begin
  rtotal.value := rtotal.value - qrinventario.fieldbyname('total').asfloat;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        qrinventario.close;
        qrinventario.sql.clear;
        qrinventario.sql.add('update inventario set estoque = 0, total = 0 where atb like :atb and ano = ' + ano + ' and mes = ' + mes);
        qrinventario.ParamByName('atb').Value := ME_FiltraATB('TB_INVENTARIO');
        qrinventario.execsql;
        frmmodulo.conexao.commit;
        bfiltrarClick(frminventario);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        qrinventario.close;
        qrinventario.sql.clear;
        qrinventario.sql.add('delete from inventario where atb like :atb and ano = ' + ano + ' and mes = ' + mes);
        qrinventario.ParamByName('atb').Value := ME_FiltraATB('TB_INVENTARIO');
        qrinventario.execsql;
        frmmodulo.conexao.commit;
        rtotal.value := 0;
...
begin
  ...
  qrgrupo_aliquota.SQL.ADD('FROM inventario WHERE atb like :atb and ANO = ' + QRINVENTARIO.FIELDBYNAME('ANO').ASSTRING + ' and mes = ' + QRINVENTARIO.FIELDBYNAME('mes').ASSTRING);
  qrgrupo_aliquota.SQL.ADD('GROUP BY TIPO, CST, ALIQUOTA');
  qrgrupo_aliquota.sql.add('order by TIPO, CST, ALIQUOTA');
  qrgrupo_aliquota.ParamByName('atb').Value := ME_FiltraATB('TB_INVENTARIO');
  qrgrupo_aliquota.OPEN;

  qrgrupo_tipo.CLOSE;
  qrgrupo_tipo.SQL.CLEAR;
  qrgrupo_tipo.SQL.ADD('SELECT SUM(total) TOTAL, TIPO');
  qrgrupo_tipo.SQL.ADD('FROM inventario WHERE atb like :atb and ano = ' + QRINVENTARIO.FIELDBYNAME('ANO').ASSTRING + ' and  mes = ' + QRINVENTARIO.FIELDBYNAME('mes').ASSTRING);
  qrgrupo_tipo.SQL.ADD('GROUP BY TIPO order by TIPO');
  qrgrupo_tipo.ParamByName('atb').Value := ME_FiltraATB('TB_INVENTARIO');
  qrgrupo_tipo.OPEN;

  frmdados_inventario := tfrmdados_inventario.create(self);
  frmdados_inventario.showmodal;

  if not cont_inventario then
    exit;

  fXproduto.LoadFromFile('\SOS\server\rel\f000126.fr3');
...
begin
  ...
  begin
    QRPRODUTO.CLOSE;
    QRPRODUTO.SQL.CLEAR;
    QRPRODUTO.SQL.ADD('SELECT codigo FROM C000025 WHERE atb like :atb and CODBARRA = ''' + ED_BARRA.TEXT + '''');
...
var
  pcusto: string;
begin
  if application.messagebox('Esta função edita todos os registros do inventário' + #13 + 'truncando o preço para 2 (duas) casas decimais!' + #13 + 'Faça uma cópia de segurança antes de prosseguir!' + #13 + 'Deseja continuar?', 'Atenção', mb_yesno + mb_iconwarning) = idyes then
  begin
    qrinventario.first;
    while not qrinventario.Eof do
    begin
      ...
      qrinventario.fieldbyname('total').asfloat := qrinventario.fieldbyname('estoque').asfloat * qrinventario.fieldbyname('custo').asfloat;
      qrinventario.Post;
      qrinventario.next;
    end;
    frmmodulo.conexao.commit;
...
procedure TfrmInventario.AdvMetroButton1Click(Sender: TObject);
begin
  close;
end;
