...
uses
  modulo, loc_funci, loc_regiao, xloc_cliente, principal;

{$R *.dfm}

procedure Tfrmlista_cliente2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
var
  cliente, VENDEDOR, regiao, tipo, situacao, setor, periodo, ordem, DIA, MES: string;
begin
  ...
  begin
    ...
    frmmodulo.qrrelatorio.fieldbyname('LINHA4').asstring := 'TÉCNICO: ' + combo_vendedor.Text;
  end;
  ...
  begin
    ...
    frmmodulo.qrrelatorio.fieldbyname('LINHA5').asstring := 'VEÍCULO ' + combo_regiao.Text + '   ';
  end;
  ...
  begin
    ...
    qrcliente.close;
    qrcliente.sql.clear;
    qrcliente.sql.Add('select * from c000007 where atb like :atb and codigo is not null ' + situacao + cliente + VENDEDOR + regiao + ' order by ' + combo_ordem.Text);
    ...
    fxcliente.LoadFromFile('\SOS\server\rel\f000001.fr3');
  ...
  begin
    ...
    fxcliente.LoadFromFile('\SOS\server\rel\f000002.fr3');
    fxcliente.ShowReport;
  ...
  begin
    qrcidade.close;
    qrcidade.sql.clear;
    qrcidade.sql.Add('SELECT CIDADE, COUNT(CODIGO) qtde_clientes FROM C000007 WHERE atb like :atb and codigo is not null  ' + situacao + cliente + VENDEDOR + regiao + ' GROUP BY CIDADE');
    qrcidade.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
    qrcidade.open;
    ...
    fxcliente.LoadFromFile('\SOS\server\rel\f000003.fr3');
    fxcliente.ShowReport;
    qrcliente.MasterSource := nil;
    qrcliente.MasterFields := '';
    qrcliente.IndexFieldNames := '';
  ...
  begin
    ...
    qrregiao.close;
    qrregiao.sql.clear;
    qrregiao.sql.Add('SELECT CODREGIAO, COUNT(CODIGO) TOTAL FROM C000007 WHERE atb like :atb and codigo is not null  ' + situacao + cliente + VENDEDOR + regiao + ' GROUP BY CODREGIAO');
    qrregiao.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
    qrregiao.open;
    ...
    fxcliente.LoadFromFile('\SOS\server\rel\f000004.fr3');
    fxcliente.ShowReport;
    qrcliente.MasterSource := nil;
    qrcliente.MasterFields := '';
    qrcliente.IndexFieldNames := '';
  ...
  begin
    ...
    fxcliente.LoadFromFile('\SOS\server\rel\f000006.fr3');
    fxcliente.ShowReport;
  end;
  ...
  begin
    ...
    begin
      qrcliente.sql.Add('select * from c000007 where atb like :atb and NASCIMENTO like ''' + combo_dia.Text + '/' + copy(combo_mes.Text, 1, 2) + '%'' ' + situacao + VENDEDOR + regiao + ' order by NASCIMENTO');
    end;
    if (combo_dia.Text <> 'TODOS') and (combo_mes.Text = '') then
    begin
      qrcliente.sql.Add('select * from c000007 where atb like :atb and NASCIMENTO like ''' + combo_dia.Text + '/%'' ' + situacao + VENDEDOR + regiao + ' order by NASCIMENTO');
    end;
    if (combo_dia.Text = 'TODOS') and (combo_mes.Text <> '') then
    begin
      qrcliente.sql.Add('select * from c000007 where atb like :atb and NASCIMENTO like ''%/' + copy(combo_mes.Text, 1, 2) + '/%'' ' + situacao + VENDEDOR + regiao + ' order by NASCIMENTO');
    end;
    qrcliente.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
    qrcliente.open;
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'ANIVERSARIANTES';
    fxcliente.LoadFromFile('\SOS\server\rel\f000005.fr3');
    fxcliente.ShowReport;
  end;
