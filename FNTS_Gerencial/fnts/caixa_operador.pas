...
begin
  ...
  if frmmodulo.qrcaixa_operadorCAIXA_GERAL.AsString = 'S' then begin
    ...
    if qrGeral.FieldByName('qtd').AsInteger > 0 then begin
      Application.MessageBox('Ja existe um caixa marcado como "Caixa Geral"!','Atenção',MB_ICONINFORMATION);
      Exit;
    end;
  end;
...
begin
  if (frmmodulo.qrcaixa_operador.State in [dsInsert, dsEdit]) then begin
    if frmmodulo.qrconfigCAIXA_AUTOMATICO_GERENCIAL.AsString = 'S' then begin
      Application.MessageBox('Inpossivel ativar esta opção sendo que está Ativo Fechamento e Abertura Automática dos Caixas do Gerencial nas configurações do sistema!','Atenção!',MB_ICONINFORMATION);
      frmmodulo.qrcaixa_operadorFECHAMENTO_CEGO.AsString := 'N';
    end;
  end;
end;
