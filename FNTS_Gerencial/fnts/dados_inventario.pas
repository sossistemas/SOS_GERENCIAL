...
uses
  ...
  Dialogs, StdCtrls, Mask, Menus, Buttons, ExtCtrls,
  AdvGlowButton, Vcl.Imaging.jpeg, AdvMetroButton,
  AdvSmoothExpanderPanel, JvExMask, JvToolEdit, AdvSmoothPanel;
...
begin
  ...
  if FileExists('\SOS\server\bd\eDI.inf') then
  begin
    assignfile(txt, '\SOS\server\bd\eDI.inf');
    reset(txt);
...
procedure Tfrmdados_inventario.AdvMetroButton1Click(Sender: TObject);
begin
  cont_inventario := false;
  close;
end;
...
begin
  ...
  frminventario.fXproduto.LoadFromFile('\SOS\server\rel\f000126.fr3');
