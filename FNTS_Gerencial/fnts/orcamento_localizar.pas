...
begin
  ...
  begin
    ...
    qrorcamento.sql.add('select * from c000056 where atb like :atb and upper(codigo) = ''' + ansiuppercase(Edit1.text) + ''' order by codigo');
  ...
  begin
    qrorcamento.close;
    qrorcamento.sql.clear;
    qrorcamento.sql.add('select * from c000056 where atb like :atb and codcliente = ''' + frmmodulo.qrcliente.fieldbyname('codigo').asstring + ''' order by codigo');
    qrorcamento.ParamByName('atb').Value := ME_FiltraATB('TB_ORCAMENTO');
    qrorcamento.open;
  end;
...
procedure TfrmOrcamento_localizar.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;

procedure TfrmOrcamento_localizar.wwDBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
begin
  ...
  begin
    ...
    qrorcamento.close;
    qrorcamento.sql.clear;
    qrorcamento.sql.add('select * from c000056 where atb like :atb and codcliente = :codcliente and GERA_NF = :GERA_NF order by codigo');
...
procedure TfrmOrcamento_localizar.comboclienteKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    BitBtn1Click(frmOrcamento_localizar);
end;
