...
procedure Tfrmxloc_cliente_veiculo.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;
...
procedure Tfrmxloc_cliente_veiculo.locKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = vk_down then
  begin
    grid.setfocus;
...
begin
  ...
  query.Close;
  query.SQL.clear;
  query.SQL.Add('select * from c000063 where atb like :atb and codigo Is NOT NULL ORDER BY VEICULO');
  query.ParamByName('atb').Value := ME_FiltraATB('TB_VEICULO_CLIENTE');
  query.open;
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000063 where atb like :atb and upper(' + tipo + ') like ''' + ref + loc.Text + '%'' order by ' + tipo);
    query.ParamByName('atb').Value := ME_FiltraATB('TB_VEICULO_CLIENTE');
    query.open;
  except
  end;
...
procedure Tfrmxloc_cliente_veiculo.ParmetrosdaPesquisa1Click(Sender: TObject);
begin
  LOC.SETFOCUS;
end;
