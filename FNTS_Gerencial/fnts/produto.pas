  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Application.MessageBox(PWideChar('Código informado novo volume já cadastrado pelo ' + qrduplo.FieldByName('tipo').AsString + qrduplo.FieldByName('produto').AsString + '"!'), 'Atenção!', MB_ICONINFORMATION);
          DBEdit1.SetFocus;
          Abort;
        end;
      end;
    ...
    try
      ...
      lRemarc := label7.Caption = 'Cód.Remarc.:';
      if lInsertMode then
      begin
        if lRemarc then
        begin
      ...
      if qrduplo.RECORDCOUNT > 0 then
      begin
        Application.MessageBox(PWideChar('Código informado para o poduto já cadastrado pelo ' + qrduplo.FieldByName('tipo').AsString + qrduplo.FieldByName('produto').AsString + '"!'), 'Atenção!', MB_ICONINFORMATION);
        DBEdit1.SetFocus;
        Abort;
      end;
      //
...
begin
  if DataSet.FieldByName('QTDE').AsFloat <= 0 then begin
    Application.MessageBox('Informe a quantidade para o volume!', 'Atenção!', MB_ICONINFORMATION);
    Abortar;
  end;
    ...
    if viCodigoBarraVolume <> DataSet.FieldByName('EANSKU').AsString then
      ...
      begin
        if qrduplo.FieldByName('ID').AsString  = '0' then
          Application.MessageBox(PWideChar('Código de barra em uso no ' + qrduplo.FieldByName('tipo').AsString +'de código :' + qrduplo.FieldByName('codigo').AsString + '!'), 'Atenção!', MB_ICONINFORMATION)
        else
          Application.MessageBox(PWideChar('Código de barra em uso no ' + qrduplo.FieldByName('tipo').AsString + 'de código :' + qrduplo.FieldByName('ID').AsString + ' pertencente ao Produto com código :' + qrduplo.FieldByName('codigo').AsString + '!'), 'Atenção!', MB_ICONINFORMATION);
        Abortar;
      end;
    end;

    if DataSet.FieldByName('PRECO').AsFloat <= 0 then begin
      Application.MessageBox('Informe o Preço de venda para o volume!', 'Atenção!', MB_ICONINFORMATION);
      Abortar;
    end;

    if DataSet.FieldByName('UNIDADE').AsString = '' then begin
      Application.MessageBox('Informe uma unidade de medida para o volume!', 'Atenção!', MB_ICONINFORMATION);
      Abortar;
    end;
    if DataSet.FieldByName('DESCRICAO').AsString = '' then begin
      Application.MessageBox('Informe uma descrição para o volume!', 'Atenção!', MB_ICONINFORMATION);
      Abortar;
    end;
...
begin
  ...
  begin
    ...
    lMsg := 'Excluir Composição de Volume';
  end
  else
  begin
    lZQry := zqryComposicaoCombo;
    lMsg := 'Excluir Composição do Combo';
  end;
