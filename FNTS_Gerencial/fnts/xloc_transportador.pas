...
procedure Tfrmxloc_transportador.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;
...
procedure Tfrmxloc_transportador.locKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = vk_down then
  begin
    grid.setfocus;
...
begin
  ...
  query.Close;
  query.SQL.clear;
  query.SQL.Add('select * from c000010 where atb like :atb and codigo = ''XXXXXX''');
  query.ParamByName('atb').Value := ME_FiltraATB('TB_TRANSPORTADOR');
  query.open;
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000010 where atb like :atb and upper(' + tipo + ') like ''' + ref + loc.Text + '%'' order by ' + tipo);
    query.ParamByName('atb').Value := ME_FiltraATB('TB_TRANSPORTADOR');
    query.open;
  except
  end;
...
procedure Tfrmxloc_transportador.ParmetrosdaPesquisa1Click(Sender: TObject);
begin
  LOC.SETFOCUS;
end;
