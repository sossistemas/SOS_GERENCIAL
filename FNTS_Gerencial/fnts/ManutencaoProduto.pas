...
uses
  ...
  Vcl.Grids, Vcl.DBGrids, JvExDBGrids, JvDBGrid, ZAbstractRODataset,
  ZAbstractDataset, ZDataset, AdvGlowButton, Vcl.Mask, RzEdit, RzBtnEdt,
  ZSqlUpdate, cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters,
  cxStyles, cxCustomData, cxFilter, cxData, cxDataStorage, cxEdit, cxNavigator,
  cxDBData, cxGridCustomTableView, cxGridTableView, cxGridDBTableView,
  cxGridLevel, cxClasses, cxGridCustomView, cxGrid, cxCalc, cxCurrencyEdit;
...
uses
  modulo, loc_grupo, loc_subgrupo, Principal, AjustaGradeProduto, UFuncoes;
...
begin
  ...
  qrProduto.SQL.Add('pro.precocusto,pro.precovenda, pro.estoqueminimo, pro.ativa, pro.usa_grade from c000025 pro inner join c000100 est on est.codproduto = pro.codigo where pro.atb like :atbpro and est.atb like :atbest');
...
begin
  ...
  begin
    ...
    if edGrupo.Text <> '000000' then
    begin
      if not frmprincipal.Locregistro(FRMMODULO.qrGRUPO, edGrupo.Text, 'codigo') then
      begin
        edGrupo.SetFocus;
        exit;
      end
...
begin
  if (edGrupo.Text = '') or (edGrupo.Text = '000000') then
  begin
  ...
  FRMMODULO.qrSUBGRUPO.sql.add('select * from c000018 where atb like :atb and codgrupo = ''' + edGrupo.Text + ''' order by SUBGRUPO');
  ...
  if not FRMMODULO.qrSUBGRUPO.IsEmpty then
  begin
...
begin
  ...
  begin
    ...
    FRMMODULO.qrSUBGRUPO.OPEN;
    FRMMODULO.qrSUBGRUPO.IndexFieldNames := 'SUBGRUPO';
    if edSubGrupo.Text <> '000000' then
    begin
      if not frmprincipal.Locregistro(FRMMODULO.qrSUBGRUPO, edSubGrupo.Text, 'codigo') then
      begin
        edSubGrupo.SetFocus;
        exit;
      end
...
procedure TfrmManutencaoProduto.edSubGrupoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    perform(wm_nextdlgctl, 0, 0);
end;

procedure TfrmManutencaoProduto.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if qrProduto.State = dsEdit then
    if Application.MessageBox('Existem alterações não salvas, Deseja Salvar?', 'Atenção!', MB_ICONQUESTION + MB_YESNO) = mrYes then
      qrProduto.Post
...
begin
  try
    if EstoqueAtual <> qrProdutoESTOQUE_ATUAL.AsFloat then
    begin
      ...
      frmmodulo.qrproduto_mov.SQL.add('ecf_serie,ecf_caixa,codcliente,codvendedor,movimento,data,cst, atb)');
      ...
      frmmodulo.qrproduto_mov.SQL.add(''''','''',''000000'',''S/VEND'',10,:datax,'+QuotedStr('000')+',:atb)');
      ...
      FRMMODULO.QRPRODUTO_MOV.CLOSE;
      FRMMODULO.QRPRODUTO_MOV.SQL.CLEAR;
      FRMMODULO.QRPRODUTO_MOV.SQL.Add('select * from c000032 where atb like :atb');
      qryTmp.SQL.Clear;
      qryTmp.SQL.Add('update c000100 set');
      qryTmp.SQL.Add('estoque_atual = ' + ConValor(qrProdutoESTOQUE_ATUAL.AsFloat));
      qryTmp.SQL.Add('where atb like :atb and codproduto = ' + QuotedStr(qrProdutoCODIGO.AsString));
      qryTmp.SQL.Add('and codfilial = ' + QuotedStr(frmmodulo.qrFilialCODIGO.AsString));
      qryTmp.ParamByName('atb').Value := ME_FiltraATB('TB_ESTOQUE');
      qryTmp.ExecSQL;
    end;
