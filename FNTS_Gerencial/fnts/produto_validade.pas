...
uses
  ...
  ZAbstractDataset, ZDataset, RzButton, RzRadChk, RzPanel, PageView, frxClass, frxDBSet, frxDesgn,
  JvExMask, JvToolEdit;
...
type
  Tfrmproduto_validade = class(TForm)
    ...
    qrvalidadeCODPRODUTO: TWideStringField;
    qrvalidadeDATA: TDateTimeField;
    qrvalidadeLOTE_FABRICACAO: TWideStringField;
...
begin
  ...
  begin
    fXproduto.LoadFromFile('\SOS\server\rel\f000205.fr3');
    fxproduto.ShowReport;
  end;
  if pagina_principal.ActivePageIndex = 1 then
  begin
    fXproduto.LoadFromFile('\SOS\server\rel\f000206.fr3');
    fxproduto.ShowReport;
  end;
