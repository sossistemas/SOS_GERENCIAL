...
uses
  ...
  DB, ZAbstractRODataset, ZAbstractDataset, ZDataset, JvExMask, JvToolEdit,
  JvBaseEdits;
...
uses
  convenio, modulo, principal, troco, contasreceber_banco, // temp,
...
begin
  ...
  try
    ...
    qrcontasreceber_pgto.sql.add('select * from c000050 where atb like :atb and codcliente = ''xx11xx''');
    ...
    begin
      ...
      begin
        ...
        qrpgto.sql.add('where atb like :atb and ');
        ...
        qrpgto.ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_RECEBER');
        qrpgto.OPEN;
        ...
        begin
          ...
          begin
            ...
            if comboforma.Text = 'DINHEIRO' then
              FieldByName('dinheiro').ASfloat := qrpgto.FieldByName('valor_atual').ASfloat;
            if comboforma.Text = 'CHEQUE A VISTA' then
              FieldByName('chequeav').ASfloat := qrpgto.FieldByName('valor_atual').ASfloat;
            if comboforma.Text = 'CHEQUE A PRAZO' then
              FieldByName('chequeap').ASfloat := qrpgto.FieldByName('valor_atual').ASfloat;
            if comboforma.Text = 'CARTAO' then
              FieldByName('cartao').ASfloat := qrpgto.FieldByName('valor_atual').ASfloat;
            if comboforma.Text = 'DEPOSITO' then
              FieldByName('boleto').ASfloat := qrpgto.FieldByName('valor_atual').ASfloat;
            ...
            FieldByName('atb').AsString := ME_GravaATB('TB_CONTA_RECEBER_PGTO');
            post;
          end;
...
procedure Tfrmconvenio_pgto.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
