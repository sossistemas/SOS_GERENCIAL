...
uses
  ...
  ZDataset, DBCtrls, Buttons, AdvGlowButton, RzPanel,
  dxGDIPlusClasses, AdvMetroButton, AdvSmoothExpanderPanel,
  JvExMask, JvToolEdit, JvBaseEdits, JvDBControls, ZAbstractDataset,
  AdvSmoothPanel;
...
type
  Tfrmproduto_consultapreco = class(TForm)
    ...
    queryIND_CFOP_NFCE: TWideStringField;
    queryFOTOBD: TBlobField;
    queryID_TIPO_SERVICO: TIntegerField;
    queryNAO_MOVIMENTA_ESTOQUE: TWideStringField;
    queryPESAGEM_AUOTMATICA: TWideStringField;
    queryINFORMA_CODIGO_BARRA_XML: TWideStringField;
    queryCODIGO_ANP: TWideStringField;
    queryCOMBO: TWideStringField;
    queryIND_CFOP_DEV_COMPRA_DENTRO: TWideStringField;
    queryIND_CFOP_DEV_COMPRA_FORA: TWideStringField;
    queryPIZZA: TWideStringField;
    queryPERCGLNN: TFloatField;
    queryPERCGLGNI: TFloatField;
    queryPGLP: TFloatField;
    queryVPART: TFloatField;
    queryECF_ICMS: TWideStringField;
...
uses
  modulo, produto_foto, xloc_servico, tabela_preco;
...
procedure Tfrmproduto_consultapreco.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;
...
procedure Tfrmproduto_consultapreco.locKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = vk_down then
  begin
  ...
  begin
    ...
    begin
      ...
      try
        ...
        query.sql.clear;
        if loc.text = '' then
          query.sql.Add('select prod.*, estq.* from c000025 prod, c000100 estq where prod.atb like :atbprod and estq.atb like :atbestq and situacao <> 1 and prod.codigo = estq.codproduto and prod.codigo is null')
        else if loc.text = '*' then
          query.sql.Add('select prod.*, estq.* from c000025 prod, c000100 estq where prod.atb like :atbprod and estq.atb like :atbestq and situacao <> 1 and  prod.codigo = estq.codproduto order by ' + tipo)
        else
        begin
          query.sql.add('select prod.*, estq.* from c000025 prod, c000100 estq where prod.atb like :atbprod and estq.atb like :atbestq and situacao <> 1 and prod.codigo = estq.codproduto');
    ...
    begin
      ...
      begin
        ...
        begin
          query.close;
          query.sql.clear;
          query.sql.Add('select prod.*, estq.* from c000025 prod, c000100 estq where prod.atb like :atbprod and estq.atb like :atbestq and situacao <> 1 and prod.codigo = estq.codproduto and CODFORNECEDOR = ''' + query2.fieldbyname('codigo').asstring + ''' order by produto');
          query.ParamByName('atbprod').Value := ME_FiltraATB('TB_PRODUTO');
          query.ParamByName('atbestq').Value := ME_FiltraATB('TB_ESTOQUE');
          query.open;
        ...
        begin
          query.close;
          query.sql.clear;
          query.sql.Add('select prod.*, estq.* from c000025 prod, c000100 estq where prod.atb like :atbprod and estq.atb like :atbestq and situacao <> 1 and prod.codigo = estq.codproduto and CODmarca = ''' + query2.fieldbyname('codigo').asstring + ''' order by produto');
          query.ParamByName('atbprod').Value := ME_FiltraATB('TB_PRODUTO');
          query.ParamByName('atbestq').Value := ME_FiltraATB('TB_ESTOQUE');
          query.open;
          pprocura.Visible := false;
...
begin
  ...
  query.Close;
  query.SQL.clear;
  query.SQL.Add('select prod.*, estq.* from c000025 prod, c000100 estq where prod.atb like :atbprod and estq.atb like :atbestq and situacao <> 1 and prod.codigo = estq.codproduto and codigo is null');
  query.ParamByName('atbprod').Value := ME_FiltraATB('TB_PRODUTO');
  query.ParamByName('atbestq').Value := ME_FiltraATB('TB_ESTOQUE');
  query.open;
  loc.setfocus;
...
begin
  if QUERY.FIELDBYNAME('foto').ASSTRING <> '' then
    bfoto.enabled := true
  else
    bfoto.enabled := false;
  voltar := true;
...
procedure Tfrmproduto_consultapreco.wwDBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    locKeyPress(frmproduto_consultapreco, Key);
  loc.setfocus;
end;
...
begin
  if QUERY.FIELDBYNAME('foto').ASSTRING <> '' then
    ...
    bfoto.enabled := false;
  AdvGlowButton4.Enabled := QUERY.FIELDBYNAME('USA_TB_PC').AsString = 'SIM';
end;

procedure Tfrmproduto_consultapreco.FotodoProduto1Click(Sender: TObject);
begin
  if bfoto.enabled then
    bfotoclick(frmproduto_consultapreco);
end;

procedure Tfrmproduto_consultapreco.AdvGlowButton4Click(Sender: TObject);
begin
  frmmodulo.qrTabelaPreco.Close;
  frmmodulo.qrTabelaPreco.Filtered := False;
  frmmodulo.qrTabelaPreco.Params.Items[0].Value := query.FieldByName('codigo').AsString;
  frmmodulo.qrTabelaPreco.Open;
  SelecionaPreco;
end;
...
procedure Tfrmproduto_consultapreco.dsStateChange(Sender: TObject);
begin
