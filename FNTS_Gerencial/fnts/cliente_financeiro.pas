...
(70 % do lucro obtido).
Classe B: Clientes que possuem um lucro intermediário
dentro do período selecionado(20 % do lucro obtido).
Classe C: Clientes que possuem um baixo lucro dentro do período selecionado
(10 % do lucro obtido).
}
...
uses
  ...
  ZDataset, Grids, Wwdbigrd, Wwdbgrid, Collection, Buttons,
  ImgList, Mask, Menus, ComCtrls, JvSpin, PageView, Wwkeycb, frxDesgn, frxClass,
  frxDBSet, AdvGlowButton,
...
type
  Tfrmcliente_financeiro = class(TForm)
    ...
    procedure wwDBGrid3DrawDataCell(Sender: TObject; const Rect: TRect; Field: TField; State: TGridDrawState);
...
uses
  modulo, principal, contasreceber_juros, loc_regiao, lista_cliente_financeiro,
  xloc_cliente;
...
begin
  ...
  qrresumo_geralTotal.DisplayFormat := mascara_valor;
  qrresumo_geralCHEQUE.DisplayFormat := mascara_valor;
  qrresumo_geralFINANCEIRA.DisplayFormat := mascara_valor;
  qrcontasreceber.close;
end;

procedure Tfrmcliente_financeiro.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  begin
    ...
    begin
      qrcontasreceber.SQL.add('select cr.*, cliente.nome, cliente.codigo from c000049 cr, c000007 cliente where cr.atb like :atbcr and  cliente.atb like :atbcliente and cr.codcliente = cliente.codigo and cr.data_vencimento <= :datai and cr.situacao = 1 and valor_atual > 0 order by ' + ordem);
      qrcontasreceber.Params.ParamByName('datai').asdatetime := (date - rdias.value);
    end;
    if rperiodo.checked then
    begin
      qrcontasreceber.SQL.add('select cr.*, cliente.nome, cliente.codigo from c000049 cr, c000007 cliente where cr.atb like :atbcr and  cliente.atb like :atbcliente ');
      qrcontasreceber.SQL.add('and cr.cr.codcliente = cliente.codigo and cr.data_vencimento >= :datai and cr.data_vencimento <= :dataf and cr.situacao = 1 and valor_atual > 0 order by ' + ordem);
  ...
  begin
    if ratrazo.checked then
    begin
      qrcontasreceber.SQL.add('select cr.*, cliente.nome, cliente.codigo from c000049 cr, c000007 cliente where cr.atb like :atbcr and  cliente.atb like :atbcliente and cr.cr.codcliente = cliente.codigo and cr.codcliente = ''' + ecodigo.text + ''' and cr.data_vencimento <= :datai and cr.situacao = 1 and valor_atual > 0 order by ' + ordem);
      qrcontasreceber.Params.ParamByName('datai').asdatetime := (date - rdias.value);
    end;
    if rperiodo.checked then
    begin
      qrcontasreceber.SQL.add('select cr.*, cliente.nome, cliente.codigo from c000049 cr, c000007 cliente where cr.atb like :atbcr and  cliente.atb like :atbcliente and cr.cr.codcliente = cliente.codigo and cr.codcliente = ''' + ecodigo.text + ''' and cr.data_vencimento >= :datai and cr.data_vencimento <= :dataf and cr.situacao = 1 and valor_atual > 0 order by ' + ordem);
      qrcontasreceber.Params.ParamByName('datai').asdatetime := DateEdit1.date;
      qrcontasreceber.Params.ParamByName('dataf').asdatetime := DateEdit2.date;
    end;
  end;
  ...
  begin
    if ratrazo.checked then
    begin
      qrcontasreceber.SQL.add('select cr.*, cliente.nome, cliente.codigo from c000049 cr, c000007 cliente where cr.atb like :atbcr and  cliente.atb like :atbcliente and cr.cr.codcliente = cliente.codigo and cliente.codregiao = ''' + ecodigo.text + ''' and cr.data_vencimento <= :datai and cr.situacao = 1 and valor_atual > 0 order by ' + ordem);
      qrcontasreceber.Params.ParamByName('datai').asdatetime := (date - rdias.value);
    end;
    if rperiodo.checked then
    begin
      qrcontasreceber.SQL.add('select cr.*, cliente.nome, cliente.codigo from c000049 cr, c000007 cliente where cr.atb like :atbcr and  cliente.atb like :atbcliente and cr.cr.codcliente = cliente.codigo and cliente.codregiao = ''' + ecodigo.text + ''' and cr.data_vencimento >= :datai and cr.data_vencimento <= :dataf and cr.situacao = 1 and valor_atual > 0 order by ' + ordem);
      qrcontasreceber.Params.ParamByName('datai').asdatetime := DateEdit1.date;
      qrcontasreceber.Params.ParamByName('dataf').asdatetime := DateEdit2.date;
    end;
...
procedure Tfrmcliente_financeiro.UpDown1Click(Sender: TObject; Button: TUDBtnType);
begin
...
procedure Tfrmcliente_financeiro.DateEdit1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    perform(wm_nextdlgctl, 0, 0);
end;

procedure Tfrmcliente_financeiro.DateEdit2KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bfiltro.SetFocus;
end;
...
procedure Tfrmcliente_financeiro.ecodigoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    perform(wm_nextdlgctl, 0, 0);
end;
...
begin
  ...
  begin
    if application.messagebox('Tem certeza que deseja bloquear os clientes de todas as prestações selecionadas?', 'Atenção', mb_yesno + MB_ICONWARNING + MB_DEFBUTTON2) = idyes then
    begin
      ...
      begin
        ...
        frmmodulo.qrcliente.close;
        frmmodulo.qrcliente.SQL.clear;
        frmmodulo.qrcliente.SQL.add('update c000007 set situacao = 3 where atb like :atb and codigo = ''' + qrcontasreceber.fieldbyname('codcliente').asstring + '''');
        frmmodulo.qrcliente.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
        frmmodulo.qrcliente.ExecSQL;
        frmPrincipal.logUC('Bloqueou Clientes: ' + qrcontasreceber.fieldbyname('codcliente').asstring, 3);
...
begin
  ...
  begin
    if application.messagebox('Tem certeza que deseja colocar observações nos clientes de todas as prestações selecionadas?', 'Atenção', mb_yesno + MB_ICONWARNING + MB_DEFBUTTON2) = idyes then
    begin
      qrcontasreceber.first;
      while not qrcontasreceber.eof do
      begin
        ...
        frmmodulo.qrcliente.close;
        frmmodulo.qrcliente.SQL.clear;
        frmmodulo.qrcliente.SQL.add('update c000007 set situacao = 2, obs4 = ''' + 'Cliente com prestações em Atrazo!' + ''' where atb like :atb and codigo = ''' + qrcontasreceber.fieldbyname('codcliente').asstring + '''');
         frmmodulo.qrcliente.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
        frmmodulo.qrcliente.ExecSQL;
        frmPrincipal.logUC('Colocou Observacoes Clientes: ' + qrcontasreceber.fieldbyname('codcliente').asstring, 3);
...
begin
  ...
  begin
    if application.messagebox('Tem certeza que deseja desbloquear os clientes de todas as prestações selecionadas?', 'Atenção', mb_yesno + MB_ICONWARNING + MB_DEFBUTTON2) = idyes then
    begin
      qrcontasreceber.first;
      while not qrcontasreceber.eof do
      begin
        frmmodulo.qrcliente.close;
        frmmodulo.qrcliente.SQL.clear;
        frmmodulo.qrcliente.SQL.add('update c000007 set situacao = 1 where atb like :atb and codigo = ''' + qrcontasreceber.fieldbyname('codcliente').asstring + '''');
        frmmodulo.qrcliente.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
        frmmodulo.qrcliente.ExecSQL;

        frmPrincipal.logUC('Desbloqueou Clientes: ' + qrcontasreceber.fieldbyname('codcliente').asstring, 3);
...
begin
  ...
  begin
    if application.messagebox('Tem certeza que deseja tirar a observação dos clientes de todas as prestações selecionadas?', 'Atenção', mb_yesno + MB_ICONWARNING + MB_DEFBUTTON2) = idyes then
    begin
      qrcontasreceber.first;
      while not qrcontasreceber.eof do
      begin
        frmmodulo.qrcliente.close;
        frmmodulo.qrcliente.SQL.clear;
        frmmodulo.qrcliente.SQL.add('update c000007 set situacao = 1, obs4 = ''' + '' + ''' where atb like :atb and codigo = ''' + qrcontasreceber.fieldbyname('codcliente').asstring + '''');
        frmmodulo.qrcliente.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
        frmmodulo.qrcliente.ExecSQL;

        frmPrincipal.logUC('Tirou Observacoes Clientes: ' + qrcontasreceber.fieldbyname('codcliente').asstring, 3);
...
procedure Tfrmcliente_financeiro.DateEdit4KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bfiltro2.SetFocus;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      qrresumo.SQL.add('jOIN c000049 cr on cliente.codigo = cr.codcliente  and cr.data_vencimento <= :datai and cr.situacao = 1 and valor_atual > 0');
      qrresumo.SQL.add('and cliente.codigo is not null where cr.atb like :atbcr and cliente.atb like :atbcliente group by cliente.codigo, cliente.nome, cliente.situacao');
      qrresumo.SQL.add('order by cliente.nome');
      qrresumo.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
    ...
    begin
      qrresumo.SQL.add('select cliente.nome, cliente.codigo,cliente.situacao, sum(cr.valor_atual) total from c000007 cliente');
      qrresumo.SQL.add('jOIN c000049 cr on cliente.codigo = cr.codcliente  and cr.data_vencimento >= :datai and cr.data_vencimento <= :dataf ');
      qrresumo.SQL.add('and cr.situacao = 1 and valor_atual > 0 and cliente.codigo is not null where cr.atb like :atbcr and cliente.atb like :atbcliente group by cliente.codigo, cliente.nome, cliente.situacao');
      qrresumo.SQL.add('where cr.atb like :atbcr and cliente.atb like :atbcliente order by cliente.nome');
  ...
  begin
    ...
    begin
      qrresumo.SQL.add('select cliente.nome, cliente.codigo,cliente.situacao, sum(cr.valor_atual) total from c000007 cliente');
      qrresumo.SQL.add('jOIN c000049 cr on cliente.codigo = cr.codcliente  and cr.codcliente = ''' + ECODIGO2.text + ''' and cr.data_vencimento <= :datai and cr.situacao = 1 and valor_atual > 0 and cliente.codigo is not null where cr.atb like :atbcr and cliente.atb like :atbcliente group by cliente.codigo, cliente.nome, cliente.situacao');
      qrresumo.SQL.add('order by cliente.nome');
      qrresumo.Params.ParamByName('datai').asdatetime := (date - RDIAS2.value);
    end;
    if RPERIODO2.checked then
  ...
  begin
    ...
    begin
      qrresumo.SQL.add('select cliente.nome, cliente.codigo, cliente.situacao, sum(cr.valor_atual) total from c000007 cliente');
      qrresumo.SQL.add('jOIN c000049 cr on cliente.codigo = cr.codcliente  and cliente.codregiao = ''' + ECODIGO2.text + ''' and cr.data_vencimento <= :datai and cr.situacao = 1 and valor_atual > 0 and cliente.codigo is not null where cr.atb like :atbcr and cliente.atb like :atbcliente group by cliente.codigo, cliente.nome, cliente.situacao');
      qrresumo.SQL.add('order by cliente.nome');
      qrresumo.Params.ParamByName('datai').asdatetime := (date - RDIAS2.value);
    end;
    if RPERIODO2.checked then
  ...
  qrresumo.ParamByName('atbcr').Value := ME_FiltraATB('TB_CONTA_RECEBER');
  qrresumo.ParamByName('atbcliente').Value := ME_FiltraATB('TB_CLIENTE');
  qrresumo.Open;
...
begin
  ...
  begin
    ...
    qrresumo.SQL.add('jOIN c000049 cr on cliente.codigo = cr.codcliente  and cr.data_vencimento <= :datai and cr.situacao = 1 where cr.atb like :atbcr and cliente.atb like :atbcliente group by cliente.codigo, cliente.nome, cliente.situacao');
    qrresumo.SQL.add('order by cliente.nome');
...
procedure Tfrmcliente_financeiro.UpDown2Click(Sender: TObject; Button: TUDBtnType);
begin
...
begin
  ...
  if RATRAZO2.checked then
  begin
    query.SQL.add('select * from c000049 where atb like :atb and codcliente = ''' + qrresumoCODIGO.value + ''' and data_vencimento <= :datai and situacao = 1 and valor_atual > 0');
    query.Params.ParamByName('datai').asdatetime := (date - rdias.value);
  end;
  if RPERIODO2.checked then
  begin
    query.SQL.add('select * from c000049 where atb like :atb and codcliente = ''' + qrresumoCODIGO.value + ''' and data_vencimento >= :datai and data_vencimento <= :dataf and situacao = 1 and valor_atual > 0');
...
procedure Tfrmcliente_financeiro.DateEdit6KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bfiltro3.SetFocus;
end;
...
procedure Tfrmcliente_financeiro.wwDBGrid3DrawDataCell(Sender: TObject; const Rect: TRect; Field: TField; State: TGridDrawState);
begin
...
begin
  ...
  QUERY2.close;
  QUERY2.SQL.clear;
  QUERY2.SQL.add('SELECT SUM(TOTAL) TOTAL FROM C000048 WHERE atb like :atb and data >= :datai and data <= :dataf');
  QUERY2.Params.ParamByName('datai').asdatetime := DateEdit5.date;
  QUERY2.Params.ParamByName('dataf').asdatetime := DateEdit6.date;
  QUERY2.ParamByName('atb').Value := ME_FiltraATB('TB_VENDAS');
  QUERY2.Open;
  ...
  begin
    ...
    qrvenda.SQL.add('jOIN c000048 cr on cliente.codigo = cr.codcliente and cr.data >= :datai and cr.data <= :dataf where cr.atb like :atbcr and cliente.atb like :atbcliente group by cliente.codigo, cliente.nome, cliente.situacao');
  ...
  begin
    qrvenda.SQL.add('select cliente.situacao, cliente.nome, cliente.codigo, sum(cr.TOTAL) total from c000007 cliente');
    qrvenda.SQL.add('jOIN c000048 cr on cliente.codigo = cr.codcliente and  cr.data >= :datai and cr.data <= :dataf and cliente.codregiao = ''' + ecodigo3.text + ''' and cr.data <= :dataf where cr.atb like :atbcr and cliente.atb like :atbcliente group by cliente.codigo, cliente.nome, cliente.situacao');
    qrvenda.SQL.add('order by cliente.nome');
    qrvenda.Params.ParamByName('datai').asdatetime := DateEdit5.date;
    qrvenda.Params.ParamByName('dataf').asdatetime := DateEdit6.date;
  end;
  qrvenda.ParamByName('atbcr').Value := ME_FiltraATB('TB_CONTA_RECEBER');
  qrvenda.ParamByName('atbcliente').Value := ME_FiltraATB('TB_CLIENTE');
  qrvenda.Open;
  qrvenda.first;
...
begin
  ...
  qrinativo.SQL.add('from c000007 where atb like :atb and situacao < 3 ');
  qrinativo.SQL.add('and DATA_ULTIMACOMPRA < :datax ');
  qrinativo.SQL.add('group by codigo, nome, situacao, data_ultimacompra order by nome');
  qrinativo.Params.ParamByName('datax').asdatetime := date - rdias_inativo.value;
  qrinativo.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
  qrinativo.Open;
end;

procedure Tfrmcliente_financeiro.rdias_inativoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bfiltro5.SetFocus;
end;
...
begin
  ...
  begin
    ...
    qrresumo_geral.SQL.add('where cr.atb like :atbcr and cliente.atb like :atbcliente and');
    ...
    qrresumo_geral.SQL.add('where ch.atb like :atbch and cliente.atb like :atbcliente');
    qrresumo_geral.SQL.add('and cliente.situacao < 3');
    ...
    qrresumo_geral.SQL.add('where fi.atb like :atbfi and cliente.atb like :atbcliente');
    qrresumo_geral.SQL.add('and cliente.situacao < 3');
    ...
    qrresumo_geral.ParamByName('atbcliente').Value := ME_FiltraATB('TB_CLIENTE');
    qrresumo_geral.ParamByName('atbcr').Value := ME_FiltraATB('TB_CONTA_RECEBER');
    qrresumo_geral.ParamByName('atbch').Value := ME_FiltraATB('TB_CHEQUE');
    qrresumo_geral.ParamByName('atbfi').Value := ME_FiltraATB('TB_FINANCEIRA_LANCAMENTO');
...
begin
  qrresumo_geralTotal.asfloat := qrresumo_geralCREDIARIO.asfloat + qrresumo_geralCHEQUE.asfloat + qrresumo_geralFINANCEIRA.asfloat;
end;
