...
uses
  ...
  Buttons, Mask, DB, ZAbstractRODataset, ZAbstractDataset,
  ZDataset, JvExMask, JvToolEdit;
...
begin
  ...
  begin
    ...
    begin
      GROUPBOX2.Caption := 'Aguarde... Preparando o arquivo';
      Application.ProcessMessages;
      ...
      begin
        edestino.Text := OpenDialog1.FileName;
      end;
      ...
      begin
        AssignFile(arquivo, 'C:\SOS\server\Logs\ERROBARRAS' + ExtractFileName(OpenDialog1.FileName));
        Rewrite(arquivo)
      ...
      begin
        try
          ...
          posicao := Pos(',', Memo1.Lines[x]);
          qtde_txt := StrToInt(Copy(Memo1.Lines[x], posicao + 1, 4));
          texto := TrimLeft(TrimRight(Copy(Memo1.Lines[x], 1, posicao - 1)));
          if frmmodulo.qrproduto.Locate('PRODUTO', texto, []) then
          begin
            LITEM.Caption := 'Atualizando C.Barras  ' + texto;
            if query.Locate('CODPRODUTO', frmmodulo.qrproduto.fieldbyname('CODIGO').AsString, []) then
            begin
              query.Edit;
              query.Fields.FieldByName('ESTOQUE_ATUAL').Value := query.Fields.FieldByName('ESTOQUE_ATUAL').Value + qtde_txt;
              query.Post;
            end
            else
              ShowMessage('Produto : ' + frmmodulo.qrproduto.FieldByName('PRODUTO').AsString + #13 + 'nao possui referênica na tabela C000100');
          end
          else
          begin
            //ShowMessage('Código de Barras :  ' + texto + '  Não Encontrado');
      ...
      edestino.Text := '';
      edestino.Text := 'C:\SOS\server\Logs\ERROBARRAS' + ExtractFileName(OpenDialog1.FileName);
      CloseFile(arquivo);
      Memo1.Text := '';
      Memo1.Lines.LoadFromFile(edestino.text);
      if Memo1.Text = '' then
