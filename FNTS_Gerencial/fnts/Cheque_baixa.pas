...
uses
  ...
  TFlatPanelUnit, Menus, AdvGlowButton, AdvMetroButton,
...
uses
  modulo, principal, loc_contacorrent, contacorrente_lancamento2, cheque_menu;
...
procedure Tfrmcheque_baixa.DBDateEdit3KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
...
begin
  if (rdesfazer.Checked) or (r1dep.checked) or (r2dep.Checked) or (rdesc.checked) or (r1dev.Checked) or (r2dev.Checked) or (rrep.Checked) or (rsust.Checked) or (rout.Checked) then
  begin
...
begin
  ...
  try
    ...
    begin
      frmcheque_menu.qrcheque.FIELDBYNAME('LIQUIDO').ASFLOAT := frmcheque_menu.qrcheque.FIELDBYNAME('VALOR').ASFLOAT - frmcheque_menu.qrcheque.FIELDBYNAME('DESCONTO').ASFLOAT;
    end;
  except
  end;
end;
...
procedure Tfrmcheque_baixa.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
