...
uses
  ...
  DBCtrls, DB, Wwdatsrc, Menus, AdvGlowButton, AdvMetroButton,
  AdvSmoothExpanderPanel, JvExMask, JvToolEdit, JvDBControls, AdvSmoothPanel;
...
uses
  modulo, principal, contacorrente, loc_conta;

{$R *.dfm}

procedure Tfrmcontacorrente_lancamento.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;

procedure Tfrmcontacorrente_lancamento.ECONTAKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bgravar.setfocus;
end;
...
begin
  ...
  begin
    frmcontacorrente.qrlancamento_conta.FieldByName('valor').asfloat := frmcontacorrente.qrlancamento_conta.FieldByName('valor').asfloat * (-1);
  end;
  ...
  begin
    ...
    frmcontacorrente.qrlancamento_conta.fieldbyname('ATB').asstring := ME_GravaATB('TB_CONTA_BANCARIA');
    enomeconta.text := '';
...
procedure Tfrmcontacorrente_lancamento.edata_cadastroEnter(Sender: TObject);
begin
  TEDIT(Sender).COLOR := $00A0FAF8;
...
procedure Tfrmcontacorrente_lancamento.edata_cadastroKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
