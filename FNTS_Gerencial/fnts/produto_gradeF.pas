...
uses
  modulo, principal, compra_menu, compra_item;
...
begin
  ...
  qrgrade_entrada.SQL.add('select * from c000033 where atb like :atb and codITEM = ''' + codigo_it + ''' order by codigo');
  ...
  begin
    ...
    begin
      ...
      qrgrade_entrada.fieldbyname('atb').Value := ME_GravaATB('TB_NOTA_ENTRADA_GRADE');
...
begin
  ...
  begin
    ...
    frmmodulo.qrgrade_produto.fieldbyname('estoque').asfloat := frmmodulo.qrgrade_produto.fieldbyname('estoque').asfloat + qrgrade_entrada.fieldbyname('qtde').asfloat - qtde_anterior;
