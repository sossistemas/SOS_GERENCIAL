...
uses
  ...
  Dialogs, StdCtrls, wwdblook, Mask, DBCtrls, ExtCtrls, Buttons,
  TFlatPanelUnit, Menus, DB, ZAbstractRODataset, ZDataset,
  AdvGlowButton, RzPanel, AdvMetroButton, AdvSmoothExpanderPanel,
  JvBaseEdits, JvDBControls, JvExMask, JvToolEdit, ZAbstractDataset,
  AdvSmoothPanel;
...
uses
  modulo, contasreceber, loc_funci, principal, xloc_cliente;
...
procedure Tfrmcontasreceber_ficha.bcancelar1Click(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    query.close;
    query.sql.clear;
    query.sql.add('select SUM(VALOR_ATUAL) TOTAL_CLIENTE from c000049 where atb like :atb and codcliente = ''' + RESULTADO_PESQUISA1 + ''' and situacao = 1');
...
procedure Tfrmcontasreceber_ficha.evendedorKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bgravar.setfocus;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      query.close;
      query.sql.clear;
      query.sql.add('select SUM(VALOR_ATUAL) TOTAL_CLIENTE from c000049 where atb like :atb and codcliente = ''' + ecliente.Text + ''' and situacao = 1');
      query.ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_RECEBER');
      query.Open;
...
procedure Tfrmcontasreceber_ficha.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
