...
uses
  ...
  Dialogs, ExtCtrls, StdCtrls, Mask, Collection,
  Buttons, DB, ZAbstractRODataset, ZDataset, frxClass,
  frxDBSet, frxDesgn, Menus, AdvGlowButton, ZAbstractDataset;
...
begin
  ...
  begin
    ...
    FXNOTA.LoadFromFile('\SOS\server\rel\f000094.fr3');
    FXNOTA.ShowReport;
  end;
  ...
  begin
    ...
    begin
      ...
      qrnota.SQL.ADD('SELECT * FROM SINTEGRA_ENTRADA WHERE atb like :atb ORDER BY CFOP,NOTAFISCAL');
      qrnota.ParamByName('atb').Value := ME_FiltraATB('TB_SINTEGRA_ENTRADAS');
    end
    else
    begin
      frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELAÇÃO DE NOTA FISCAL DE EMITIDA POR CFOP';
      qrnota.SQL.ADD('SELECT * FROM SINTEGRA_SAIDA WHERE atb like :atb ORDER BY CFOP,NOTAFISCAL');
      qrnota.ParamByName('atb').Value := ME_FiltraATB('TB_SINTEGRA_SAIDAS');
    end;
    qrnota.open;
    FXNOTA.LoadFromFile('\SOS\server\rel\f000095.fr3');
    FXNOTA.ShowReport;
  end;
  ...
  begin
    ...
    begin
      ...
      qrnota.SQL.ADD('SELECT * FROM SINTEGRA_ENTRADA WHERE atb like :atb ORDER BY DATA_ESCRITURACAO');
      qrnota.ParamByName('atb').Value := ME_FiltraATB('TB_SINTEGRA_ENTRADAS');
    end
    else
    begin
      frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RESUMO DAS NOTAS FISCAIS DE SAÍDA';
      qrnota.SQL.ADD('SELECT * FROM SINTEGRA_SAIDA WHERE atb like :atb ORDER BY DATA_ESCRITURACAO');
      qrnota.ParamByName('atb').Value := ME_FiltraATB('TB_SINTEGRA_SAIDAS');
    end;

    qrnota.open;
    FXNOTA.LoadFromFile('\SOS\server\rel\f000127.fr3');
    FXNOTA.ShowReport;
  end;
...
begin
  ...
  begin
    QUERY.CLOSE;
    QUERY.SQL.CLEAR;
    QUERY.SQL.ADD('SELECT * FROM C000009 WHERE atb like :atb and CODIGO = ''' + frmPrincipal.zerarcodigo(QRNOTA.FIELDBYNAME('CODIGO_CLIENTE').ASSTRING, 6) + '''');
  ...
  begin
    QUERY.CLOSE;
    QUERY.SQL.CLEAR;
    QUERY.SQL.ADD('SELECT * FROM C000007 WHERE atb like :atb and CODIGO = ''' + frmPrincipal.zerarcodigo(QRNOTA.FIELDBYNAME('CODIGO_CLIENTE').ASSTRING, 6) + '''');
