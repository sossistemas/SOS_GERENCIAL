...
uses
  ...
  Buttons, Grids, Wwdbigrd, Wwdbgrid, DB, ZAbstractRODataset,
  ZDataset, wwdblook, wwdbedit, Menus, Wwdotdot, Wwdbcomb,
  AdvGlowButton, AdvShapeButton, frxDesgn, frxClass, frxDBSet, UCBase,
  AdvMetroButton, AdvSmoothExpanderPanel, JvBaseEdits,
  JvDBControls, JvExMask, JvToolEdit, ZAbstractDataset, AdvSmoothPanel;
...
uses
  modulo, principal, loc_funci, Math,
  xloc_cliente, baixa_localizar, xloc_empresa;

{$R *.dfm}

procedure Tfrmbaixa_estoque.qritem_baixa_produtosBeforePost(DataSet: TDataSet);
begin
  frmmodulo.qrbaixa_produtos.fieldbyname('subtotal').asfloat := frmmodulo.qrbaixa_produtos.fieldbyname('subtotal').asfloat + qritem_baixa_produtos.fieldbyname('total').asfloat - valor_anterior;

  frmmodulo.qrbaixa_produtos.fieldbyname('total').asfloat := frmmodulo.qrbaixa_produtos.fieldbyname('subtotal').asfloat + frmmodulo.qrbaixa_produtos.fieldbyname('acrescimo').asfloat - frmmodulo.qrbaixa_produtos.fieldbyname('desconto').asfloat;

  qritem_baixa_produtos.fieldbyname('codorcamento').asstring := frmmodulo.qrbaixa_produtos.fieldbyname('codigo').asstring;

end;

procedure Tfrmbaixa_estoque.qritem_baixa_produtosNewRecord(DataSet: TDataSet);
begin
  qritem_baixa_produtosATB.AsString := ME_FiltraATB('TB_BAIXA_ESTOQUE_ITEM');
end;

procedure Tfrmbaixa_estoque.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
begin
  ...
  frmmodulo.qrbaixa_produtos.SQL.add('select * from c000096 where atb like :atb order by codigo');
  ...
  begin
    ...
    qritem_baixa_produtos.SQL.add('select * from c000097 where atb like :atb and codorcamento = ''' + frmmodulo.qrbaixa_produtos.fieldbyname('codigo').asstring + '''');
  ...
  begin
    qritem_baixa_produtos.close;
    qritem_baixa_produtos.SQL.clear;
    qritem_baixa_produtos.SQL.add('select * from c000097 where atb like :atb');
    qritem_baixa_produtos.open;
  end;
...
begin
  ...
  begin
    ...
    begin
      frmmodulo.qrempresa.Locate('codigo', resultado_pesquisa1, [loCaseInsensitive]);
      frmmodulo.qrbaixa_produtos.fieldbyname('codfilial').asstring := frmmodulo.qrempresa.fieldbyname('codigo').asstring;
...
begin
  ...
  begin
    ...
    frmmodulo.qrbaixa_produtos.fieldbyname('TIPO').AsInteger := wwdbComboBox1.ITEMINDEX + 2;
  end;
  ...
  if application.messagebox('Deseja Finalizar a Operação?', 'Atenção', mb_yesno + MB_ICONWARNING) = idyes then
  begin
    item := 1;
    ...
    begin
      ...
      frmmodulo.QRPRODUTO_MOV.fieldbyname('MOVIMENTO').AsInteger := StrToInt(copy(wwdbComboBox1.Text, 1, 2));
...
begin
  ...
  frmmodulo.qrempresa.Locate('codigo', frmmodulo.qrbaixa_produtos.fieldbyname('codfilial').asstring, [loCaseInsensitive]);
  ...
  if wwdbComboBox1.ITEMINDEX = 4 then
  begin
    fxorcamento.LoadFromFile('\SOS\server\rel\f000069_FILIAL.fr3');
    fxorcamento.ShowReport;
  end
  else
  begin
    fxorcamento.LoadFromFile('\SOS\server\rel\f000069_2.fr3');
    fxorcamento.ShowReport;
  end;
...
begin
  ...
  begin
    ...
    begin
      if frmmodulo.qrprodutoNAO_MOVIMENTA_ESTOQUE.AsString = 'S' then
      begin
...
procedure Tfrmbaixa_estoque.qritem_baixa_produtosBeforeOpen(DataSet: TDataSet);
begin
...
begin
  ...
  frmmodulo.QRPRODUTO_MOV.ExecSQL;
  frmmodulo.qrbaixa_produtos.fieldbyname('subtotal').asfloat := frmmodulo.qrbaixa_produtos.fieldbyname('subtotal').asfloat - qritem_baixa_produtos.fieldbyname('total').asfloat;
...
begin
  if (qritem_baixa_produtos.state = dsedit) or (qritem_baixa_produtos.state = dsinsert) then
  begin
    qritem_baixa_produtos.fieldbyname('total').asfloat := (qritem_baixa_produtos.fieldbyname('qtde').asfloat * qritem_baixa_produtos.fieldbyname('unitario').asfloat) - qritem_baixa_produtos.fieldbyname('desconto').asfloat + qritem_baixa_produtos.fieldbyname('acrescimo').asfloat;
  end;
end;
...
begin
  ...
  if (frmmodulo.qrbaixa_produtos.state = dsinsert) or (frmmodulo.qrbaixa_produtos.state = dsedit) then
  begin
    frmmodulo.qrbaixa_produtos.fieldbyname('codfilial').asstring := frmprincipal.zerarcodigo(ecliente.Text, 6);
    ...
    begin
      ...
      if wwdbComboBox1.ITEMINDEX = 4 then
      begin
        if not frmprincipal.Locregistro(frmmodulo.qrempresa, dbedtfilial.Text, 'codigo') then
        begin
          blocclienteClick(frmbaixa_estoque);
        end
        else
...
procedure Tfrmbaixa_estoque.dsorcamentoDataChange(Sender: TObject; Field: TField);
begin
  if frmmodulo.qrbaixa_produtos.fieldbyname('tipo').AsInteger = 0 then
    wwdbComboBox1.Text := 'INSTALAÇÃO';
  if frmmodulo.qrbaixa_produtos.fieldbyname('tipo').AsInteger = 1 then
...
procedure Tfrmbaixa_estoque.wwdbComboBox1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    ecliente.SetFocus;
end;
...
begin
  ...
  begin
    application.messagebox('Impossível Alterar ,movimento já finalizado', 'Atenção', mb_ok + MB_ICONWARNING);
    Exit;
  end;
