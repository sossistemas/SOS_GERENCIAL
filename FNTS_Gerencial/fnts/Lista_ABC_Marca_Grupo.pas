...
uses
  ...
  Dialogs, AdvGlowButton, ExtCtrls, StdCtrls, Mask, DB, DBAccess, frxClass,
  frxDBSet, frxDesgn, ZAbstractRODataset, ZAbstractDataset, ZDataset,
  AdvReflectionImage, Vcl.Menus, AdvMetroButton, AdvSmoothPanel,
...
uses
  xloc_marca, xloc_grupo, modulo, principal;
...
procedure TfrmLista_ABC_Marca_Grupo.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
procedure TfrmLista_ABC_Marca_Grupo.DateEdit1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    perform(WM_NEXTDLGCTL, 0, 0);
end;

procedure TfrmLista_ABC_Marca_Grupo.DateEdit2KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bimprimir.setfocus;
end;
...
procedure TfrmLista_ABC_Marca_Grupo.combo_marcaKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    perform(WM_NEXTDLGCTL, 0, 0);
end;

procedure TfrmLista_ABC_Marca_Grupo.combo_grupoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    perform(WM_NEXTDLGCTL, 0, 0);
end;
...
begin
  ...
  qrtotal.sql.Add('where c000032.atb like :atbc000032 and c000025.atb like :atbc000025 and ');
  ...
  qrtotal.ParamByName('atbc000032').Value := ME_FiltraATB('TB_MOVIMENTO');
  qrtotal.ParamByName('atbc000025').Value := ME_FiltraATB('TB_PRODUTO');
  qrtotal.open;
  ...
  query.sql.Add('c000032.atb like :atbc000032 and ');
  query.sql.Add('c000025.atb like :atbc000025 and ');
  query.sql.Add('c000019.atb like :atbc000019 and ');
  query.sql.Add('c000017.atb like :atbc000017 and ');
  ...
  query.ParamByName('atbc000032').Value := ME_FiltraATB('TB_MOVIMENTO');
  query.ParamByName('atbc000025').Value := ME_FiltraATB('TB_PRODUTO');
  query.ParamByName('atbc000019').Value := ME_FiltraATB('TB_MARCA');
  query.ParamByName('atbc000017').Value := ME_FiltraATB('TB_GRUPO_PRODUTO');
  ...
  relABC.LoadFromFile('\SOS\server\rel\f000195.fr3');
  relABC.ShowReport;
...
begin
  ...
  qrMarca.sql.Add('c000032.atb like :atbc000032 and ');
  qrMarca.sql.Add('c000025.atb like :atbc000025 and ');
  qrMarca.sql.Add('c000019.atb like :atbc000019 and ');
  ...
  qrMarca.ParamByName('atbc000032').Value := ME_FiltraATB('TB_MOVIMENTO');
  qrMarca.ParamByName('atbc000025').Value := ME_FiltraATB('TB_PRODUTO');
  qrMarca.ParamByName('atbc000019').Value := ME_FiltraATB('TB_MARCA');
  ...
  if (query.FIELDBYNAME('total_venda').AsFloat > 0) and (qrMarca.FIELDBYNAME('total').AsFloat > 0) then
  begin
    query.FIELDBYNAME('perc_marca').AsFloat := (query.FIELDBYNAME('total_venda').AsFloat * 100) / qrMarca.FIELDBYNAME('total').AsFloat;

    query.FIELDBYNAME('perc_total').AsFloat := (query.FIELDBYNAME('total_venda').AsFloat * 100) / rtotal;
