...
uses
  ...
  ZDataset, Buttons, AdvGlowButton, AdvMetroButton,
...
procedure Tfrmxloc_fornecedor.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;
...
procedure Tfrmxloc_fornecedor.locKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = vk_down then
  begin
    grid.setfocus;
...
begin
  ...
  if parametro_pesquisa <> '' then
    ...
    query.SQL.Add('select * from c000009 where atb like :atb and codigo = ''XXXXXX''');
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000009 where atb like :atb and upper(' + tipo + ') like ''' + ref + loc.Text + '%'' order by ' + tipo);
    query.ParamByName('atb').Value := ME_FiltraATB('TB_FORNECEDOR');
    query.open;
  except
  end;
