...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, TFlatPanelUnit, Mask, Buttons, Grids,
  Wwdbigrd, Wwdbgrid, DB, ZAbstractRODataset, ZDataset,
  wwdblook, AdvGlowButton, Menus, AdvMetroButton,
...
uses
  modulo, principal, contasreceber;

{$R *.dfm}

procedure Tfrmcontasreceber_renegociar.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
procedure Tfrmcontasreceber_renegociar.rdescontoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
...
begin
  try
    ...
    begin
      ...
      frmcontasreceber.qrproduto_mov.sql.add('update c000032 set codnota = ''' + numero_venda + ''' where atb like :atb and codnota = ''' + frmcontasreceber.qrcontasreceber.fieldbyname('codvenda').asstring + ''' and movimento in (2,7) and codcliente = ''' + frmcontasreceber.qrcr.fieldbyname('codcliente').asstring + '''');
      frmcontasreceber.qrproduto_mov.ParamByName('atb').Value := ME_FiltraATB('TB_MOVIMENTO');
      ...
      begin
        ...
        frmcontasreceber.qrcontasreceber.sql.Add('delete from c000049 where atb like :atb and codigo = ''' + frmcontasreceber.qrcr.fieldbyname('codigo').asstring + '''');
        frmcontasreceber.qrcontasreceber.ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_RECEBER');
    ...
    frmcontasreceber.qrcontasreceber.Close;
    frmcontasreceber.qrcontasreceber.sql.clear;
    frmcontasreceber.qrcontasreceber.sql.Add('select * from c000049 where atb like :atb and codigo = ''nenhum''');
    frmcontasreceber.qrcontasreceber.ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_RECEBER');
    ...
    begin
      ...
      frmcontasreceber.qrcontasreceber.fieldbyname('codcaixa').asstring := Caixa_Geral;
      ...
      frmcontasreceber.qrcontasreceber.fieldbyname('atb').asstring := ME_GravaATB('TB_CONTA_RECEBER');
...
procedure Tfrmcontasreceber_renegociar.wwDBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    BGRAVAR.SETFOCUS;
end;

procedure Tfrmcontasreceber_renegociar.rintervaloKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    BITBTN1.SETFOCUS;
end;
...
procedure Tfrmcontasreceber_renegociar.qrvenda_contasreceberBeforePost(DataSet: TDataSet);
begin
  qrvenda_contasreceber.fieldbyname('terminal').asstring := numero_terminal;
end;
