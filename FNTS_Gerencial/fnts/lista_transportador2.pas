...
procedure Tfrmlista_transportador2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  begin
    transportador := ' and codIGO = ''' + copy(combo_transportador.text, 1, 6) + '''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA3').asstring := 'TRANSPORTADOR: ' + COMBO_transportador.TEXT;
  end;
  ...
  begin
    ...
    qrtransportador.sql.add('select * from c000010 where atb like :atb and NOME IS NOT NULL ' + transportador + ' order by ' + COMBO_ORDEM.TEXT);
    ...
    FXtransportador.LoadFromFile('\SOS\server\rel\f000011.fr3');
  ...
  begin
    ...
    FXtransportador.LoadFromFile('\SOS\server\rel\f000012.fr3');
    FXtransportador.ShowReport;
  end;
