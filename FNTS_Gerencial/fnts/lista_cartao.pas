...
uses
  ...
  Menus, Grids, DBGrids, wwdblook, AdvGlowButton, MemDS, DBAccess,
...
uses
  modulo, loc_caixa, xloc_cliente, xloc_funcionario, principal, xloc_banco,
  xloc_CARTAO;
...
procedure Tfrmlista_cartao.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  begin
    cliente := ' and c000124.cod_cliente = ''' + copy(combo_cliente.text, 1, 6) + '''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA3').asstring := 'CLIENTE: ' + COMBO_CLIENTE.TEXT;
  end;
  if combo_banco.Text = 'TODAS' then
    banco := ''
  else
  begin
    banco := ' and c000124.banco = ''' + copy(combo_banco.text, 1, 3) + '''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA4').asstring := 'BANCO: ' + COMBO_banco.TEXT;
  end;
  ...
  begin
    if COMBO_SITUACAO.ITEMINDEX = 1 then
      situacao := ' and C000124.situacao = ''ABERTO''';
    if COMBO_SITUACAO.ITEMINDEX = 2 then
      situacao := ' and C000124.situacao = ''BAIXADO''';
  ...
  begin
    if combo_tipo.ITEMINDEX = 1 then
      tipo := ' AND C000124.TIPO = ' + QuotedStr('CREDITO')
    else
      tipo := ' AND C000124.TIPO = ' + QuotedStr('DEBITO')
  end;
  ...
  begin
    ...
    qrCARTAO.sql.add('c000124.atb like :atbc000124 and ');
    qrCARTAO.sql.add('c000007.atb like :atbc000007 and ');
    qrCARTAO.sql.add('c000013.atb like :atbc000013 and ');

    qrCARTAO.sql.add('c000124.cod_cliente = c000007.codigo and');
    qrCARTAO.sql.add('c000124.banco = c000013.numero');
    qrCARTAO.sql.add(cliente + banco + situacao + periodo + tipo);
    ...
    qrCARTAO.ParamByName('atbc000124').Value := ME_FiltraATB('TB_MOVIMENTO_CARTAO');
    qrCARTAO.ParamByName('atbc000007').Value := ME_FiltraATB('TB_CLIENTE');
    qrCARTAO.ParamByName('atbc000013').Value := ME_FiltraATB('TB_BANCO');

    qrCARTAO.open;

    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'MOVIMENTO DE CARTÃO';
    fxCARTAO.LoadFromFile('\SOS\server\rel\f000192.fr3');
    fxCARTAO.ShowReport;
  end;
end;
