...
uses
  ...
  Mask, maskutils, PageView, Wwdbigrd, Wwdbgrid, AdvGlowButton, dxGDIPlusClasses,
  AdvMetroButton, AdvSmoothPanel,
  AdvSmoothExpanderPanel, JvExMask, JvToolEdit, JvDBControls;
...
uses
  modulo, principal, loc_banco,
  xloc_cliente, cheque_conta, cheque_menu;
...
begin
  ...
  begin
    frmprincipal.logUC('Incluiu Cheque - Nr: ' + DBEdit7.text + ' Cliente: ' + DBEdit2.text, 1);
    frmcheque_menu.qrcheque.post;
  end;

  if (frmcheque_menu.qrcheque.State = dsedit) then
  begin
    frmprincipal.logUC('Alterou Cheque - Nr: ' + DBEdit7.text + ' Cliente: ' + DBEdit2.text, 2);
    frmcheque_menu.qrcheque.post;
  end;
...
begin
  ...
  if frmcheque_menu.qrcheque.FIELDBYNAME('TITULAR').asstring = '' then
    frmcheque_menu.qrcheque.FIELDBYNAME('TITULAR').asstring := frmcheque_menu.qrcheque.FIELDBYNAME('CLIENTE').asstring;
...
begin
  ...
  begin
    ...
    qrconta_cliente.sql.add('select count(codigo) cheque, conta, codbanco, agencia, data_conta from c000040 where atb like :atb and codcliente = ''' + ecliente.text + ''' group by conta, codbanco, agencia, data_conta');
    qrconta_cliente.ParamByName('atb').Value := ME_FiltraATB('TB_CHEQUE');
