...
uses
  ...
  ImgList, Menus, AdvGlowButton, ExtCtrls, RzPrgres,
  TFlatPanelUnit, ZConnection, RzButton, RzRadChk, ZAbstractConnection,
  System.ImageList;
...
//    verifica: TJvDatabaseItems;
    ...
    qrpdvDESCRICAO: TWideStringField;
    qrpdvCAMINHO: TWideStringField;
    qrpdvATUALIZAR: TWideStringField;
    qrpdvHOSTNAME: TWideStringField;
    qrpdvCX: TWideStringField;
      ...
      begin
        try
          ...
          textox := PWideChar('Não foi possível se conectar o banco de dados: ' + #13 +
            qrpdv.fieldbyname('hostname').asstring + ':C:\SOS\bd\base.fdb' + #13 +
...
begin
  ...
  AssignFile(TXT, 'C:\SOS\server\CG' + IntToStr(ANO) + IntToStr(MES) + IntToStr(DIA) + '_' + IntToStr(HORA) + IntToStr(MINUTO) + IntToStr(SEGUNDO) + '.001');
  ...
  bar.Percent := 0;
  origem := 'C:\SOS\server\CG' + IntToStr(ANO) + IntToStr(MES) + IntToStr(DIA) + '_' + IntToStr(HORA) + IntToStr(MINUTO) + IntToStr(SEGUNDO);
