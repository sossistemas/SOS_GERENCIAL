...
type
  TfrmExtras = class(TForm)
    ...
    procedure qrExtrasBeforeOpen(DataSet: TDataSet);
    procedure qrExtrasNewRecord(DataSet: TDataSet);
  private
...
begin
  if Application.MessageBox('Deseja excluir esta Regisro?','Atenção',MB_ICONQUESTION+MB_YESNO)=mrYes then begin
    qrExclui.Close;
    qrExclui.SQL.Clear;
    qrExclui.SQL.Add('select count(*) qtd from extra_produtos gr where gr.atb like :atb and gr.id_extra = ' + qrExtrasID.AsString);
    qrExclui.ParamByName('atb').Value := ME_FiltraATB('TB_PRODUTO_EXTRAS');
    qrExclui.Open;
...
procedure TfrmExtras.qrExtrasBeforeOpen(DataSet: TDataSet);
begin
  qrExtras.ParamByName('ATB').AsString := ME_FiltraATB('TB_EXTRAS');
end;
...
procedure TfrmExtras.qrExtrasNewRecord(DataSet: TDataSet);
begin
  qrExtrasATB.AsString := ME_GravaATB('TB_EXTRAS');
end;
