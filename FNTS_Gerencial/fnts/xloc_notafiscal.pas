...
uses
  ...
  ZDataset, DBCtrls, Buttons, ZAbstractDataset;
...
procedure Tfrmxloc_notafiscal.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;
...
procedure Tfrmxloc_notafiscal.locKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = vk_down then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      query.sql.clear;
      if loc.text = '' then
        query.sql.Add('select * from c000061 where atb like :atb and codigo = ''xxxxxx''')
      else
        query.sql.Add('select * from c000061 where atb like :atb and upper(' + tipo + ') like ''' + ref + loc.Text + '%'' order by ' + tipo);
      query.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_FISCAL_SAIDA');
      query.open;
    end;
    ...
    begin
      try
        ...
        begin
          query.sql.Add('select * from c000061 where atb like :atb and DATA = :DATAX  order by ' + tipo);
    ...
    begin
      ...
      begin
        query.close;
        query.sql.clear;
        query.sql.Add('select * from c000061 where atb like :atb and CODCLIENTE = ''' + query2.fieldbyname('codigo').asstring + ''' order by NUMERO');
        query.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_FISCAL_SAIDA');
        query.open;
...
begin
  ...
  begin
    ...
    QUERY2.SQL.CLEAR;
    if loc.text = '' then
      query2.sql.Add('select * from c000007 where atb like :atb and codigo = ''xxxxxx''')
    else
      QUERY2.SQL.ADD('SELECT * FROM C000007 where atb like :atb and upper(NOME) like ''' + ref + loc.Text + '%'' order by NOME');
    QUERY2.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
    QUERY2.OPEN;
...
procedure Tfrmxloc_notafiscal.wwDBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    locKeyPress(frmxloc_NOTAFISCAL, Key);
  loc.setfocus;
end;
