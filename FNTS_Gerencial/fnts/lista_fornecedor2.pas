...
procedure Tfrmlista_fornecedor2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  begin
    FORNECEDOR := ' and codIGO = ''' + copy(combo_FORNECEDOR.text, 1, 6) + '''';
  ...
  begin
    ...
    qrFORNECEDOR.sql.add('select * from c000009 where atb like :atb and NOME IS NOT NULL ' + FORNECEDOR + ' order by ' + COMBO_ORDEM.TEXT);
    ...
    FXFORNECEDOR.LoadFromFile('\SOS\server\rel\f000009.fr3');
  ...
  begin
    ...
    FXFORNECEDOR.LoadFromFile('\SOS\server\rel\f000010.fr3');
    FXFORNECEDOR.ShowReport;
  end;
...
begin
  if QRFORNECEDORTIPO.Value = 2 then
    QRFORNECEDORpessoa.Value := 'JURIDICA'
  else
    QRFORNECEDORpessoa.Value := 'FISICA';
