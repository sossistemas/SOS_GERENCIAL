...
type
  Tfrmnotafiscal_importacupom = class(TForm)
    ...
    lbtotal: TLabel;
    Edit1: TEdit;
    qrCupomGEROU_SAT: TWideStringField;
    cxGrid1DBTableView1Column1: TcxGridDBColumn;
    qrChave: TZQuery;
    qrCupomCUPOM_NUMERO: TWideStringField;
    qNFCE: TZQuery;
    qNFCENUMERO: TIntegerField;
    qNFCEDATA: TDateField;
    qNFCETOTAL: TFloatField;
    qNFCECLIENTE: TWideStringField;
    qNFCECHAVE: TWideStringField;
    qNFCESITUACAO: TIntegerField;
    qNFCETROCO: TFloatField;
    qNFCEHORA: TWideStringField;
    qNFCECONTINGENCIA: TWideStringField;
    qNFCEENVIADOCONTINGENCIA: TWideStringField;
    qNFCEMOTIVOCONTIGENCIA: TWideStringField;
    qNFCEXMLENVIO: TBlobField;
    qNFCEXMLCACNELAMENTO: TBlobField;
    qNFCECUPOM: TWideStringField;
    qrCupomNUMERONOTA: TWideStringField;
    qrCupomNUMERO: TIntegerField;
...
var
  ...
  vitem: integer;
  vcodvendedor, vnumero_nota, vcfop, vcodcliente, vcodfilial, vmodelo, vserie, Cupons, NNota: string;
  vemissao, vsaida: TDateTime;
begin
  if not qrCupom.IsEmpty then
  begin

    if qrCupomNFE.AsString = 'S' then
    begin
      Application.messagebox('Já Foi Emitido Uma Nota Fiscal Para Esse Cupom!', 'Atenção!', mb_ok + MB_ICONEXCLAMATION);
      exit;
    end;
    Cupons := '';
    qrCupom.Filtered := True;
    qrCupom.Filter := ' SEL = ''S'' ';
    qrCupom.First;
    while not qrCupom.Eof do
    begin
      if Cupons = '' then
        Cupons := qrCupomCUPOM_NUMERO.AsString
      else
        Cupons := Cupons + ''',''' + qrCupomCUPOM_NUMERO.AsString;
      qrCupom.Next;
    end;
    qrCupom.Filtered := False;
    qrCupom.Filter := '';
    //------------------------------------------------------------------------------------------------------------------------------
    vtotal := 0;
    vsubtotal := 0;
    ...
    query.Close;
    query.SQL.Clear;
    query.SQL.Add('select * from c000032 where atb like :atb and cupom_numero in(''' + Cupons + ''')');  // alterado 09/02/2018 por diego molina para pegar todos os cuposn selecionado
    query.SQL.Add('and cupom_modelo = ''2D''');
    query.SQL.Add('order by cupom_numero');
    ...
    begin
      ...
      frmnotafiscal.qry.first;
      query.First;
      while not query.Eof do
      begin
        if (query.FieldByName('NFCE').AsString = 'S') or (query.FieldByName('gerou_sat').AsString = 'S') then
        begin
          qrChave.Close;
          qrChave.SQL.Clear;
          if (query.FieldByName('NFCE').AsString = 'S') then
          begin
            qrChave.SQL.Add('select s.chave from nfce s where s.atb like :atb and s.cupom = ' + QuotedStr(query.FieldByName('CUPOM_NUMERO').AsString));
          end
          else
          begin
            qrChave.SQL.Add('select s.chave from sat s where s.atb like :atb and s.cupom = ' + QuotedStr(query.FieldByName('CUPOM_NUMERO').AsString));
          end;
          qrChave.ParamByName('atb').Value := ME_FiltraATB('TB_NFCE');
          qrChave.Open;
          if NNota <> query.FieldByName('cupom_numero').AsString then
            if not qrChave.IsEmpty then
            begin

              frmnotafiscal.qry.Append;
              frmnotafiscal.qry.FieldByName('CHAVE').AsString := qrChave.FieldByName('chave').AsString;
              frmnotafiscal.qry.Post;
              NNota := query.FieldByName('cupom_numero').AsString;
            end;
        end;
        ...
        begin
          ...
          if StrToInt(frmmodulo.qrfilial.fieldbyname('crt').asstring) <> 1 then
          begin
          ...
          begin
            qrnotafiscal_item.FieldByName('VALOR_ICMS').AsFloat := 0;
            qrnotafiscal_item.FieldByName('BASE_CALCULO').AsFloat := 0;
            qrnotafiscal_item.FieldByName('ICMS_REDUZIDO').AsFloat := 0;
            qrnotafiscal_item.FieldByName('MARGEM_AGREGADA').AsFloat := 0;
            qrnotafiscal_item.FieldByName('BASE_SUB').AsFloat := 0;
            qrnotafiscal_item.FieldByName('ICMS_SUB').AsFloat := 0;
            qrnotafiscal_item.FieldByName('ISENTO').AsFloat := 0;
          ...
          qrnotafiscal_item.FieldByName('SUBTOTAL').AsFloat := query.fieldbyname('total').AsFloat + query.fieldbyname('desconto').AsFloat;
          qrnotafiscal_item.FieldByName('ITEM').AsString := IntToStr(vitem + 1); //query.fieldbyname('cupom_item').AsString;  //alterado por diego molina 12/02/2019 para inserir varios cupons
          qrnotafiscal_item.FieldByName('GRADE').AsString := '';

          qrnotafiscal_itemATB.AsString := ME_GravaATB('TB_NOTA_FISCAL_SAIDA_ITENS');
      ...
      query.Close;
      query.SQL.Clear;
      query.SQL.Add('update c000032 set cupom_modelo = ''01'' where atb like :atb and cupom_numero = ''' + qrCupomCUPOM_NUMERO.AsString + '''');
      query.ParamByName('atb').Value := ME_FiltraATB('TB_MOVIMENTO');
      query.ExecSQL;
...
begin
  qrCupom.Close;
  qrCupom.SQL.Clear;
  qrCupom.SQL.Add('select ''S'' sel, cup.cupom_numero, cup.data, sum(cup.total) total, cup.nfce, cup.gerou_sat, ');
  qrCupom.SQL.Add('  case cup.cupom_modelo when ''2D'' then');
  qrCupom.SQL.Add('    ''N'' else ''S'' end nfe, cup.numeronota');
  qrCupom.SQL.Add('from c000032 cup where atb like :atb and ');
  ...
  qrCupom.SQL.Add('group by cup.cupom_numero, cup.data, cup.nfce, cup.cupom_modelo, cup.gerou_sat, cup.numeronota');
  qrCupom.ParamByName('atb').Value := ME_FiltraATB('TB_MOVIMENTO');
  qrCupom.Open;
...
begin
  qNFCE.ParamByName('atb').Value := ME_FiltraATB('TB_NFCE');
  qNFCE.Open;
  DateEdit1.Date := Date;
  DateEdit2.Date := Date;
...
begin
  Total := 0;
  if (qrCupom.Active) and not (qrCupom.IsEmpty) then
  begin
    try
...
procedure Tfrmnotafiscal_importacupom.cxGrid1DBTableView1SELPropertiesEditValueChanged(Sender: TObject);
begin
  Total;
end;
...
procedure Tfrmnotafiscal_importacupom.eclienteKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
