...
uses
  ...
  Menus, AdvGlowButton, Vcl.Imaging.jpeg, AdvMetroButton,
...
procedure Tfrmlista_notacompra.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  begin
    ...
    if ecfop.text <> '' then
      cfop := ' and cfop = ''' + ecfop.text + ''''
    else
      cfop := '';

    qrnota.close;
    qrnota.sql.clear;
    qrnota.SQL.add('select * from c000087 where atb like :atb and data_lancamento BETWEEN :datai and :dataf ' + cfop + ' order by ' + ordem);
    ...
    FXNOTA.LoadFromFile('\SOS\server\rel\f000088.fr3');
    FXNOTA.ShowReport;
  end;
  ...
  begin
    ...
    fxnota.LoadFromFile('\SOS\server\rel\f000088_3.fr3');
    fxnota.ShowReport;
  end;
  ...
  begin
    qrnota.close;
    qrnota.sql.clear;
    qrnota.SQL.add('select * from c000087 where atb like :atb and data_lancamento BETWEEN :datai and :dataf ' + cfop + ' order by cfop,' + ordem);
    qrnota.params.ParamByName('datai').asdatetime := dateedit1.date;
    qrnota.params.ParamByName('dataf').asdatetime := dateedit2.date;
    qrnota.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_ENTRADA');
    qrnota.open;
    ...
    FXNOTA.LoadFromFile('\SOS\server\rel\f000088_2.fr3');
    FXNOTA.ShowReport;
  end;
  ...
  begin
    ...
    FXNOTA.LoadFromFile('\SOS\server\rel\f000088_4.fr3');
    FXNOTA.ShowReport;
  end;
  ...
  begin
    ...
    qrcompra_produto.sql.add('c000088.atb like :atbc000088 and ');
    qrcompra_produto.sql.add('c000025.atb like :atbc000025 and ');
    ...
    qrcompra_produto.ParamByName('atbc000088').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
    ;
    qrcompra_produto.ParamByName('atbc000025').Value := ME_FiltraATB('TB_PRODUTO');
    ;
  ...
  begin
    ...
    qrentrada.sql.add('c000087.atb like :atbc000087 and ');
    qrentrada.sql.add('c000088.atb like :atbc000088 and ');
    qrentrada.sql.add('c000009.atb like :atbc000009 and ');
    ...
    qrentrada.ParamByName('atbc000087').Value := ME_FiltraATB('TB_NOTA_ENTRADA');
    qrentrada.ParamByName('atbc000088').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
    qrentrada.ParamByName('atbc000009').Value := ME_FiltraATB('TB_FORNECEDOR');
    qrentrada.open;
...
begin
  ...
  if combo_relatorio.ItemIndex = 2 then
  begin
    FXNOTA.LoadFromFile('\SOS\server\rel\f000088_5.fr3');
    FXNOTA.DesignReport;
  end;
...
procedure Tfrmlista_notacompra.combo_relatorioKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
