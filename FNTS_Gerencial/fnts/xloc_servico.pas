...
procedure Tfrmxloc_servico.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;
...
procedure Tfrmxloc_servico.locKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = vk_down then
  begin
    grid.setfocus;
...
begin
  ...
  begin
    ...
    begin
      ...
      resultado_pesquisa2 := query.fieldbyname('SERVICO').asstring;
      close;
    end;
  end;
end;

procedure Tfrmxloc_servico.locKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    if query.RecNo > 0 then
...
begin
  ...
  query.Close;
  query.SQL.clear;
  query.SQL.Add('select * from c000011 where atb like :atb and codigo = ''XXXXXX''');
  query.ParamByName('atb').Value := ME_FiltraATB('TB_SERVICOS');
  query.open;
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000011 where atb like :atb and upper(' + tipo + ') like ''' + ref + loc.Text + '%'' order by ' + tipo);
    query.ParamByName('atb').Value := ME_FiltraATB('TB_SERVICOS');
    query.open;
  except
  end;
