...
begin
  ...
  qrproduto.Close;
  qrproduto.SQL.Clear;
  qrproduto.SQL.Add('select * from c000025 where atb like :atb and data_remarcacao_venda >= ' + QuotedStr(FormatDateTime('DD.MM.YYYY',Data)));
  ...
  if (cdsTemp.Active) and (cdsTemp.RecordCount > 0)then begin
    try
      ...
      while not cdsTemp.Eof do begin
        ...
        qrproduto.Close;
        qrproduto.SQL.Clear;
        qrproduto.SQL.Add('Select * from c000025 where atb like :atb and codigo = '+QuotedStr(cdsTempProduto.AsString));
        qrproduto.ParamByName('atb').Value := ME_FiltraATB('TB_PRODUTO');
        qrproduto.Open;
...
begin
  if Key = #13 then begin
    ...
    qrproduto.close;
    qrproduto.sql.clear;
    qrproduto.sql.add('select prod.*, est.* from c000025 prod, c000100 est where prod.atb like :atbpro and est.atb like :atbest and prod.codigo = est.codproduto and codigo = ''' + campo + '''');
    ...
    if qrproduto.RecordCount = 0 then begin
      qrproduto.close;
      qrproduto.sql.clear;
      qrproduto.sql.add('select prod.*, est.* from c000025 prod, c000100 est where prod.atb like :atbpro and est.atb like :atbest and prod.codigo = est.codproduto and  codbarra = ''' + ed_produto.Text + '''');
      qrproduto.ParamByName('atbpro').Value := ME_FiltraATB('TB_PRODUTO');
      qrproduto.ParamByName('atbest').Value := ME_FiltraATB('TB_ESTOQUE');
      qrproduto.open;
      if qrproduto.recordcount = 0 then begin
        qrproduto.close;
        qrproduto.sql.clear;
        qrproduto.sql.add('select prod.*, est.* from c000025 prod, c000100 est where prod.atb like :atbpro and est.atb like :atbest and prod.codigo = est.codproduto and  upper(produto) like ''' + ansiuppercase(ed_produto.Text) + '%''');
        qrproduto.ParamByName('atbpro').Value := ME_FiltraATB('TB_PRODUTO');
        qrproduto.ParamByName('atbest').Value := ME_FiltraATB('TB_ESTOQUE');
        qrproduto.open;
      end;
    ...
    if qrproduto.RecordCount > 0 then begin
      if qrproduto.recordcount = 1 then begin
        ...
        {  ed_produto.text := qrproduto.fieldbyname('produto').asstring;
         ...
         ed_produto.text := frmxloc_produto.query.fieldbyname('PRODUTO').asstring;
         lbCodigo.Caption  := frmxloc_produto.query.fieldbyname('CODIGO').asstring;
          edEtiquetas.SetFocus;
        end else begin
