...
uses
  ...
  Dialogs, AdvPreviewMenu, AdvPreviewMenuStylers, AdvToolBar, AdvToolBarStylers,
  AdvOfficeStatusBarStylers, StdCtrls, DBCtrls, Mask, RzEdit,
  RzDBEdit, RzDBBnEd, wwdblook, RzDBCmbo, Buttons, DB, AdvGlowButton,
  ImgList, Menus, cxControls, cxContainer, cxEdit, cxTextEdit,
  cxCurrencyEdit, ExtCtrls, ZAbstractRODataset,
  ZDataset, TFlatPanelUnit, RzPanel, AdvMetroButton,
  AdvSmoothExpanderPanel, JvExMask, JvToolEdit, JvDBControls, ZAbstractDataset,
  System.ImageList, AdvOfficeStatusBar, AdvSmoothPanel, RzCmboBx;
...
type
  Tfrmcompra_item = class(TForm)
    ...
    elote: TDBEdit;
    evalidade: TJvDBDateEdit;
    Label25: TLabel;
    Label44: TLabel;
    qrgrade_entrada: TZQuery;
    qrgrade_entradaCODIGO: TWideStringField;
    qrgrade_entradaCODPRODUTO: TWideStringField;
    qrgrade_entradaLOTE: TWideStringField;
    qrgrade_entradaESTOQUE: TFloatField;
    qrgrade_entradaCODCLIENTE: TWideStringField;
    qrgrade_entradaPRECOCUSTO: TFloatField;
    qrgrade_entradaPRECOVENDA: TFloatField;
    qrgrade_entradaSITUACAO: TIntegerField;
    qrgrade_entradaCLIENTE: TWideStringField;
    qrgrade_entradaCODNOTA: TWideStringField;
    qrgrade_entradaCODITEM: TWideStringField;
    qrgrade_entradaNUMERONOTA: TWideStringField;
    qrgrade_entradaCODFORNECEDOR: TWideStringField;
    qrgrade_entradaCODVENDA_ITEM: TWideStringField;
    qrgrade_entradaFILTRO: TIntegerField;
    qrgrade_entradaCODVENDA: TWideStringField;
...
uses
  compra, principal, xloc_produto, modulo, xloc_cfop, xloc_cst, compra_menu,
  produto_preco, Math, produto_serial, produto_gradeF;
...
begin
  ...
  for i := 0 to ComponentCount - 1 do
    ...
    begin
      if (tedit(Components[i]).Name = 'ecodproduto') or (tedit(Components[i]).Name = 'eun_compra') or (tedit(Components[i]).Name = 'ecst') or (tedit(Components[i]).Name = 'ecfop') or (tedit(Components[i]).Name = 'eqtde') or (tedit(Components[i]).Name = 'esubtotal') or (tedit(Components[i]).Name = 'eunitario') or (tedit(Components[i]).Name = 'etotal') then
      begin
        if (tedit(Components[i]).Text = '') or (tedit(Components[i]).Text = '0,00') or (tedit(components[i]).text = '0,000000') or (tedit(components[i]).text = '0,000') then
        begin
...
begin
  ...
  begin
    ...
    frmmodulo.qrproduto.sql.add('select * from c000025 where atb like :atb and codbarra = ''' + ecodproduto.text + '''');
    ...
    begin
      ...
      begin
        frmmodulo.qrproduto.Close;
        frmmodulo.qrproduto.sql.clear;
        frmmodulo.qrproduto.sql.add('select * from c000025 where atb like :atb and codIGO = ''' + frmcompra.qrnota_item.FieldByName('codproduto').asstring + '''');
        frmmodulo.qrproduto.open;
        if frmmodulo.qrproduto.RecordCount > 0 then
        begin
          vcusto_anterior := frmmodulo.qrproduto.fieldbyname('precocusto').AsFloat;
...
procedure Tfrmcompra_item.eun_compraKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
...
procedure Tfrmcompra_item.FormClose(Sender: TObject; var Action: TCloseAction);
var
  vdesconto: double;
begin
  ...
  begin
    ...
    begin
      if frmcompra.XDESCONTO.Checked = false then
        frmcompra.vtotal := frmcompra.vtotal + (frmcompra.vprodutos + frmcompra.vvst + frmcompra.vfrete + frmcompra.vseguro + frmcompra.voutras + frmcompra.vespecial + frmcompra.vipi) - frmcompra.rdesconto.Value
      else
  ...
  if vdesconto > 0 then
    frmcompra.vtotal := frmcompra.vtotal + (frmcompra.vprodutos + frmcompra.vvst + frmcompra.vfrete + frmcompra.vseguro + frmcompra.voutras + frmcompra.vespecial + frmcompra.vipi) - vdesconto;
...
begin
  ...
  frmcompra.qrnota_item.FieldByName('SUBTOTAL').asfloat := frmcompra.qrnota_item.FieldByName('qtde').asfloat * eunitario.Value;

  frmcompra.qrnota_item.FieldByName('total').asfloat := frmcompra.qrnota_item.FieldByName('subtotal').asfloat - frmcompra.qrnota_item.FieldByName('desconto').asfloat;

  if frmcompra.qrnota_item.FieldByName('total').asfloat > 0 then
  begin

    frmcompra.qrnota_item.FieldByName('FRETE').asfloat := (frmcompra.qrnota_item.FieldByName('total').asfloat * FRMCOMPRA_MENU.QRNOTA.FIELDBYNAME('CONF_FRETE').ASFLOAT) / FRMCOMPRA_MENU.QRNOTA.FIELDBYNAME('CONF_TOTALPRODUTOS').ASFLOAT;

    frmcompra.qrnota_item.FieldByName('SEGURO').asfloat := (frmcompra.qrnota_item.FieldByName('total').asfloat * FRMCOMPRA_MENU.QRNOTA.FIELDBYNAME('CONF_SEGURO').ASFLOAT) / FRMCOMPRA_MENU.QRNOTA.FIELDBYNAME('CONF_TOTALPRODUTOS').ASFLOAT;

    frmcompra.qrnota_item.FieldByName('OUTRAS').asfloat := (frmcompra.qrnota_item.FieldByName('total').asfloat * FRMCOMPRA_MENU.QRNOTA.FIELDBYNAME('CONF_OUTRAS').ASFLOAT) / FRMCOMPRA_MENU.QRNOTA.FIELDBYNAME('CONF_TOTALPRODUTOS').ASFLOAT;
    ...
    begin
      if frmcompra.XDESCONTO.Checked = false then
      begin
        frmcompra.qrnota_item.FieldByName('DESCONTO').asfloat := (frmcompra.qrnota_item.FieldByName('SUBTOTAL').asfloat * FRMCOMPRA_MENU.QRNOTA.FIELDBYNAME('CONF_DESCONTO').ASFLOAT) / FRMCOMPRA_MENU.QRNOTA.FIELDBYNAME('CONF_TOTALPRODUTOS').ASFLOAT;
      end;
    end;
  end;
...
begin
  ...
  qrproduto.sql.add('precocusto = :precocusto,');
  qrproduto.sql.add('DATA_ULTIMACOMPRA = :DATA_ULTIMACOMPRA,');
  qrproduto.sql.add('notafiscal = :notafiscal,');
  qrproduto.sql.add('CLASSIFICACAO_FISCAL = :CLASSIFICACAO_FISCAL,');
  qrproduto.sql.add('codfornecedor = :codfornecedor');
  qrproduto.sql.add('where atb like :atb and codigo = ''' + ecodproduto.Text + '''');
  ...
  qrproduto.Params.ParamByName('CLASSIFICACAO_FISCAL').asstring := ECLASSIF.Text;
  ...
  try
    ...
    begin
      ...
      frmmodulo.qrpreco.sql.Add('select * from c000026 where atb like :atb and codproduto = ''' + frmmodulo.qrproduto.fieldbyname('codigo').asstring + '''');
  ...
  begin
    ...
    frmmodulo.qrfornecedor_codigo.sql.add('select * from fornecedor_codigo where atb like :atb and codigo = ''' + ed_cod_fornecedor.text + '''');
  ...
  begin
    ...
    begin
      ...
      if check_atualiza.checked then
        frmcompra.batualiza_itensClick(frmcompra);
      ...
      FRMCOMPRA.qrnota_item.FieldByName('ATB').AsString := ME_GravaATB('TB_NOTA_ENTRADA_ITENS');
...
procedure Tfrmcompra_item.combo_retidoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    EALIQUOTA.SETFOCUS;
end;

procedure Tfrmcompra_item.ealiquotaKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    perform(wm_nextdlgctl, 0, 0);
end;
...
begin
  ...
  if (ealiquota.value <> 0.00) and (ealiquota.value <> 7.00) and (ealiquota.value <> 12.00) and (ealiquota.value <> 17.00) and (ealiquota.value <> 18.00) and (ealiquota.value <> 25.00) and (ealiquota.value <> 27.00) then
  begin
  ...
  begin
    vdesc_total_nota := (frmcompra.qrnota_item.FieldByName('SUBTOTAL').asfloat * FRMCOMPRA_MENU.QRNOTA.FIELDBYNAME('CONF_DESCONTO').ASFLOAT) / FRMCOMPRA_MENU.QRNOTA.FIELDBYNAME('CONF_TOTALPRODUTOS').ASFLOAT;
  end;
  ...
  if (frmcompra.qrnota_item.fieldbyname('CST').asstring = '010') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '020') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '060') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '070') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '110') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '210') then
  begin

    if (frmcompra.qrnota_item.fieldbyname('CST').asstring = '020') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '070') then // reducao na base de calculo
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if frmcompra.etipo_baseicms.Checked = true then
              frmcompra.qrnota_item.fieldbyname('ICMS_BASE').asfloat := ((frmcompra.qrnota_item.fieldbyname('TOTAL').asfloat + efrete.Value + eoutras.Value) * (frmcompra.qrnota_item.FieldByName('icms_reducao').asfloat / 100)) / (frmcompra.qrnota_item.fieldbyname('ICMS_ALIQUOTA').asfloat / 100)
            else
              frmcompra.qrnota_item.fieldbyname('ICMS_BASE').asfloat := ((frmcompra.qrnota_item.fieldbyname('TOTAL').asfloat) * (frmcompra.qrnota_item.FieldByName('icms_reducao').asfloat / 100)) / (frmcompra.qrnota_item.fieldbyname('ICMS_ALIQUOTA').asfloat / 100);

            frmcompra.qrnota_item.fieldbyname('ICMS_VALOR').asfloat := frmcompra.qrnota_item.fieldbyname('ICMS_BASE').asfloat * (frmcompra.qrnota_item.fieldbyname('ICMS_ALIQUOTA').asfLOAT / 100);
          end;
        end
        else
        begin
      ...
      begin
        if frmcompra.etipo_baseicms.Checked = true then
          frmcompra.qrnota_item.fieldbyname('ICMS_BASE').asfloat := (frmcompra.qrnota_item.fieldbyname('TOTAL').asfloat + efrete.Value + eoutras.Value) - ((frmcompra.qrnota_item.fieldbyname('TOTAL').asfloat + efrete.Value + eoutras.Value) * frmcompra.qrnota_item.FieldByName('icms_reducao').asfloat / 100)
        else
          frmcompra.qrnota_item.fieldbyname('ICMS_BASE').asfloat := (frmcompra.qrnota_item.fieldbyname('TOTAL').asfloat) - ((frmcompra.qrnota_item.fieldbyname('TOTAL').asfloat + efrete.Value + eoutras.Value) * frmcompra.qrnota_item.FieldByName('icms_reducao').asfloat / 100);
    ...
    begin
      ...
      begin
        ...
        begin
          frmcompra.qrnota_item.fieldbyname('SUB_BASE').asfloat := (frmcompra.qrnota_item.fieldbyname('TOTAL').asfloat + efrete.Value + eoutras.Value + frmcompra_menu.qrnota.fieldbyname('ITEM_ESPECIAL_TOTAL').asfloat) + ((frmcompra.qrnota_item.fieldbyname('TOTAL').asfloat + efrete.Value + eoutras.Value + frmcompra_menu.qrnota.fieldbyname('ITEM_ESPECIAL_TOTAL').asfloat) * (frmcompra_menu.qrnota.fieldbyname('CONF_ICMSREDITO_PERC').asfloat / 100));
        end;
      ...
      begin
        ...
        begin
          ...
          if emargem_sub.Value = 0 then
          begin
            frmcompra.qrnota_item.FieldByName('SUB_BASE').asfloat := frmcompra.qrnota_item.FieldByName('total').asfloat + (frmcompra.qrnota_item.FieldByName('total').asfloat * (frmcompra.qrnota_item.FieldByName('sub_margem').asfloat / 100));

            frmcompra.qrnota_item.FieldByName('SUB_VALOR').asfloat := (frmcompra.qrnota_item.FieldByName('SUB_BASE').asfloat * (frmcompra.qrnota_item.fieldbyname('ICMS_ALIQUOTA').asfLOAT / 100)) - frmcompra.qrnota_item.fieldbyname('ICMS_VALOR').asfloat;
          end;
  ...
  begin
    (*************** ISENTO DE ICMS *********************************)
    if (frmcompra.qrnota_item.fieldbyname('CST').asstring = '040') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '030') then
    begin
    ...
    begin
      (************** NAO TRIBUTADO *********************************)
      if (frmcompra.qrnota_item.fieldbyname('CST').asstring = '041') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '050') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '090') then
      begin
      ...
      begin
        if frmcompra.qrnota_item.FieldByName('icms_reducao').asfloat > 0 then
          ...
          else if frmcompra.qrnota_item.fieldbyname('ICMS_ALIQUOTA').asfLOAT > 0 then
          begin
            if frmcompra.etipo_baseicms.Checked = true then
              frmcompra.qrnota_item.fieldbyname('ICMS_BASE').asfloat := (frmcompra.qrnota_item.fieldbyname('TOTAL').asfloat + efrete.Value + eoutras.Value)
            else
...
procedure Tfrmcompra_item.combo_ipiKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    if combo_ipi.ItemIndex > 0 then
...
procedure Tfrmcompra_item.evalor_ipiKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bgravar.setfocus;
end;
...
begin
  ...
  begin
    ...
    frmcompra.qrnota_item.fieldbyname('ipi_valor').asfloat := frmcompra.qrnota_item.fieldbyname('ipi_base').asfloat * (frmcompra.qrnota_item.fieldbyname('ipi_aliquota').asfloat / 100)
  end;

end;

procedure Tfrmcompra_item.AdvMetroButton1Click(Sender: TObject);
begin
  frmcompra.qrnota_item.Cancel;
  close;
end;
...
begin
  ...
  begin
    if (combo_retido.ItemIndex = 1) and (frmcompra_menu.qrnota.fieldbyname('CONF_ICMSREDITO_PERC').asfloat > 0) then // icms retido
    begin
      frmcompra.qrnota_item.fieldbyname('SUB_BASE').asfloat := (frmcompra.qrnota_item.fieldbyname('TOTAL').asfloat + efrete.Value + eoutras.Value + frmcompra.qrnota_item.fieldbyname('SUB_PRODUTOS').asfloat) + ((frmcompra.qrnota_item.fieldbyname('TOTAL').asfloat + efrete.Value + eoutras.Value) * (frmcompra_menu.qrnota.fieldbyname('CONF_ICMSREDITO_PERC').asfloat / 100));
    end
    else if eprod_sub.Value = 0 then
    begin
...
if eprod_sub.Value > 0 then
  begin
    frmcompra.qrnota_item.fieldbyname('SUB_BASE').asfloat := (etotal.Value * frmcompra.ebase_subst.Value) / frmcompra.qrnota_item.fieldbyname('SUB_PRODUTOS').asfloat;
    frmcompra.qrnota_item.fieldbyname('SUB_VALOR').asfloat := (etotal.Value * frmcompra.evalor_subst.Value) / frmcompra.qrnota_item.fieldbyname('SUB_PRODUTOS').asfloat;
  end;
...
begin
  if check_atualiza.Checked then
    check_atualiza.Checked := false
  else
    check_atualiza.Checked := true;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          frmmodulo.qrproduto.Close;
          frmmodulo.qrproduto.sql.clear;
          frmmodulo.qrproduto.sql.add('select * from c000025 where atb like :atb and codigo = ''' + ecodproduto.text + '''');
          frmmodulo.qrproduto.open;
          if frmmodulo.qrproduto.RecordCount > 0 then
          begin
            frmcompra.qrnota_item.FieldByName('codproduto').asstring := frmmodulo.qrproduto.fieldbyname('codigo').asstring;
...
procedure Tfrmcompra_item.evalor_outrasKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bgravar.setfocus;
end;
...
begin
  ...
  if (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '1152') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '1102') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '2102') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '1403') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '2403') then
  begin

    if (frmcompra.qrnota_item.fieldbyname('CST').asstring = '010') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '020') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '060') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '070') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '110') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '210') then
    begin

      if (frmcompra.qrnota_item.fieldbyname('CST').asstring = '020') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '070') then
      begin
        frmcompra.qrnota_item.fieldbyname('ICMS_ISENTO').AsFloat := ((esubtotal.Value - edesconto.Value) + efrete.Value + eseguro.Value + eoutras.Value) - ebase_icms.Value; //+ evalor_ipi.Value;
      end
      else
      begin
  ...
  begin
    ...
    begin
      ...
      if (frmcompra.qrnota_item.fieldbyname('CST').asstring = '041') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '050') or (frmcompra.qrnota_item.fieldbyname('CST').asstring = '090') then
      begin
        frmcompra.qrnota_item.fieldbyname('ICMS_NAOTRIBUTADO').AsFloat := ((esubtotal.Value - edesconto.Value) + efrete.Value + eseguro.Value + eoutras.Value) - ebase_icms.Value; //+ evalor_ipi.Value;
      end
    end;
  end;

  if ((frmcompra.qrnota_item.fieldbyname('CST').asstring = '000') and (frmcompra.qrnota_item.fieldbyname('ICMS_VALOR').AsFloat = 0)) or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '1553') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '1653') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '1556') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '2352') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '2556') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring
    = '1407') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '2407') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '1303') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '1253') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '1551') or (frmcompra.qrnota_item.fieldbyname('CFOP').asstring = '2551') then
  begin
    frmcompra.qrnota_item.fieldbyname('ICMS_OUTRAS').AsFloat := ((esubtotal.Value - edesconto.Value) + efrete.Value + eseguro.Value + eoutras.Value); //+evalor_ipi.Value;
  end;
  ...
  if evalor_ipi.Value > 0 then
  begin
    frmcompra.qrnota_item.fieldbyname('ICMS_OUTRAS').AsFloat := frmcompra.qrnota_item.fieldbyname('ICMS_OUTRAS').AsFloat + evalor_ipi.Value;
  end;
