...
uses
  ...
  Mask, maskutils, ImgList, ComCtrls, Wwdbigrd, Wwdbgrid, Wwkeycb, AdvGlowButton,
  frxDesgn, frxClass, frxDBSet, UCBase, AdvMetroButton, AdvSmoothPanel,
...
uses
  modulo, principal, contacorrente_ficha, contacorrente_lancamento;
...
begin
  ...
  conta_selecionada := '';
  SetWindowLong(arvore.Handle, GWL_STYLE, GetWindowLong(arvore.Handle, GWL_STYLE) or $80);

  frmmodulo.qrbanco.close;
  frmmodulo.qrbanco.SQL.clear;
  frmmodulo.qrbanco.SQL.add('select * from c000013 WHERE atb like :atb order by numero desc');
  ...
  try
    ...
    begin
      ...
      frmmodulo.qrcontacorrente.SQL.add('select * from c000041 where atb like :atb and codbanco = ''' + frmmodulo.qrbanco.fieldbyname('numero').asstring + ''' order by conta');
  ...
  frmmodulo.qrbanco.close;
  frmmodulo.qrbanco.SQL.clear;
  frmmodulo.qrbanco.SQL.add('select * from c000013 WHERE atb like :atb order by BANCO');
  frmmodulo.qrbanco.open;
  frmmodulo.qrbanco.INDEXFIELDNAMES := 'BANCO';
...
begin
  ...
  begin
    ...
    if qrlancamento_conta.fieldbyname('tipo').asstring = 'D' then
      qrlancamento_conta.fieldbyname('VALOR').ASFLOAT := qrlancamento_conta.fieldbyname('VALOR').ASFLOAT * (-1);
    frmcontacorrente_lancamento := tfrmcontacorrente_lancamento.create(self);
    frmcontacorrente_lancamento.showmodal;
    bfiltrarClick(frmcontacorrente);
  end
...
procedure Tfrmcontacorrente.RxDBCalcEdit2KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bgravar.setfocus;
end;
...
procedure Tfrmcontacorrente.wwDBGrid1DrawDataCell(Sender: TObject; const Rect: TRect; Field: TField; State: TGridDrawState);
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        frmprincipal.logUC('Excluiu Conta Corrente - Nr: ' + conta_selecionada, 3);

        qrlancamento_conta.close;
        qrlancamento_conta.SQL.clear;
        qrlancamento_conta.SQL.add('delete from c000042 where atb like :atb and codconta = ''' + conta_selecionada + '''');
        qrlancamento_conta.ParamByName('atb').Value := ME_FiltraATB('TB_MOV_CONTA_CORRENTE');
        qrlancamento_conta.ExecSQL;

        frmmodulo.qrcontacorrente.close;
        frmmodulo.qrcontacorrente.SQL.clear;
        frmmodulo.qrcontacorrente.SQL.add('delete from c000041 where atb like :atb and codigo = ''' + conta_selecionada + '''');
        frmmodulo.qrcontacorrente.ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_BANCARIA');
...
begin
  ...
  begin
    ...
    frmmodulo.qrcontacorrente.close;
    frmmodulo.qrcontacorrente.SQL.clear;
    frmmodulo.qrcontacorrente.SQL.add('select * from c000041 where atb like :atb and CODIGO = ''' + conta_selecionada + '''');
    frmmodulo.qrcontacorrente.open;

    qrsaldo_anterior.close;
    qrsaldo_anterior.SQL.clear;
    qrsaldo_anterior.SQL.add('select sum(valor) saldo_anterior from c000042 where atb like :atb and codcontacorrente = ''' + conta_selecionada + ''' and data < :datai');
    qrsaldo_anterior.Params.ParamByName('datai').value := DateEdit1.date;
    qrsaldo_anterior.ParamByName('atb').Value := ME_FiltraATB('TB_MOV_CONTA_CORRENTE');
    qrsaldo_anterior.open;
    rsaldo_anterior.value := qrsaldo_anterior.fieldbyname('saldo_anterior').ASFLOAT;

    qrsaldo_anterior.close;
    qrsaldo_anterior.SQL.clear;
    qrsaldo_anterior.SQL.add('select sum(valor) saldo_atual from c000042 where atb like :atb and codcontacorrente = ''' + conta_selecionada + ''' and data >= :datai and data <= :dataf');
    qrsaldo_anterior.Params.ParamByName('datai').value := DateEdit1.date;
    ...
    qrlancamento_conta.close;
    qrlancamento_conta.SQL.clear;
    qrlancamento_conta.SQL.add('select * from c000042 where atb like :atb and codcontacorrente = ''' + conta_selecionada + ''' and data >= :datai and data <= :dataf order by data, codigo');
...
procedure Tfrmcontacorrente.arvoreKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
begin
  ...
  fxcontacorrente.LoadFromFile('\SOS\server\rel\f000152.fr3');
...
procedure Tfrmcontacorrente.qrlancamento_contaNewRecord(DataSet: TDataSet);
begin
  qrlancamento_contaATB.AsString := ME_GravaATB('TB_CONTA_BANCARIA');
end;
