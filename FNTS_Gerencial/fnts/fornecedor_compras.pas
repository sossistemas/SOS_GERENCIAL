...
uses
  ...
  Dialogs, AdvToolBar, DB, ImgList,
  Menus, Grids, Wwdbigrd, Wwdbgrid, StdCtrls, RzEdit,
  Mask, AdvGlowButton, ZAbstractRODataset, ZDataset, DBCtrls,
  ExtCtrls, AdvMetroButton, AdvSmoothExpanderPanel, JvExMask,
  JvToolEdit, ZAbstractDataset, AdvSmoothPanel;
...
type
  Tfrmfornecedor_compras = class(TForm)
    ...
    qrcomprasFATOR: TFloatField;
    qrcomprasTIPOFATOR: TWideStringField;
    qrcomprasCSOSN: TWideStringField;
    qrcomprasCFOP_CAD: TWideStringField;
    qrcomprasVBCSTRET: TFloatField;
    qrcomprasPST: TFloatField;
    qrcomprasVICMSSTRET: TFloatField;
    qrcomprasPREDBCEFET: TFloatField;
    qrcomprasVBCEFET: TFloatField;
    qrcomprasPICMSEFET: TFloatField;
    qrcomprasVICMSEFET: TFloatField;
    qrcomprasQTDVENDIDO: TFloatField;
    qrcomprasID_CONVERSOR_UNIDADE: TIntegerField;
    qrcomprasDESCRICAO_FATOR: TWideStringField;
    qrcomprasID_CONVERSAO_UNIDADE: TIntegerField;
    qrcomprasATB: TWideStringField;
    ...
    qrcomprasCSOSN_1: TWideStringField;
    ...
    qrcomprasORIGEM: TIntegerField;
    qrcomprasIND_CFOP_NFCE: TWideStringField;
    qrcomprasFOTOBD: TBlobField;
    qrcomprasID_TIPO_SERVICO: TIntegerField;
    qrcomprasNAO_MOVIMENTA_ESTOQUE: TWideStringField;
    qrcomprasPESAGEM_AUOTMATICA: TWideStringField;
    qrcomprasINFORMA_CODIGO_BARRA_XML: TWideStringField;
    qrcomprasCODIGO_ANP: TWideStringField;
    qrcomprasCOMBO: TWideStringField;
    qrcomprasIND_CFOP_DEV_COMPRA_DENTRO: TWideStringField;
    qrcomprasIND_CFOP_DEV_COMPRA_FORA: TWideStringField;
    qrcomprasPIZZA: TWideStringField;
    qrcomprasPERCGLNN: TFloatField;
    qrcomprasPERCGLGNI: TFloatField;
    qrcomprasPGLP: TFloatField;
    qrcomprasVPART: TFloatField;
    qrcomprasECF_ICMS: TWideStringField;
    qrcomprasIDONLINE: TIntegerField;
    qrcomprasEMDESTAQUE: TWideStringField;
    qrcomprasDESCONTOMAXIMO: TFloatField;
    qrcomprasATB_1: TWideStringField;
...
procedure Tfrmfornecedor_compras.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  if (enumero.text = '000000') or (enumero.text = '') then
    ...
    nota := ' and n.numero = ''' + enumero.text + '''';
  qrcompras.close;
  qrcompras.sql.clear;
  qrcompras.SQL.add('select i.*,p.*, n.* from c000088 i,  c000087 n, c000025 p');
  qrcompras.SQL.add('where i.atb like :atbi and n.atb like :atbn and p.atb like :atbp and n.codfornecedor = ''' + frmmodulo.qrfornecedor.FieldByName('CODIGO').AsString + '''');
  ...
  qrcompras.ParamByName('atbi').Value := ME_FiltraATB('TB_NOTA_ENTRADA_ITENS');
  qrcompras.ParamByName('atbn').Value := ME_FiltraATB('TB_NOTA_ENTRADA');
  qrcompras.ParamByName('atbp').Value := ME_FiltraATB('TB_PRODUTO');
  qrcompras.open;
...
procedure Tfrmfornecedor_compras.edata_inicioKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
...
procedure Tfrmfornecedor_compras.ENUMEROKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bfiltrar.setfocus;
end;
