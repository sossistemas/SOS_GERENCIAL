...
uses
  ...
  Dialogs, DB, Grids, Wwdbigrd, Wwdbgrid, ExtCtrls, StdCtrls, Mask, RzEdit,
  RzBtnEdt, AdvGlowButton, ZAbstractRODataset, ZAbstractDataset, ZDataset, Menus,
  AdvMenus, DBCtrls, UCBase, AdvMetroButton, AdvSmoothPanel,
  AdvSmoothExpanderPanel, JvDBControls, JvExMask, JvToolEdit;
...
uses
  modulo, principal, xloc_cliente, cheque, Cheque_baixa, lista_cheque2;
...
begin
  ...
  qrcheque.FIELDBYNAME('ATB').ASSTRING := ME_GravaATB('TB_CHEQUE');
...
begin
  ...
  begin
    ...
    frmprincipal.logUC('Excluiu Cheque - Nr: ' + qrcheque.FieldByName('numero').AsString + ' CodCliente: ' + qrcheque.FieldByName('codcliente').AsString, 3);
...
begin
  ...
  qrcheque.close;
  qrcheque.sql.Clear;
  qrcheque.sql.add('select * from c000040 where atb like :atb and vencimento BETWEEN :datai and :dataf ' + cliente + cheque + situacao + ' order by vencimento');
...
procedure Tfrmcheque_menu.combo_situacaoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bfiltrar.setfocus;
end;
