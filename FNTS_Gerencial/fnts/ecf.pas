...
begin
  ...
  if FileExists('C:\SOS\RETORNO.TXT') then DeleteFile('C:\SOS\RETORNO.TXT');
  ...
  begin
    ...
    assignfile(f, 'C:\SOS\retorno.txt');
    rewrite(f);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        texto := PWideChar('Não foi possível executar o comando!' + #13 + 'Motivo: ' + 'Impressora Fiscal não responde.');
        APPLICATION.MESSAGEBOX(texto, 'Atenção', mb_ok + MB_ICONerror);
        RESULT := 'ERRO';
      end
      else
      begin
        texto := PWideChar('Não foi possível executar o comando!' + #13 + 'Motivo: ' + copy(rt, 11, 100) + '.');
        APPLICATION.MESSAGEBOX(texto, 'Atenção', mb_ok + MB_ICONerror);
        RESULT := 'ERRO';
      end;
    end
  ...
  begin
    ...
    begin
      texto := PWideChar('Não foi possível executar o comando!' + #13 + 'Motivo: ' + texto + '.');
      APPLICATION.MESSAGEBOX(texto, 'Atenção', mb_ok + MB_ICONerror);
      RESULT := 'ERRO';
    end;
