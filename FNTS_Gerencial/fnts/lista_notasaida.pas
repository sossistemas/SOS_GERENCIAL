...
uses
  ...
  Menus, AdvGlowButton, AdvMetroButton, AdvSmoothExpanderPanel,
...
procedure Tfrmlista_notasaida.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
begin
  ...
  case COMBO_ORDEM.ITEMINDEX of
    0:
      ordem := ' ORDER BY DATA, NUMERO';
    1:
      ordem := ' ORDER BY DATA_SAIDA, NUMERO';
    2:
  ...
  begin
    ...
    if ecfop.text <> '' then
      ...
      cfop := '';
    if combo_movimento.ItemIndex = 0 then
      movimento := '';
    if combo_movimento.ItemIndex = 1 then
      movimento := ' and movimento = ''S''';
    if combo_movimento.ItemIndex = 2 then
      movimento := ' and movimento = ''E''';
    ...
    qrnota.close;
    qrnota.sql.clear;
    qrnota.SQL.add('select * from c000061 where atb like :atb and data BETWEEN :datai and :dataf ' + fornecedor_imp + nota + SITUACAO + movimento + cfop + ordem);
    qrnota.params.ParamByName('datai').asdatetime := dateedit1.date;
    qrnota.params.ParamByName('dataf').asdatetime := dateedit2.date;
    qrnota.ParamByName('atb').Value := ME_FiltraATB('TB_NOTA_FISCAL_SAIDA');
    qrnota.open;

    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELAÇÃO DE NOTA FISCAL DE SAÍDA';
    FXNOTA.LoadFromFile('\SOS\server\rel\f000089.fr3');
    FXNOTA.ShowReport;
  end;
  ...
  begin
    if combo_situacao.ItemIndex = 0 then
      ...
      SITUACAO := ' and situacao = ' + inttostr(combo_situacao.itemIndex);
    ...
    FXNOTA.LoadFromFile('\SOS\server\rel\f000089_2.fr3');
    FXNOTA.ShowReport;
  end;
  ...
  begin
    ...
    FXNOTA.LoadFromFile('\SOS\server\rel\f000061_2.fr3');
    FXNOTA.ShowReport;
  end;
  ...
  begin
    ...
    FXNOTA.LoadFromFile('\SOS\server\rel\f000061_3.fr3');
    FXNOTA.ShowReport;
  end;
...
procedure Tfrmlista_notasaida.combo_relatorioKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
