...
uses
  ...
  Menus, Grids, DBGrids, wwdblook, AdvGlowButton, Vcl.Imaging.jpeg,
...
uses
  modulo, xloc_cliente, principal, xloc_banco;
...
procedure Tfrmlista_cheque2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
end;
...
var
  cliente, banco, cedente, caixa, tipo, forma, situacao, periodo, ordem, conta: string;
begin
  ...
  if combo_banco.Text = 'TODOS' then
    banco := ''
  else
  begin
    banco := ' and CH.CODbanco = ''' + copy(combo_banco.text, 1, 6) + '''';
    frmmodulo.qrrelatorio.fieldbyname('LINHA4').asstring := 'VENEDEDOR: ' + COMBO_banco.TEXT;
  end;
  ...
  begin
    QRCHEQUE.close;
    QRCHEQUE.sql.clear;
    QRCHEQUE.sql.add('select ch.*, cli.nome, cli.telefone1, cli.telefone2 from c000040 ch, c000007 cli where ch.atb like :atbch and cli.atb like :atbcli and ch.codcliente = cli.codigo AND ' + periodo + cliente + banco + situacao + ' order by ' + ordem);
    QRCHEQUE.Params.ParamByName('datai').asdatetime := dateedit1.date;
    QRCHEQUE.Params.ParamByName('dataf').asdatetime := dateedit2.date;

    QRCHEQUE.ParamByName('atbch').Value := ME_FiltraATB('TB_CHEQUE');
    QRCHEQUE.ParamByName('atbcli').Value := ME_FiltraATB('TB_CLIENTE');

    QRCHEQUE.open;
    QRCHEQUE.REFRESH;

    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELATÓRIO DE CHEQUES';
    fxCHEQUE.LoadFromFile('\SOS\server\rel\f000083.fr3');
    fxCHEQUE.ShowReport;
  end;
  ...
  begin
    QRCHEQUE.close;
    QRCHEQUE.sql.clear;
    QRCHEQUE.sql.add('select ch.*, cli.nome, cli.telefone1, cli.telefone2 from c000040 ch, c000007 cli where ch.atb like :atbch and cli.atb like :atbcli and ch.codcliente = cli.codigo AND ' + periodo + cliente + banco + situacao + ' order by cli.nome');
    QRCHEQUE.Params.ParamByName('datai').asdatetime := dateedit1.date;
    QRCHEQUE.Params.ParamByName('dataf').asdatetime := dateedit2.date;
    ...
    frmmodulo.qrrelatorio.fieldbyname('LINHA1').asstring := 'RELATÓRIO DE CHEQUES';
    fxCHEQUE.LoadFromFile('\SOS\server\rel\f000082.fr3');
    fxCHEQUE.ShowReport;
  end;
...
begin
  case qrchequeSITUACAO.Value of
    1:
      qrcheque.fieldbyname('SIT').Value := 'ATIVO';
    2:
      qrcheque.fieldbyname('SIT').Value := 'Dep.1a.vez';
    3:
      qrcheque.fieldbyname('SIT').Value := 'Dep.2a.vez';
    4:
      qrcheque.fieldbyname('SIT').Value := 'Descontado';
    5:
      qrcheque.fieldbyname('SIT').Value := 'Dev.1a.vez';
    6:
      qrcheque.fieldbyname('SIT').Value := 'Dev.2a.vez';
    7:
      qrcheque.fieldbyname('SIT').Value := 'Repassado';
    8:
      qrcheque.fieldbyname('SIT').Value := 'Sustado';
    9:
      qrcheque.fieldbyname('SIT').Value := 'Outros';
  end;
