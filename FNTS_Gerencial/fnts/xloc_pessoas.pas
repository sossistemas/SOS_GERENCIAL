...
procedure Tfrmxloc_Pessoas.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;
...
procedure Tfrmxloc_Pessoas.locKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = vk_down then
  begin
    grid.setfocus;
...
begin
  ...
  begin
    ...
    begin
      resultado_pesquisa1 := query.fieldbyname('nome').asstring;
      close;
    end;
  end;
end;
...
begin
  ...
  query.Close;
  query.SQL.clear;
  query.SQL.Add('select * from c000070 where atb like :atb and codigo is not null ' + parametro_pesquisa + ' ORDER BY nome');
  query.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE_PARENTESCO');
  query.open;
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000070 where atb like :atb and upper(' + tipo + ') like ''' + ref + loc.Text + '%''' + parametro_pesquisa + ' order by nome');
    query.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE_PARENTESCO');
    query.open;
  except
  end;
