...
uses
  ...
  StdCtrls, cxStyles, cxGraphics, cxDataStorage,
  cxEdit, DB, cxDBData, cxGridLevel, cxClasses, cxControls, cxGridCustomView,
  cxGridCustomTableView, cxGridTableView, cxGridDBTableView, cxGrid,
  ZAbstractRODataset, ZDataset, cxGridCardView,
  cxGridDBCardView, cxBlobEdit, cxDBLookupComboBox, cxImage, cxLookAndFeels,
  Buttons, Menus, PageView, frxDesgn, frxClass, frxDBSet, cxCheckBox, Mask,
  AdvGlowButton, cxLookAndFeelPainters, dxSkinsCore, dxSkinBlack, dxSkinBlue,
  ...
  dxSkinXmas2008Blue, dxSkinscxPCPainter, RzPanel,
  dxSkinOffice2013White, cxNavigator, dxSkinMetropolis, dxSkinMetropolisDark,
  dxSkinOffice2013DarkGray, dxSkinOffice2013LightGray, AdvMetroButton,
  AdvSmoothExpanderPanel, JvMemoryDataset, JvExMask, JvToolEdit, cxCustomData,
  cxFilter, cxData, ZAbstractDataset, AdvSmoothPanel, cxGridCustomLayoutView;
...
uses
  modulo, xloc_convenio, principal, convenio_pgto;

{$R *.dfm}

procedure Tfrmconvenio_receber.locconvenioKeyPress(Sender: TObject; var Key: Char);
var
  dia: integer;
begin
  if Key = #13 then
  begin
    ...
    qrconvenio.sql.add('select * from c000071 where atb like :atb and upper(nome) like ''' + AnsiUpperCase(locconvenio.text) + '%'' order by nome');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          qrcliente.sql.add('rec.atb like :atbrec and ');
          qrcliente.sql.add('cli.atb like :atbcli and ');
          ...
          qrcliente.Params.ParamByName('atbrec').Value := ME_FiltraATB('TB_CONTA_RECEBER');
          qrcliente.Params.ParamByName('atbcli').Value := ME_FiltraATB('TB_CLIENTE');
          qrcliente.open;
...
begin
  ...
  qrproduto.sql.add('MOV.atb like :atbMOV and ');
  qrproduto.sql.add('PRO.atb like :atbPRO and ');

  qrproduto.sql.add('MOV.CODPRODUTO = PRO.CODIGO');
  qrproduto.sql.add('AND MOV.MOVIMENTO <> 9');
  qrproduto.ParamByName('atbMOV').Value := ME_FiltraATB('TB_MOVIMENTO');
  ...
  qrproduto2.sql.add('MOV.atb like :atbMOV and ');
  qrproduto2.sql.add('PRO.atb like :atbPRO and ');

  qrproduto2.sql.add('MOV.CODPRODUTO = PRO.CODIGO');
  qrproduto2.sql.add('AND MOV.MOVIMENTO <> 9');
  qrproduto2.ParamByName('atbMOV').Value := ME_FiltraATB('TB_MOVIMENTO');
  qrproduto2.ParamByName('atbPRO').Value := ME_FiltraATB('TB_PRODUTO');
  qrproduto2.open;
  ...
  qrproduto3.sql.add('MOV.atb like :atbMOV and ');
  qrproduto3.sql.add('PRO.atb like :atbPRO and ');
  qrproduto3.sql.add('CLI.atb like :atbCLI and ');
  qrproduto3.sql.add('CR.atb like :atbCR and ');
  ...
  qrproduto3.ParamByName('atbMOV').Value := ME_FiltraATB('TB_MOVIMENTO');
  qrproduto3.ParamByName('atbPRO').Value := ME_FiltraATB('TB_PRODUTO');
  qrproduto3.ParamByName('atbCLI').Value := ME_FiltraATB('TB_CLIENTE');
  qrproduto3.ParamByName('atbCR').Value := ME_FiltraATB('TB_CONTA_RECEBER');
  qrproduto3.open;
  ...
  qrreceber.sql.add('CR.atb like :atbCR and ');
  qrreceber.sql.add('CLI.atb like :atbCLI and ');
  ...
  qrreceber.ParamByName('atbCR').Value := ME_FiltraATB('TB_CONTA_RECEBER');
  qrreceber.ParamByName('atbCLI').Value := ME_FiltraATB('TB_CLIENTE');
  qrreceber.open;
...
begin
  ...
  fxconvenio.LoadFromFile('\SOS\server\rel\f000087.fr3');
...
begin
  ...
  query.sql.add('where');
  query.sql.add('rec.atb like :atbrec and ');
  query.sql.add('CLI.atb like :atbCLI and ');
  ...
  query.ParamByName('atbrec').Value := ME_FiltraATB('TB_CONTA_RECEBER');
  query.ParamByName('atbCLI').Value := ME_FiltraATB('TB_CLIENTE');
  query.open;
  ...
  fxconvenio.LoadFromFile('\SOS\server\rel\f000136.fr3');
  fxconvenio.ShowReport;
