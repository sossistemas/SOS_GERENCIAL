...
uses
  ...
  ImgList, SHELLAPI, ComCtrls, RzDBEdit, RzDBBnEd, AdvGlowButton,
  AdvToolBar, RzPanel, ExtDlgs, AdvPanel, UCBase,
  AdvSmoothPanel, AdvMenus, dxGDIPlusClasses,
  sBitBtn, W7Classes,
  AdvSmoothExpanderPanel, AdvMetroButton, pngimage, JPEG,
  JvToolEdit,
  JvBaseEdits, JvDBControls, JvExMask, AdvSmoothButton, System.ImageList,
  ZAbstractDataset, RzEdit;
...
uses
  ...
  Math, lista_servicos_periodicos, funcionario, convenio, xloc_convenio,
  cliente_inicio, xloc_cidade, webcam2, xloc_cnae, loc_paises, RetornaEndereco,
  ConsultaCPF, ConsultaCNPJ, xloc_fornecedor, UFuncoes;
...
begin
  ...
  rvalor.DisplayFormat := mascara_valor;
  rjuro.DisplayFormat := mascara_valor;
  rtotal.DisplayFormat := mascara_valor;
  rtotal_pago.DisplayFormat := mascara_valor;
  qrprodutoQTDE.DisplayFormat := mascara_qtde;
  qrprodutoUNITARIO.DisplayFormat := mascara_valor;
  qrprodutoTOTAL.DisplayFormat := mascara_valor;
  ...
  FRMMODULO.qrconvenio.sql.add('select * from c000071 where atb like :atb order by nome');
...
begin
  ...
  begin
    ...
    qrfornecedor.open;
    if not qrfornecedor.IsEmpty then
    begin
...
begin
  ...
  begin
    ...
    qrduplo.close;
    qrduplo.sql.clear;
    qrduplo.sql.add('select * from c000049 where atb like :atb and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''');
    qrduplo.ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_RECEBER');
    qrduplo.OPEN;
    ...
    begin
      qrduplo.close;
      qrduplo.sql.clear;
      qrduplo.sql.add('select * from c000048 where atb like :atb and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''');
      qrduplo.ParamByName('atb').Value := ME_FiltraATB('TB_VENDAS');
      qrduplo.OPEN;
      if qrduplo.recordcount > 0 then
      begin
...
begin
  ...
  begin
    ...
    if RzDBButtonEdit1.Text <> '0000' then
    begin
      if not frmprincipal.Locregistro(FRMMODULO.qrpais, RzDBButtonEdit1.Text, 'codigo') then
      begin
...
begin
  ...
  begin
    ...
    begin
      if not frmprincipal.Locregistro(FRMMODULO.qrregiao, dbedit28.Text, 'codigo') then
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Application.MessageBox('Vendedor inválido!', 'Atenção!', MB_ICONINFORMATION);
          dbedit29.SetFocus;
          Exit;
        end;
      end
...
begin
  ...
  begin
    if (FRMMODULO.qrcliente.State = dsinsert) or (FRMMODULO.qrcliente.State = dsedit) then
    begin
      if (combo_pessoa.ItemIndex = 0) or (combo_pessoa.ItemIndex = 2) then
      begin
      ...
      begin
        ...
        begin
          query1.close;
          query1.sql.clear;
          query1.sql.add('select * from c000007 where atb like :atb and cpf = ''' + DBEdit15.Text + '''');
          query1.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
          query1.OPEN;
...
begin
  ...
  begin
    QUERY.close;
    QUERY.sql.clear;
    QUERY.sql.add('select SUM(TOTAL) TOTAL_GERAL from c000048 where atb like :atb and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''');
    QUERY.ParamByName('atb').Value := ME_FiltraATB('TB_VENDAS');
    QUERY.OPEN;
    LTOTAL.caption := FORMATFLOAT(mascara_valor, QUERY.FieldByName('TOTAL_GERAL').asfloat);

    qrvenda.close;
    qrvenda.sql.clear;
    qrvenda.sql.add('select * from c000048 where atb like :atb and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + ''' order by data, codigo');
    qrvenda.ParamByName('atb').Value := ME_FiltraATB('TB_VENDAS');
    qrvenda.OPEN;

    qrproduto.close;
    qrproduto.sql.clear;
    qrproduto.sql.add('select ven.codigo, pro.produto, pro.unidade, ven.qtde, ven.unitario, ven.total, ven.serial from c000032 ven inner join c000025 pro on pro.codigo = ven.codproduto where ven.atb like :atbven and pro.atb like :atbpro and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''');
    qrproduto.ParamByName('atbven').Value := ME_FiltraATB('TB_VENDAS');
    qrproduto.ParamByName('atbpro').Value := ME_FiltraATB('TB_PRODUTO');
    qrproduto.OPEN;
    ...
    qrcontasreceber.sql.add('select * from c000049 where atb like :atb and codcliente =''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + ''' and situacao = 1 order by data_vencimento, codigo');
    ...
    qrcontasreceberpg.sql.add('select * from c000049 where atb like :atb and codcliente =''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + ''' and situacao = 2 order by data_vencimento, codigo');
    qrcontasreceberpg.ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_RECEBER');
    ...
    qrvenda_mes.sql.add('select sum(total) sum_0, ' +
       '(select sum(total) from c000048 where atb like :atb and data >= :data11 and data < :data10 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''), ' +
       '(select sum(total) from c000048 where atb like :atb and data >= :data10 and data < :data09 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''), ' +
       '(select sum(total) from c000048 where atb like :atb and data >= :data09 and data < :data08 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''), ' +
       '(select sum(total) from c000048 where atb like :atb and data >= :data08 and data < :data07 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''), ' +
       '(select sum(total) from c000048 where atb like :atb and data >= :data07 and data < :data06 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''), ' +
       '(select sum(total) from c000048 where atb like :atb and data >= :data06 and data < :data05 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''), ' +
       '(select sum(total) from c000048 where atb like :atb and data >= :data05 and data < :data04 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''), ' +
       '(select sum(total) from c000048 where atb like :atb and data >= :data04 and data < :data03 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''), ' +
       '(select sum(total) from c000048 where atb like :atb and data >= :data03 and data < :data02 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''), ' +
       '(select sum(total) from c000048 where atb like :atb and data >= :data02 and data < :data01 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''), ' +
       '(select sum(total) from c000048 where atb like :atb and data >= :data01 and data < :data00 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + ''')  ' +
       'from c000048 where atb like :atb and data >= :data12 and data < :data11 and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''');
    ...
    qrvenda_mes.ParamByName('atb').Value := ME_FiltraATB('TB_VENDAS');
    qrvenda_mes.OPEN;
  ...
  begin
    qrpessoas.close;
    qrpessoas.sql.clear;
    qrpessoas.sql.add('SELECT * FROM C000070 WHERE atb like :atb and CODCLIENTE = ''' + FRMMODULO.qrcliente.FieldByName('CODIGO').asstring + ''' ORDER BY NOME');
  ...
  begin
    ...
    FRMMODULO.qrservicos_periodicos.sql.add('where atb like :atb and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('CODIGO').asstring + '''');
...
begin
  qrproduto.close;
  qrproduto.sql.clear;
  qrproduto.sql.add('select ven.codigo, pro.produto, pro.unidade, ven.qtde, ven.unitario, ven.total, ven.serial from c000032 ven inner join c000025 pro on pro.codigo = ven.codproduto where ven.atb like :atbven and pro.atb like :atbpro and codnota = ''' + qrvenda.FieldByName('codigo').asstring + ''' and codcliente = ''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + '''');
  qrproduto.ParamByName('atbven').Value := ME_FiltraATB('TB_VENDAS');
  qrproduto.ParamByName('atbpro').Value := ME_FiltraATB('TB_PRODUTO');
  qrproduto.OPEN;
  ...
  qrcontasreceber.close;
  qrcontasreceber.sql.clear;
  qrcontasreceber.sql.add('select * from c000049 where atb like :atb and codvenda = ''' + qrvenda.FieldByName('codigo').asstring + ''' and codcliente =''' + FRMMODULO.qrcliente.FieldByName('codigo').asstring + ''' and situacao = 1 order by data_vencimento, codigo');
  qrcontasreceber.ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_RECEBER');
  qrcontasreceber.OPEN;
...
begin
  ...
  begin
    ...
    begin
      ...
      qrpgto.close;
      qrpgto.sql.clear;
      qrpgto.sql.add('select * from c000050 where atb like :atb and CODCONTA = ''' + Copy(qrcontasreceberpg.FieldByName('codigo').asstring, 1, 9) + ''' and codcliente = ''' + qrcontasreceber.FieldByName('codcliente').asstring + ''' order by data');
      qrpgto.ParamByName('atb').Value := ME_FiltraATB('TB_CONTA_RECEBER_PGTO');
      qrpgto.OPEN;
      Memo1.Text := '';
      qrpgto.first;
      while not qrpgto.eof do
      begin
        Memo1.lines.add(qrpgto.FieldByName('data').asstring + ' -->' + frmprincipal.texto_justifica(FORMATFLOAT(mascara_valor, qrpgto.FieldByName('valor_pago').asfloat), 12, ' ', 'E'));
        qrpgto.next;
      end;
      PPGTO.visible := true;
    end;
...
begin
  ...
  begin
    ...
    qrduplo.sql.add('where atb like :atb and upper(NOME) = ''' + ansiuppercase(DBEdit2.Text) + ''' and codigo <> ''' + DBEdit1.Text + '''');
    qrduplo.ParamByName('atb').Value := ME_FiltraATB('TB_CLIENTE');
    qrduplo.OPEN;
    if qrduplo.recordcount > 0 then
    begin
...
begin
  ...
  else if combo_pessoa.ItemIndex = 1 then
  begin
...
begin
  if (DBEdit14.Text <> '') and (UpperCase(DBEdit14.Text) <> 'ISENTO') then
  begin
    DBComboBox3.ItemIndex := 0;
  end
  else if (UpperCase(DBEdit14.Text) = 'ISENTO') then
  begin
