...
uses
  ...
  Dialogs, StdCtrls, Buttons, ExtCtrls, Mask, Collection, TFlatPanelUnit, DB,
  ZAbstractRODataset, ZAbstractDataset, ZDataset, Grids, Wwdbigrd, Wwdbgrid,
  Menus, frxDesgn, frxClass, frxDBSet, AdvGlowButton, VirtualTable, UCBase,
  AdvMetroButton, AdvSmoothPanel, AdvSmoothExpanderPanel, MemDS, JvExMask,
  JvToolEdit;
...
uses
  modulo, xloc_subgrupo, principal;

{$R *.dfm}

procedure Tfrmproduto_falta.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
begin
  ...
  FRMMODULO.qrproduto.SQL.Add('select * from C000025 where atb like :atb order by codigo');
  ...
  qrdata.SQL.Add('  pro.atb like :atbpro and est.atb like :atbest and mov.atb like :atbmov');
  ...
  qrdata.ParamByName('atbpro').Value := ME_FiltraATB('TB_PRODUTO');
  qrdata.ParamByName('atbest').Value := ME_FiltraATB('TB_ESTOQUE');
  qrdata.ParamByName('atbmov').Value := ME_FiltraATB('TB_MOVIMENTO');
  qrdata.Open;
  ...
  begin
    ...
    if FRMMODULO.qrproduto.Locate('CODIGO', qrdata.FIELDBYNAME('CODIGO').ASSTRING, [loCaseInsensitive]) then
    begin
      qrproduto.FIELDBYNAME('peso').ASfloat := FRMMODULO.qrproduto.FIELDBYNAME('ESTOQUEMINIMO').ASfloat - qrdata.FIELDBYNAME('estoque_atual').ASfloat;
...
procedure Tfrmproduto_falta.combo_subgrupoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    combo_estoque.setfocus;
end;

procedure Tfrmproduto_falta.combo_estoqueKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    bfiltrar.setfocus;
end;
...
begin
  frmPrincipal.logUC('Ecluiu Produto/Faltas - ' + qrproduto.FIELDBYNAME('produto').ASSTRING, 3);
  qrproduto.Delete;
end;
...
begin
  ...
  fxproduto.LoadFromFile('\SOS\server\rel\f000084.fr3');
