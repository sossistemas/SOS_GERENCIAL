...
uses
  ...
  Dialogs, StdCtrls, Mask, ExtCtrls, Grids, Wwdbigrd, Wwdbgrid, DB, DBAccess,
  VirtualTable, RzEdit, RzBtnEdt, AdvGlowButton, ZAbstractRODataset,
  ZAbstractDataset, ZDataset, DBClient, JvExMask, JvToolEdit;
...
uses
  loc_fornecedor, modulo, principal, produto;

{$R *.dfm}

procedure TfrmProduto_serial_ficha.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := CAFREE;
end;
...
procedure TfrmProduto_serial_ficha.DateEdit1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    perform(wm_nextdlgctl, 0, 0);
end;

procedure TfrmProduto_serial_ficha.Edit1KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    perform(wm_nextdlgctl, 0, 0);
end;

procedure TfrmProduto_serial_ficha.ed_precoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    grid.setfocus;
end;

procedure TfrmProduto_serial_ficha.ed_cod_fornecedorButtonClick(Sender: TObject);
begin
...
procedure TfrmProduto_serial_ficha.ed_cod_fornecedorKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
begin
  ...
  begin
    ...
    query.sql.add('datacompra,numeronota,codfornecedor,codnota,atb)');
    ...
    query.parambyname('atb').asstring := ME_GravaATB('TB_SERIAL_PRODUTO');
    query.ExecSQL;
    qrserial.next;
  end;
